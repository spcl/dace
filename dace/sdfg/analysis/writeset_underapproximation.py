...
def _find_unconditionally_executed_states(cfg: ControlFlowRegion) -> Set[SDFGState]:
    """
    Returns all states that are executed unconditionally in an SDFG
    """
    states = set()
    dummy_sink = cfg.add_state("dummy_state")
    for sink_node in cfg.sink_nodes():
        if sink_node is not dummy_sink:
            cfg.add_edge(sink_node, dummy_sink, dace.sdfg.InterstateEdge())
    # get all the nodes that are executed unconditionally in the state-machine a.k.a nodes
    # that dominate the sink states
    dominators = cfg_analysis.all_dominators(cfg)
    for dom in dominators[dummy_sink]:
        if isinstance(dom, SDFGState):
            states.add(dom)
        elif isinstance(dom, ControlFlowRegion):
            states.update(_find_unconditionally_executed_states(dom))
    # remove dummy state
    cfg.remove_node(dummy_sink)
    return states
...
class UnderapproximateWrites(ppl.Pass):
    ...
    # Mapping of states to the iteration variables surrounding them, including the ones from surrounding SDFGs

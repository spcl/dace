...
def replace_dict(subgraph: 'StateSubgraphView',
    ...
    if isinstance(subgraph, dace.SDFGState):
        ...
        for node in subgraph.nodes():
            ...
            if isinstance(node, nodes.AccessNode) and state.in_degree(node) == 0 and isinstance(node.desc(sdfg), data.Scalar):
                    node_data_symbolic = dace.symbolic.pystr_to_symbolic(node.data)
                    if node_data_symbolic in symrepl:
                        tasklet = state.add_tasklet(name="constant", inputs={}, outputs={f'{node.data}_cp_val'}, code=f'{node.data}_cp_val = {symrepl[node_data_symbolic]}')
                        sdfg.add_array(f'{node.data}_cp', [1], type(sympyexpr_to_python(symrepl[node_data_symbolic])), transient=True)
                        ...
                        for edge in state.out_edges(node):
                            for producer_tree in state.memlet_tree(edge).traverse_children(
                                include_self=True
                            ):

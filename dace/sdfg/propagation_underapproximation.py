        ...
        for i, smpattern in enumerate(self.patterns_per_dim):
            ...
            for expr in expressions:
                if isinstance(expr[i], symbolic.SymExpr):
                    ...
                    dexprs.append((
                        expr[i][0].expr if isinstance(expr[i][0], symbolic.SymExpr) else expr[i][0],
...
def _find_live_iteration_variables(nsdfg: nodes.NestedSDFG,
                                   sdfg: SDFG,
                                   state: SDFGState):
    """
    ...
    def _underapproximate_writes_state(self, sdfg: SDFG, state: SDFGState):
        ...
        for node in state.nodes():
            if isinstance(node, nodes.NestedSDFG):
                ...
                _find_live_iteration_variables(node, sdfg, state)

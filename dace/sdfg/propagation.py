...
def propagate_symbol(dfg_state,
                     memlet: Memlet,
                     sym_name: str,
                     sym_range: subsets.Range,
                     arr=None) -> Memlet:
    """ Substitutes a symbol in a Memlet with its range.
        :param dfg_state: An SDFGState object representing the graph.
        :param memlet: The memlet to be propagated through the symbol range.
        :param sym_name: The symbol name.
        :param sym_range: The symbo range.
    """
    if memlet.is_empty():
        return Memlet()
    ...
    defined_vars = []
    variable_context = [defined_vars, [symbolic.pystr_to_symbolic(sym_name)]]
    ...
    for md in aggdata:
        ...
        for pclass in MemletPattern.extensions():
            ...
            warnings.warn('Cannot find appropriate memlet pattern to '
                            'propagate %s through %s' %
                            (str(md.subset), str(sym_range)))
            tmp_subset = copy.deepcopy(md.subset)
    ...
    # Some unions failed
    if new_subset is None:
        new_subset = copy.deepcopy(md.subset)

...
class UsedNames:
    """Membership over the names an SDFG has taken, answered without collecting them.

    :func:`dace.utils.find_new_name` only ever asks ``name in existing_names``, so the names never
    have to be gathered -- and gathering them is what made minting expensive. Building
    ``arrays.keys() | constants.keys() | symbols.keys()`` costs a set of every name in the SDFG per
    minted name, which is quadratic over a parse that mints thousands. Three dict lookups answer
    the same question in constant time.
    ...
    That answer cannot be had cheaply: it is a walk over every state and every node, so it is opt
    in, and :meth:`SDFG.find_new_name_avoiding_connectors` is the caller-visible name for the cost.
    The walk is memoised per instance, so a name needing several attempts (``tmp``, ``tmp_0``,
    ``tmp_1``, ...) still walks once. ``states()`` stays inside this SDFG's own namespace: it
    descends into control flow regions but not into NestedSDFG nodes, which have namespaces of
    their own.
    """
    ...
    def __contains__(self, name: str) -> bool:
        sdfg = self.sdfg
        if name in sdfg._arrays or name in sdfg.constants_prop or name in sdfg.symbols:
            return True

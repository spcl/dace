...
class ReverseReduce(BackwardImplementation):
    ...
    def backward_can_be_applied(node: Node, state: SDFGState, sdfg: SDFG) -> bool:
        ...
        if reduction_type not in (dtypes.ReductionType.Sum, dtypes.ReductionType.Max):
    ...
    def backward(forward_node: Node, context: BackwardContext, given_gradients: typing.List[typing.Optional[str]],
        ...
        if reduction_type is dtypes.ReductionType.Sum:
            ...
            reduction_memlet = Memlet.simple(rev_input_conn_name, "0" if reduce_all_axes else ",".join("i" + str(i)
                                                                                            for i in non_reduce_axes))
            reverse_reduction_memelt = Memlet.simple(
                        rev_output_conn_name, ",".join("i" + str(i) for i in all_axes), wcr_str="lambda x, y: x + y")
            
            max_val_memlet = Memlet.simple(max_conn_name, "0" if reduce_all_axes else ",".join("i" + str(i)
                                                                                            for i in non_reduce_axes))
            ...
            _, _, exit_map = state.add_mapped_tasklet(
                ...
                tasklet_code, {
                    "__out": reverse_reduction_memelt
                    
                },
                external_edges=True)

            # Add the nested SDFG to the backward state
            nsdfg = context.backward_state.add_nested_sdfg(sdfg, None, {rev_input_conn_name, max_conn_name, max_idx_conn_name},
            ...
            if isinstance(context.forward_sdfg.arrays[fwd_in_node.data], (dace.data.View, dace.data.ArrayView)):
                ...
                if isinstance(in_node, dace.nodes.AccessNode):
                    ...
                    assert not isinstance(context.forward_sdfg.arrays[in_node.data], (dace.data.View, dace.data.ArrayView))
                    # Add the read node
            ...
            if isinstance(context.forward_sdfg.arrays[fwd_out_node.data], (dace.data.View, dace.data.ArrayView)):
                ...
                if isinstance(out_node, dace.nodes.AccessNode):
                    # Make sure this is not a view iteself
                    assert not isinstance(context.forward_sdfg.arrays[out_node.data], (dace.data.View, dace.data.ArrayView))
                    # Add the read node

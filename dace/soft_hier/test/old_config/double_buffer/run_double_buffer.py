...
def _my_gen_matmul_bd_sdfg_outerK(hardware_matmul_mnk: typing.Tuple, global_storage: dace.dtypes.StorageType,
                                  local_storage: dace.dtypes.StorageType, device_schedule: dace.dtypes.ScheduleType,
                                  thread_group_schedule: dace.dtypes.ScheduleType, thread_group_dims: typing.Tuple,
                                  input_float, output_float, coarsening_factor, mmad_tasklet_str: str):
    sdfg = dace.SDFG("GEMM")
    ...
    for name in ["A", "B", "C"]:
        if name == "A" or name == "B":
            ...
            state.add_edge(thread_group_map_entry, f"OUT_{name}", block_tiled_map_entry, f"IN_{name}",
                           dace.memlet.Memlet(f"{name}[{access_str}]"))
    ...
    for name, shape in [("A", (tM, tK // coarsening_factor)), ("B", (tK // coarsening_factor, tN))]:
        ...
        arrn, arr = sdfg.add_array(name=f"local_coarsened_{name}",
                                   shape=shape,
                                   dtype=input_float,
                                   storage=local_storage,
                                   transient=True)
    ...
    for name in ["A", "B", "C"]:
        if name == "A" or name == "B":
            if name == "A":
                access_str_1 = ", ".join([
                    f"i + gi * {tM}:i + gi * {tM} + {tM}",
                    f"bK*{tK//coarsening_factor}:bK*{tK//coarsening_factor}+{tK//coarsening_factor}"
                ])
                access_str_2 = ", ".join([f"0:{tM}", f"0:{tK//coarsening_factor}"])
            elif name == "B":
                access_str_1 = ", ".join([
                    f"bK*{tK//coarsening_factor}:bK*{tK//coarsening_factor}+{tK//coarsening_factor}",
                    f"j + gj * {tN}:j + gj * {tN} + {tN}"
                ])
            ...
            state.add_edge(block_tiled_map_entry, f"OUT_{name}",
                           local_coarsened_access_nodes[f"local_coarsened_{name}"], None,
                           dace.memlet.Memlet(f"{name}[{access_str_1}]"))
            block_tiled_map_entry.add_out_connector(f"OUT_{name}")
            state.add_edge(local_coarsened_access_nodes[f"local_coarsened_{name}"], None, thread_coarsened_map_entry,
                           f"IN_local_coarsened_{name}", dace.memlet.Memlet(f"local_coarsened_{name}[{access_str_2}]"))
            thread_coarsened_map_entry.add_in_connector(f"IN_local_coarsened_{name}")

    access_str = ", ".join(
        [f"0:{coarsening_factor}*{coarsening_factor}", f"0:{tM//coarsening_factor}", f"0:{tN//coarsening_factor}"])
    state.add_edge(block_tiled_map_entry, f"OUT_accumulator", thread_coarsened_map_entry, f"IN_accumulator",
                   dace.memlet.Memlet(f"accumulator[{access_str}]"))
    ...
    state.add_edge(thread_coarsened_map_entry, f"OUT_local_coarsened_A", matmul_tasklet, "_in_local_a",
                   dace.memlet.Memlet(f"local_coarsened_A[0:{tK//coarsening_factor}, 0:{tN//coarsening_factor}]"))
    state.add_edge(thread_coarsened_map_entry, f"OUT_local_coarsened_B", matmul_tasklet, "_in_local_b",
                   dace.memlet.Memlet(f"local_coarsened_B[0:{tK//coarsening_factor}, 0:{tN//coarsening_factor}]"))
    access_str = ", ".join([f"ci*{coarsening_factor} + cj", f"0:{tM//coarsening_factor}", f"0:{tN//coarsening_factor}"])
    state.add_edge(thread_coarsened_map_entry, f"OUT_accumulator", matmul_tasklet, "_in_accumulator",
                   dace.memlet.Memlet(f"accumulator[{access_str}]"))
    ...
    state.add_edge(matmul_tasklet, "_out_accumulator", thread_coarsened_map_exit, f"IN_accumulator",
                   dace.memlet.Memlet(f"accumulator[{access_str}]"))
    thread_coarsened_map_exit.add_in_connector("IN_accumulator")

    access_str = ", ".join(
        [f"0:{coarsening_factor}*{coarsening_factor}", f"0:{tM//coarsening_factor}", f"0:{tN//coarsening_factor}"])
    state.add_edge(thread_coarsened_map_exit, f"OUT_accumulator", block_tiled_map_exit, f"IN_accumulator",
                   dace.memlet.Memlet(f"accumulator[{access_str}]"))
    ...
    DoubleBuffering.apply_to(sdfg,
                             map_entry=block_tiled_map_entry,
                             transient=local_coarsened_access_nodes["local_coarsened_A"])
    return sdfg


def _my_gen_matmul_bd_sdfg_innerK(hardware_matmul_mnk: typing.Tuple, global_storage: dace.dtypes.StorageType,
                                  local_storage: dace.dtypes.StorageType, device_schedule: dace.dtypes.ScheduleType,
                                  thread_group_schedule: dace.dtypes.ScheduleType, thread_group_dims: typing.Tuple,
                                  input_float, output_float, coarsening_factor, mmad_tasklet_str: str):
    sdfg = dace.SDFG("GEMM")
...
if __name__ == "__main__":
    ...
    sdfg = _my_gen_matmul_bd_sdfg_innerK(

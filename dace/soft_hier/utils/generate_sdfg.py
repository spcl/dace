...
def _my_gen_split_K_BSP_matmul_sdfg(hardware_matmul_mnk: typing.Tuple,
    ...
    for arr_name, shape, ftype in [("A", (M, K), input_float), ("B", (K, N), input_float), ("C", (M, N), output_float)]:
        ...
        if arr_name == "C":
            arrn, arr = sdfg.add_array(name=arr_name, shape=shape, dtype=ftype, storage=global_storage, transient=False, is_hbm_interleaved=is_hbm_interleaved, hbm_split_scheme=hbm_split_scheme[2], hbm_placement_scheme=hbm_placement_scheme[2])
            arrs[arrn] = arr
    arrn, arr = sdfg.add_array(name="accumulator", shape=(coarsening_factor*coarsening_factor, tM//coarsening_factor, tN//coarsening_factor), dtype=output_float, storage=local_storage, transient=True)
    arrs[arrn] = arr
    ...
    if summa_range is not None:
        ...
        BSP_communication_code_block, 
        BSP_sync, 
        post_shift_code_block) = BSP_generator_func(i, j, gi, gj, gM, gN, tM, tN, tK, M, N, K, k_group_dims=k_group_dims, summa_range=summa_range)
    else:
        (pre_shift_code_block, 
        BSP_stride,
        BSP_init_code_block, 

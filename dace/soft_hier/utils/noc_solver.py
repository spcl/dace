...
def solve_noc_lp(p=4):
    ...
    x = pulp.LpVariable.dicts("x", [(i, j) for i in range(p) for j in range(p)], cat="Binary")
...
def get_neighbor(assignment, p):
    ...
    for _ in range(100):  # Try up to 100 times to find a valid move.
        ...
        if (new_assignment[cell_a] == new_assignment[cell_d] and new_assignment[cell_b] == new_assignment[cell_c]
                and new_assignment[cell_a] != new_assignment[cell_b]):

...
class SplitHBMLoad(transformation.SingleStateTransformation):
    ...
    def apply(self, graph: sd.SDFGState, sdfg: sd.SDFG):
        ...
        for edge, parent in schedule_nsdfg.all_edges_recursive():
            if isinstance(edge, GF.MultiConnectorEdge):
                if edge.data.data in transients_to_modify.values():
                    edge.data.subset = self._modify_memlet(schedule_nsdfg,
                                                           edge.data.subset,
                                                           edge.data.data,
                                                           param_name='__db_local_array_param')
                    ...
                    if dataname is not None:
                        ...
                        edge.data.other_subset = self._modify_memlet(schedule_nsdfg,
                                                                     subset,
                                                                     dataname,
                                                                     param_name='__db_local_array_param')
                ...
                if edge.data.data in local_streams:
                    edge.data.subset = self._modify_memlet(schedule_nsdfg,
                                                           edge.data.subset,
                                                           edge.data.data,
                                                           param_name='__db_local_stream_param')
                    modified_subsets.append(edge.data.subset)
                else:  # Could be other_subset
                    path = parent.memlet_path(edge)
                    src_node = path[0].src
        ...
        for stream in local_streams:
            ...
            nsdfg.add_stream(name=stream,
                             dtype=st_desc.dtype,
                             shape=st_desc.shape,
                             buffer_size=st_desc.buffer_size,
                             transient=st_desc.transient,
                             storage=st_desc.storage)
            ...
            nsdfg.add_array(name=transient_array,
                            shape=arr_desc.shape,
                            dtype=arr_desc.dtype,
                            storage=arr_desc.storage,
                            transient=arr_desc.transient)

            acc_trans_in = nstate.add_access(transient_array)
            schedule_nsdfg_node.add_in_connector(transient_array, force=True)
            input_edge = nstate.add_edge(acc_trans_in, None, schedule_nsdfg_node, transient_array,
                                         Memlet(f"{transient_array}"))
        ...
        new_start_state = schedule_nsdfg.add_state_after(schedule_nsdfg.start_state,
                                                         label=old_start_state.label,
                                                         is_start_block=True)
        ...
        cb_hbm_split_load_cfg1 = ControlFlowRegion(label="hbm_split_load_s1")

        # condition when to communicate
        comm_hbm_split_load_cond = CodeBlock(code=f"({map_param} < {map_rend} - {map_rstride} )",
                                             language=dtypes.Language.Python)

        cb_hbm_split_load.add_branch(condition=comm_hbm_split_load_cond, branch=cb_hbm_split_load_cfg1)
        ...
        for transient in transients_to_modify.values():
            ...
            for edge in initial_state.edges():
                if isinstance(edge.dst, nodes.AccessNode) and edge.dst.data == transient:
                    ...
                    cb_communication_local = ConditionalBlock(label=f"hbm_load_{transient}",
                                                              sdfg=schedule_nsdfg,
                    ...
                    if transient == "local_A":
                        # Load from HBM
                        comm_cfg1_local_cond_hbm = CodeBlock(code=f"({gj} == {NPE} - {lr_param} - 1)",
                                                             language=dtypes.Language.Python)

                    if transient == "local_B":
                        # Load from HBM
                        comm_cfg1_local_cond_hbm = CodeBlock(code=f"({gi} == {NPE} - {lr_param} - 1)",
                                                             language=dtypes.Language.Python)

                    # Load from HBM
                    cb_comm_local_hbm_cfg = ControlFlowRegion(label=f"cb_{transient}_hbm")
            ...
            if state.name == "compute":
                ...
                for edge in state.edges():
                    if isinstance(edge.dst, nodes.AccessNode) and edge.dst.data in transients_to_modify.values():
                        edge.data.subset = self._replace_in_subset(edge.data.subset, "__db_local_stream_param",
                                                                   current_tcdm_expr)
                        edge.data.other_subset = self._replace_in_subset(edge.data.other_subset,
                                                                         "__db_local_array_param", next_tcdm_expr)
                    elif isinstance(edge.src, nodes.AccessNode) and edge.src.data in transients_to_modify.values():
                        edge.data.subset = self._replace_in_subset(edge.data.subset, "__db_local_array_param",
                                                                   current_tcdm_expr)
                        edge.data.other_subset = self._replace_in_subset(edge.data.other_subset,
                                                                         "__db_local_stream_param", current_tcdm_expr)

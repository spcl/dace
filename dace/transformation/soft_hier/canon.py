...
class CanonTransformer(transformation.SingleStateTransformation):
    ...
    def apply(self, graph: sd.SDFGState, sdfg: sd.SDFG):
            ...
            if isinstance(src_node, nodes.AccessNode) and src_node.data in global_arrays:
                ...
                for idx, range in enumerate(ranges):
                    ...
                    if f"{r_start}" == f"{map_param}":
                        if idx == 0:
                            memlet.subset = self._replace_in_subset(memlet.subset, map_param, f"{map_param} + {gi}*{map_rstride}")
                            memlet.other_subset = None
                        if idx == 1:
                            memlet.subset = self._replace_in_subset(memlet.subset, map_param, f"{map_param} + {gj}*{map_rstride}")
                            memlet.other_subset = None
        ...
        for transient in transients_to_modify:
            ...
            lr_s1.add_edge(local_an, None, s_an, None, 
                            memlet=Memlet(
                            data=transient,
                            subset=local_subset,
        ...
        lr_s2.add_tasklet(name="SoftHier_sync", inputs=None, outputs=None, code="flex_global_barrier_xy();", language=dtypes.Language.CPP)

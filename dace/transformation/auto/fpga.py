...
def fpga_global_to_local(sdfg: SDFG, max_size: int = 1048576) -> None:
    ...
    for name, desc in sdfg.arrays.items():
        if desc.transient and name not in sdfg.shared_transients() and desc.storage == dtypes.StorageType.FPGA_Global:
            ...
            if total_size is not None and total_size * desc.dtype.bytes <= max_size:
                ...
                for node, graph in sdfg.all_nodes_recursive():
                    if isinstance(node, nodes.AccessNode):
                        ...
                        for (_, candidate), memlet_trace, state_trace, sdfg_trace in trace:

...
def validate_and_pad_params_to_three(params):
    ...
    for param in params:
        if len(param) < 3:
            ...
            raise ValueError(f"Tuple {param} has length greater than 3, which is not allowed.")
...
def _tile(
    ...
    for i, current_config in enumerate(combinations):
        ...
        if len(work_maps) > 1:
            raise NotImplementedError("Memory tiling (Tiling of Work Maps) more than once is currently not implemented")
        ...
        if not is_assign_kernel:
            for i in range(len(work_maps)):
                ...
                work_map_tile = work_map_tile[:tuple_size_needed] + (1,) * (tuple_size_needed - len(work_map_tile))
        ...
        if work_on_copy and verify:
            ...
            if shr_mem_needed >= 48 * 1024:
                print(f"Kernel uses too much shared memory for config {current_config}, skipping.")
            else:

...
class BlockTiling(transformation.SingleStateTransformation):
    ...
    def apply(self, graph: SDFGState, sdfg: SDFG):
        ...
        for i in range(min(3, len(thread_block_entry.map.params)), 0, -1):
            block_iterations.append(possible_block_iters[-i])
        ...
        for i in range(min(3, len(thread_block_entry.map.range)), 0, -1):
            ...
            inner_tile_sizes.append(step)
        inner_tile_sizes += [1] * (3 - len(inner_tile_sizes))
        ...
        for i in range(min(3, len(thread_block_entry.map.range)), 0, -1):
            ...
            block_strides.append(dev_step)
        block_strides += [1] * (3 - len(block_strides))

        MapTiling.apply_to(sdfg=sdfg, options=dict(prefix="s_", tile_sizes=block_iterations, strides=block_strides,  tile_trivial=True),  map_entry=thread_block_entry)

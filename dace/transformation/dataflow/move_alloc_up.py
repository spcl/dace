...
def offset_memlets(state: dace.SDFGState, offsets: List[Any],
                   entry_node: dace.nodes.MapEntry,
                   name: str):
...
def move_access_node_up(state: dace.SDFGState, access_node: dace.nodes.AccessNode, do_offset_memlets:bool):
    ...
    state.add_edge(
        lvl2_ie.src, lvl2_ie.src_conn,
        lvl2_oe.dst, lvl2_oe.dst_conn,
        lvl2_memlet
    )
    ...
    state.add_edge(
        new_access, None,
        lvl1_ie.dst, lvl1_ie.dst_conn,
        lvl1_ie_memlet
    )
    state.remove_edge(lvl1_ie)
    state.add_edge(
        lvl1_ie.src, lvl1_ie.src_conn,
        new_access, None,
        copy.deepcopy(lvl1_ie.data)
    )
...
def move_exit_access_node_down(state: dace.SDFGState, access_node: dace.nodes.AccessNode, do_offset_memlets:bool):
    ...
    lvl2_map_entry = {n for n in state.nodes() if isinstance(n, dace.nodes.MapEntry) and state.exit_node(n) == lvl2_map_exit}.pop()
    ...
    state.add_edge(
        new_access, None,
        lvl1_oe.dst, lvl1_oe.dst_conn,
        copy.deepcopy(lvl1_oe.data)
    )

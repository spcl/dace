function [ rhoOpts, varsOpt, Xopts ] = MaxRhoFromFile( inputFile )
    if nargin == 0
        inputFile = 'inp_sys.txt';
    end
    input = evalin(symengine, fileread(inputFile));
    
    [rhoOpts, varsOpt, Xopts, vars ] = MaxRho( input );    
    ...
    fid = fopen('output_sys.txt', 'w');
    fprintf(fid,"%s", outputJson);
    fclose(fid);
end


function varsOpt = GuessSolution(input, whichOnes, vars, grad)

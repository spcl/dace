function [curRhoOpts, guess, Xopt] = GuessSolution(dom, output, guess)
    if nargin < 3
        if nargin < 2
        ...
        syms Lambda X M S;
        assume(X > 0.1);
        [L, sys] = FormKKTSystem(dom, output);
        ...
        nOrig = n;
        varsOrig = vars;
        varsFlattened = vars(counter > minOccur);
        dom = subs(dom, varsFlattened, ones([1, length(varsFlattened)]));               
        vars = symvar(dom); 
        n = length(vars);
        ...
        domOrig = dom;
        dom = dom - a0;   
        [L, sys] = FormKKTSystem(dom, prod(symvar(dom)));
        ...
        %choicesA = choicesA(:,2:end);
        %choicesB = [choicesB(:,1), choicesB(:,3:end)];
...
%                 a = 1;
%             end
            %sysGuess = simplify(subs(sys, vars, guess));
            ...
            if ~ismember(symvar(X), symvar(grad))
                if grad < 0
...
%                 curChoiceB(3) = 3;
    ...
    guess = [ones(1, length(varsFlattened)) guess];
    vars = [varsFlattened, vars];
    rho = subs(output/ ( X - S), vars, guess);
    dRho = diff(rho, X);
    Xopt = solve(dRho == 0, X);
    curRhoOpts = simplify(subs(rho, X, Xopt));
    
    % here we are only to check one manual, user-provided guesss
    else 
        if nargin < 2
            output = prod(symvar(dom));
        else
...
function [L, sys] = FormKKTSystem(dom, output)
    syms Lambda X M S;
    vars = union(symvar(dom),symvar(output));   
    n = length(vars);
    c = children(dom);
    ...
    % KKT conditions system for locally optimal constrainted optimization problem
    uu = sym('uu', size(vars));
    assume(uu <= 0);    
    assume(S >= 1000);
    assume(X >= 1001);
    assume(Lambda >= 0);
    assume(vars >= 1);
    ineqconstr = uu.*(vars - 1);
    %form Lagrangian from the KKT multipliers
    L = output - Lambda*(dom - X) - sum(ineqconstr);
    try
        grad = gradient(L, [vars Lambda]);
    catch
        'Malformed input'
        input
        exit()

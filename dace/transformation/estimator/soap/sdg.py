...
from dace.transformation.estimator.soap.soap import AccessParams, SoapStatement, OutputArrayParams
...
class SDG:
    ...
    def arraystr2access(self, array_str : str) -> Dict[str, Tuple[str, Tuple[int]]]:
        """
        Converts the array string to the access function dictionary.
        Example input:
        array_str = A[i0,i1]
        Example output:
        {'A': ('i0*i1', (0,0))}
        the first element in the tuple is the product of iteration variables.
        the second is the tuple of zeros of lenght equal to the number of iteration variables.
        """
        array_name, array_access = array_str.split('[')
        array_access = array_access[:-1].split(',')
        access_function = '*'.join(array_access)
        access_offsets = (0,) * len(array_access)
        return {array_name: (access_function, access_offsets)}


    def from_ir(self, ir : str) -> None:
        """
        Constructs the SDG from the input IR string.
        """
        ...
        """
        We are interested only in the `Program` part of the IR.
        Each line corresponds to one SOAP statement.
        We parse the line, and create a new SOAP statement, similar to the _from_SDFG_node method.
        """

        # Iterate over the lines of the IR, only the `Program` part
        section_names = ["In:", "Out:", "Program:"]

        sections = {}
        # Split the IR into sections
        for sec_name in reversed(section_names):
            if sec_name not in ir:
                warn(f"Section {sec_name} not found in the IR string.")
                continue
            sections[sec_name] = ir.split(sec_name)[-1].split("\n")
            ir = ir.split(sec_name)[0]
        
        for st_number, statement in enumerate(sections["Program:"]):
            if len(statement) == 0:
                continue
            # Parse the line
            ranges, statement = statement.split(":")
            ranges = ranges.split()

            # create iteration variables i0, i1, i2... corresponding to the number of ranges M, N, K, 
            # substitute the iteration variables with the wildcard placeholders {0}, {1}, {2},
            # and create a dictionary daceRanges with the ranges of each iteration variable
            # e.g., dace_ranges = {'i0': (0, M-1, 1), 'i1': (0, N-1, 1), 'i2': (0, K-1, 1)}
            dace_ranges = {}
            for i, var in enumerate(ranges):
                iter_var = f"i{i}"
                statement = statement.replace("{" + str(i) + "}", iter_var)
                dace_ranges[iter_var] = (sp.sympify(0), dace.symbol(var) - 1, 1)


            # if the reduction is present ( += or -=), add the left hand side (output)
            # to the right hand side of the equation
            if "+=" in statement:
                out_access = statement.split("+=")[0].strip()
                in_accesses = statement.split("+=")[1].split() + [out_access]
                is_wcr = True
                # find the SSA dimension. This is the iteration variable that is 
                # not present in the out_accesses
                ssa_dim = dace.symbol([x for x in dace_ranges.keys() if x not in out_access][0])
                # ssa dim is of the form: [iter_var, range, step]. When the range is not known,
                # the default form is [iter_var, iter_var, 1]
                ssa_dim = [ssa_dim, ssa_dim, 1]
            else:
                out_access = statement.split('=')[0].strip()
                in_accesses = statement.split("=")[1].split()
                is_wcr = False

            # replace all operands (*, /, + , -) with a space. We don't care about arithmetic, just the access functions
            # in_accesses = in_accesses.replace("*", " ").replace("/", " ").replace("+", " ").replace("-", " ")

            # remove scalar symbols (e.g., alpha, beta) from the access functions. We care only about arrays
            in_accesses = [x for x in in_accesses if '[' in x and ']' in x]

            # Create a new SOAP statement
            S = SoapStatement()
            S.name = f"ST_{st_number}"
            S.tasklet = [statement]
            S.daceRanges[S.name] = dace_ranges 
            S.output_arrays = self.arraystr2access(out_access)


            # add input phis
            for in_access in in_accesses:
                arr_name, (base_access, offset) = list(self.arraystr2access(in_access).items())[0]
                if arr_name not in S.phis.keys():
                    S.phis[arr_name] = defaultdict(set)
                if in_access == out_access and is_wcr:
                    # add SSA dim
                    S.phis[arr_name][base_access] = AccessParams(offsets={offset}, ssa_dim=[ssa_dim])
                else:
                    S.phis[arr_name][base_access] = AccessParams(offsets={offset})


            # add output access
            for arr_name, (base_access, offset) in S.output_arrays.items():
                S.output_accesses[arr_name] = OutputArrayParams(baseAccess=base_access,
                                                                  offsets={offset}, wcr=is_wcr,
                                                                  ssa_dim=[ssa_dim] if is_wcr else None)
            # S.output_access = S.output_arrays
            # S.output_accesses[out_access] = OutputArrayParams(baseAccess= offsets=set(offset), wcr=is_wcr)
    ...
    def _add_statement_to_SDG(self, S : SoapStatement, sdg_scope : SdgScope = None) -> None:
        output_arrays_vers = []
        ...
        for output_array in S.output_accesses.keys():
            ...
            if sdg_scope and output_array in sdg_scope.SDFG_arrays.keys() and  \
        ...
        for array_name, array_accesses in S.phis.items():
            ...
            # check if it's transient
            if sdg_scope and array_name in sdg_scope.SDFG_arrays.keys() and  \
    ...
    def plot_SDG(self, filename : str = 'SDG.dot') -> None:
        # prepare the MultiDigraph self.graph for drawing
        g = copy.deepcopy(self.graph)
        # remove nodedata from the nodes
        for node in g.nodes:
            g.nodes[node].pop('st', None)
    ...
    def recursive_SDG_subgraphing(self, node : str, nodes_so_far : Set[str], checked_subgraphs : set) -> List[SoapStatement]:
        ...
        base_st = self.graph.nodes[node]['st']
        S: SoapStatement = copy.deepcopy(base_st)
        S.name = node
        S.subgraph = set([node])
        S.subgraph_inputs = set(self.graph.predecessors(node))
        sdg_statements = []
    ...
    def calculate_IO_of_SDG(self) -> Tuple[sp.core.Expr, List[SoapStatement]]:
        ...
        # Form a networkx directed graph from the list of 
        # subgraphs ret_subgraphs.
        # create direct edges between the subgraphs based on the
        # S.subgraph_inputs (destinations) and S.subgraph (sources).
        #  If there is an intersection
        # between S1.subgraph and S2.subgraph_inputs , then we add an edge
        #  (S1, S2) in the graph.
        subgraphs_graph = nx.DiGraph()
        for S in ret_subgraphs:
            subgraphs_graph.add_node(S.name, st = S)
        for S1 in ret_subgraphs:
            for S2 in ret_subgraphs:
                if S1 != S2 and len(S1.subgraph.intersection(S2.subgraph_inputs)) > 0:
                    subgraphs_graph.add_edge(S1.name, S2.name)


        return Q_total, subgraphs_graph

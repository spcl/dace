...
def get_default_gateway_ip() -> Optional[str]:
    """
    Get the IP address of the default gateway using the `ip route` command.
    It is used to communicate with the remote MATLAB server that runs on the Windows host machine
    from the Linux WSL machine.
    """
    try:
        ...
        # Regular expression to find the default gateway IP
        match = re.search(r'default via (\d+\.\d+\.\d+\.\d+)', result.stdout)
        
        if match:
            return match.group(1)
        else:
            print("No default gateway found in the output.")
            return None

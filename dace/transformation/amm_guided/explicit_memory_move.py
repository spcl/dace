...
class MemoryMovementNode(CodeLibraryNode):
    ...
    def write_inner_loop(self, num_threads, line_len, dim_check, dynamic_check=False, tiles_evenly=False):
        ...
        if num_threads > line_len:
            ...
            real_lines_at_a_time = min(
                int(lines_at_a_time), int(self._dst_arr.shape[-2]))
            ...
            if len(self._src_arr.shape) == 1:
                ...
                offset_expression_1d = "+".join([f"({stride}*({offset}))" for (
                    offset, _, _), stride in zip(self._src_subset, strides)])
            ...
            if len(self._src_arr.shape) == 1:
                ...
                further_access_str_src += " + " + \
                    f"((i{var_id}) * {self._src_arr.strides[-2]}) + i{var_id+1}"
    ...
    def generate_code(self, inputs, outputs):
        ...
        outer_dim_checks = [f"Min({lim - beg}, {dst_lim})" for (beg, end, step), lim,
                            dst_lim in zip(self._src_subset, self._src_arr.shape, self._dst_arr.shape)]
...
class ExplicitMemoryMove(transformation.SingleStateTransformation):
    ...
    memory_location = Property(
        dtype=dtypes.StorageType, default=dtypes.StorageType.GPU_Shared, desc="Destination memory location")
    ...
    def apply(self, state: SDFGState, sdfg: SDFG):
        ...
        for out_edge in state.out_edges(self.map_entry):
            ...
            loc1_to_loc2_map[src_arr_name] = (
                dst_arr_name, [beg for (beg, end, step) in memlet.subset])
            ...
            for beg, end, step in old_subset:
                ...
                if isinstance(_beg, SymExpr) or isinstance(_beg, symbol):
                    thread_block_offset.append(
                        any([_beg.has(symbol(v)) for v in self.thread_block_map_entry.map.params]))
                else:
                    thread_block_offset.append(False)
                new_subset.append((_beg, _end, _step))

            new_range_list = new_subset
            to_dst_memlet = Memlet(subset=subsets.Range(
                new_range_list), data=dst_arr_name)
            to_map_memlet = Memlet(subset=subsets.Range(
                new_range_list), data=dst_arr_name)
            ...
            while nodeset:
                ...
                if not isinstance(n, nodes.MapExit):
                    edges_to_check = edges_to_check.union(
                        set(state.out_edges(n)))
            ...
            for edge in edges_to_check:
                ...
                if memlet.data in loc1_to_loc2_map.keys():
                    ...
                    new_subset_list = [(beg-offset, end-offset, step)
                    ...
                    for i, ((beg, end, step), apply_offset) in enumerate(zip(new_subset_list, thread_block_offset)):
                        if apply_offset:
                            ...
                            nb = (beg+symbol(params[i]),
                                  end+symbol(params[i]), step)
                            new_subset_list[i] = nb

                    new_memlet = Memlet(subset=subsets.Range(
                        new_subset_list), data=dst_name)

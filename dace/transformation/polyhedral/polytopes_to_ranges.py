...
import dace
from dace import dtypes, registry, subsets, symbolic, Memlet
...
def polytopes_to_ranges(sdfg):
    sdfg.apply_transformations_repeated(PolytopesToRanges, validate=True)
                ...
                if isinstance(node, nodes.NestedSDFG):
                    ...
                    for e in state.out_edges(node):
                        if isinstance(e.data.subset, Polytope):
                            ...
                            # bounding_ranges = get_overapprox_range_list_from_set(subset.isl_set)
...
class PolytopesToRanges(pm.Transformation):
    """
    ...
    @staticmethod
    def expressions():
        return [sdutil.node_path_graph(PolytopesToRanges.map_entry)]
    ...
    def can_be_applied(graph: dace.SDFGState,
        ...
        map_entry = graph.node(candidate[PolytopesToRanges.map_entry])
        return isinstance(map_entry.range, Polytope)

    def apply(self, sdfg: dace.SDFG):
        map_entry = self.map_entry(sdfg)
        if map_entry.map.get_param_num() == 1:
            new_range = map_entry.range.to_ranges()
            map_entry.range = new_range
            return [map_entry]
        else:
            ...
            new_maps = [
                ...
                for param, param_range in zip(current_map.params[1:],
                                              current_map.range[1:])
            ]

...
class GPUPersistentKernel(SubgraphTransformation):
    ...
    def match(sdfg: SDFG, subgraph):
        ...
        while len(front) > 0:
            current = front.pop(0)
            unseen = [suc for suc in subgraph.successors(current)
    ...
    def apply(self, sdfg: SDFG, subgraph):
        
        if not self.match(sdfg, subgraph):
            raise Exception('The given subgraph cannot be fused!')
        
        entry_states_in, entry_states_out = self.get_entry_states(sdfg,
                                                                  subgraph)
            ...
            if len(entry_edge.data.assignments) > 0:
                entry_guard_state = sdfg.add_state(
                    label='{}kernel_entry_guard'.format(
                        self.kernel_prefix + '_'
                        if self.kernel_prefix != '' else ''
                    )
                )
                sdfg.add_edge(
                    entry_state_out, entry_guard_state,
        ...
        kernel_sdfg = SDFG('{}kernel'.format(
            self.kernel_prefix + '_'
            if self.kernel_prefix != '' else ''
        ))
        ...
        for data in kernel_data:
            if (isinstance(sdfg.arrays[data], dace.data.Stream)
                or (isinstance(sdfg.arrays[data], dace.data.Array)
        ...
        for arg in kernel_args_read:
            ...
            launch_state.add_memlet_path(
                read_node, map_entry, nested_sdfg,
                dst_conn=arg,
        ...
        for arg in kernel_args_write:
            ...
            launch_state.add_memlet_path(
                nested_sdfg, map_exit, write_node,
                src_conn=arg,
                memlet=Memlet.from_array(arg, sdfg.arrays[arg])
            )
    ...
    def is_gpu_state(sdfg: SDFG, state: SDFGState) -> bool:
        ...
        for schedule in [n.map.schedule for n in state.nodes()

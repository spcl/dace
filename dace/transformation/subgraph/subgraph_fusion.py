""" This module contains classes that implement subgraph fusion (WIP)
"""
import dace
...
class SubgraphFusion(object):
    ...
    def match(sdfg, subgraph):
        ...
        map_entries = get_lowest_scope_maps(sdfg, graph, subgraph)
    ...
    def fuse(self, sdfg, graph, map_entries, **kwargs):
        ...
        for node in intermediate_nodes:
            if node.data in transients_adjusted:
                continue
            ...
            while True:
                try: # executed if there are multiple in_edges
                    ...
                    #print("DURING", in_edge.data.data, ":   ", target_subset, target_subset_curr)
                    target_subset = subsets.union(target_subset, target_subset_curr)
                except StopIteration:
                    break
            ######

            base_offset = target_subset.min_element()
            #print(in_edge.data.data, ":   ", target_subset, base_offset)

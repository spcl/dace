...
def repl_map_ranges(node_list: Iterable[dace.nodes.Node], repldict: Dict[str, str]) -> None:
    """Substitute ``repldict`` into the range of every map entry in ``node_list``.

    A nested map's bound can read an enclosing scope's iterator, so rebasing that iterator
    has to reach the bound as well. A map that redefines the name as one of its own
    parameters shadows the outer one and is left alone.
    """
    sym_repldict = {dace.symbolic.pystr_to_symbolic(k): dace.symbolic.pystr_to_symbolic(v) for k, v in repldict.items()}
    for node in node_list:
        if not isinstance(node, dace.nodes.MapEntry):
            continue
        local = {k: v for k, v in sym_repldict.items() if str(k) not in node.map.params}
        if local:
            node.map.range.replace(local)


def repl_map_ranges_recursive(cfg: ControlFlowRegion, repldict: Dict[str, str]) -> None:
    """Substitute ``repldict`` into every map range of ``cfg``, descending into nested SDFGs."""
    for state in [cfg] if isinstance(cfg, dace.SDFGState) else cfg.all_states():
        repl_map_ranges(state.nodes(), repldict)
        for node in state.nodes():
            if isinstance(node, dace.nodes.NestedSDFG):
                repl_map_ranges_recursive(node.sdfg, repldict)
...
def repl_recursive(cfg: ControlFlowRegion, repldict: Dict[str, str]) -> None:
    ...
    map ranges, nested loop headers and branch conditions. ``cfg``'s OWN loop header is left alone."""
    repl_interstate_edges_recursive(cfg, repldict)
    repl_memlets_recursive(cfg, repldict)
    repl_tasklets_recursive(cfg, repldict)
    repl_map_ranges_recursive(cfg, repldict)
                ...
                for state_node in state.nodes():
                    if isinstance(state_node, dace.nodes.MapEntry):
                        ...
                        if has_matches:
                            ...
                            repl_map_ranges(nodes_between, repldict)

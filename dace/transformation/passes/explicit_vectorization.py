...
class ExplicitVectorization(ppl.Pass):
    ...
    def _vectorize_map(self, state: SDFGState, first_map_entry: dace.nodes.MapEntry):
        ...
        parents = dict()
        for node in state.all_nodes_between(first_map_entry, state.exit_node(first_map_entry)):
            parent = state.scope_dict()[node]
            parents[node] = parent

        maps_that_are_not_parents = set()
        for k in parents:
            if k not in parents.values():
    ...
    def _extend_temporary_scalaers(self, state: SDFGState, map_entry: dace.nodes.MapEntry):
            ...
            if isinstance(node, dace.nodes.AccessNode):
                ...
                if isinstance(desc, dace.data.Scalar):
                    ...
                    for edge in state.all_edges(*nodes):
                        if edge.data is not None and edge.data.data == node.data:
                            assert edge.data.subset == dace.subsets.Range([(0,0,1)])
                            edge.data.memlet = dace.memlet.Memlet(
                                data=f"{node.data}_vec",
                                subset=dace.subsets.Range([(0, self.vector_width - 1, 1)]),
                            )

    def _get_vector_templates(self, rhs1: str, rhs2: str, lhs: str, op: str):
    ...
    def _extend_memlets(self, state: SDFGState, map_entry: dace.nodes.MapEntry):
        ...
        for edge in edges:
            ...
            stride_offset = 0 if self.stride_type == "F" else -1 # lest-contig is Fortran and right-contig is C
            ...
            new_range_list[stride_offset] = (lb, le.subs(used_param, dace.symbolic.SymExpr(f"4 * {used_param}")) - 1, ls)
            ...
            edge.memlet = dace.memlet.Memlet(
                data=memlet.data,
    ...
    def _replace_tasklets(self, state: SDFGState, map_entry: dace.nodes.MapEntry):
        ...
        for node in nodes:
            if isinstance(node, dace.nodes.Tasklet):
                ...
                node.code = self._get_vector_templates(rhs1=rhs1, rhs2=rhs2, lhs=lhs, op=op)
    ...
    def _offset_memlets(self, state: SDFGState, map_entry: dace.nodes.MapEntry, offsets: List[dace.symbolic.SymExpr], dataname: str):
        ...
        for edge in edges:
            if edge.data is None or edge.data.data != dataname:
                continue
            memlet : dace.memlet.Memlet = edge.data
            new_range_list = [(b - offset_i, e - offset_i, s) for (b, e, s), offset_i in zip(memlet.subset, offsets)]
    ...
    def _copy_in_and_copy_out(self, state: SDFGState, map_entry: dace.nodes.MapEntry):
        ...
        for ie in state.in_edges(map_entry):
            ...
            if array.storage != self.vector_input_storage:
                ...
                state.add_edge(an, None, map_entry, ie.dst_conn, dace.memlet.Memlet(f"{ie.data.data}_vec[0:{self.vector_width - 1}]"))
        ...
        for oe in state.out_edges(map_exit):
            ...
            if array.storage != self.vector_output_storage:
                ...
                state.add_edge(map_entry, src_conn, an, None, copy.deepcopy(data))
                state.add_edge(an, None, dst, ie.dst_conn, dace.memlet.Memlet(f"{oe.data.data}_vec[0:{self.vector_width - 1}]"))
    ...
    def _extract_single_op(src: str) -> str:
        BINOP_SYMBOLS = {
            ast.Add: "+",
            ast.Sub: "-",
            ast.Mult: "*",
        ...
        for node in ast.walk(tree):
            ...
            # Unary float op
            elif isinstance(node, ast.UnaryOp) and isinstance(node.operand, ast.Constant) and isinstance(node.operand.value, float):

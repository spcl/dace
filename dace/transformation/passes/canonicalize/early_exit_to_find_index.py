...
class EarlyExitToFindIndex(ppl.Pass):
    ...
    def _match(self, loop: LoopRegion, sdfg: SDFG) -> Optional[_Match]:
        ...
        if not self._check_soundness(
                sdfg, loop, cond_block, cond_iedge_bindings, cond_expr_str,
                body_pre_blocks, body_post_blocks, true_pre_break_states):
            return None
    ...
    def _partition_body(self, loop: LoopRegion, cond_block: ConditionalBlock
    ...
    def _resolve_cond_expression(self, loop: LoopRegion, cond_block: ConditionalBlock
    ...
    def _check_soundness(self, sdfg, loop, cond_block, cond_iedge_bindings,
                         cond_expr_str, body_pre_blocks, body_post_blocks,
                         true_pre_break_states) -> bool:
    ...
    def _collect_array_writes(self, blocks, sdfg: SDFG) -> Set[str]:
        ...
        for b in blocks:
            ...
            for n in b.data_nodes():
                ...
                if desc is None:
                    continue
                if getattr(desc, 'transient', False):
                    continue
                out.add(n.data)
        return out
    ...
    def _body_parallelizable_modulo_break(self, loop, cond_block, sdfg) -> bool:
        ...
        try:
            ...
            return instance.can_be_applied(loop_copy.parent_graph, expr_index=0,
                                            sdfg=sdfg_copy, permissive=False)
        except Exception:
            return False
        ...
        for ie in in_es:
            for oe in out_es:
                combined = dace.InterstateEdge(
                    ...
                    assignments={**(ie.data.assignments or {}), **(oe.data.assignments or {})},
                )
    ...
    def _rewrite(self, m: _Match, sdfg: SDFG):
        ...
        phi_name, _ = sdfg.add_array(f'{_PHI_PREFIX}{idx}',
                                     [m.iter_end - m.iter_start + 1],
                                     dace.int64,
                                     transient=True, find_new_name=True)
        exit_buf_name, _ = sdfg.add_scalar(f'{_EXIT_BUF_PREFIX}{idx}',
                                           dace.int64, transient=True,
                                           find_new_name=True)
        ...
        if any(isinstance(b, SDFGState) and len(b.nodes()) > 0 for b in m.body_pre_blocks):
            ...
            sym_bound = True
            self._emit_body_map(s_pre, sdfg, m.body_pre_blocks, m,
        ...
        if any(isinstance(b, SDFGState) and len(b.nodes()) > 0 for b in m.body_post_blocks):
            ...
            if sym_bound:
                ...
                sym_bound = True
            self._emit_body_map(s_post, sdfg, m.body_post_blocks, m,
    ...
    def _emit_reduce_min(self, state: SDFGState, sdfg: SDFG, phi_name: str,
                         exit_buf_name: str, m: _Match):
        N_expr = symbolic.symstr(m.iter_end + 1)
        ...
        # "exit_i = N" semantics.
        node = Reduce(name=f'{m.loop.label}_findfirst_reduce',
                      wcr='lambda a, b: min(a, b)', axes=[0], identity=N_expr)
        node.add_in_connector('_in')
        node.add_out_connector('_out')
        state.add_node(node)
        size = symbolic.simplify(m.iter_end - m.iter_start + 1)
        state.add_edge(read, None, node, '_in',
                       mm.Memlet(data=phi_name, subset=subsets.Range([(0, size - 1, 1)])))
        state.add_edge(node, '_out', write, None,
                       mm.Memlet(data=exit_buf_name, subset=subsets.Range([(0, 0, 1)])))

    def _emit_body_map(self, state: SDFGState, sdfg: SDFG, body_blocks: List,
                        m: _Match, upper_str: str):
        ...
        content_states = [b for b in body_blocks
                          if isinstance(b, SDFGState) and len(b.nodes()) > 0]
        ...
        for e in src_state.edges():
            if e.data is None or e.data.is_empty():
                continue
            state.add_edge(node_map[e.src], e.src_conn,
        ...
        for orig, cn in node_map.items():
            ...
            if src_state.in_degree(orig) == 0 and src_state.out_degree(orig) > 0:
                ...
                state.add_memlet_path(ext, map_entry, cn,

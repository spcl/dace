...
class DetectSoftHierGather(ppl.Pass):
    ...
    gather_template = """
{{
int32_t idx[{vector_length}] = {{ {initializer_values} }};
_softhier_vgather_fp32_int32_(_in, idx, _out, {vector_length});
}}
"""
        ...
        for state in sdfg.all_states():
            for node in state.nodes():
                if isinstance(node, nodes.AccessNode) and node.data.endswith("_packed"):
                    ...
                    if state.sdfg.arrays[node.data].dtype != dace.float32:
                        continue
                    ...
                    if len(idx_data) != 1:
                        #raise Exception(idx_data)
                        continue
                    # dtypes should be float32
                    if state.sdfg.arrays[next(iter(idx_data))].dtype != dace.float32:
                        #raise Exception("Not fp32")
                        continue

                    # TODO: last dimension being the vectorized dimension can't be always correct
                    initializer_values = ", ".join([str(s[-1]) for d, s in idx_data_and_subset])
                    gather_code = DetectSoftHierGather.gather_template.format(initializer_values=initializer_values,

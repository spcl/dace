...
class DefaultSharedMemorySync(ppl.Pass):
    ...
    def tb_exits_analysis(self, map_entry: MapEntry, map_exit: MapExit, 
                                state: SDFGState) -> Tuple[bool, bool, bool]:
        """
        ...
        if not race_cond_danger:
            race_cond_danger = any(
                ...
                and self.map_writes_to_smem(inner_scope)
    ...
    def insert_synchronization_after_tb_exits(self, tb_map_exits: Dict[MapExit, SDFGState]) -> None:
        ...
        for map_exit, state in tb_map_exits.items():
            ...
            sync_tasklet = state.add_tasklet(name="sync_threads", inputs=set(), outputs=set(),

...
class InferGPUGridAndBlockSize(ppl.Pass):
    ...
    def apply_pass(self, sdfg: SDFG, kernels_with_added_tb_maps: Set[nodes.MapEntry]) -> Dict[nodes.MapEntry, Tuple[List, List]]:
        """
        ...
        kernel_maps: Set[Tuple[nodes.MapEntry, SDFGState,]] = set()
    ...
    def _infer_gpu_block_size(self, state:SDFGState, kernel_map_entry: nodes.MapEntry) -> List:
        ...
        if not threadblock_maps:
            ...
            raise ValueError(
                f"{self.__class__.__name__} expects at least one explicit nested GPU_ThreadBlock map, "
    ...
    def _get_internal_threadblock_maps(self, state: SDFGState, kernel_map_entry: nodes.MapEntry) -> List[nodes.MapEntry]:
        """

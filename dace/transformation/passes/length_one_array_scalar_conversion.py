...
for a ``Scalar``, and a by-value scalar cannot receive a written-back result). So a non-transient is
instead STAGED, only when ``stage_nontransients_arrays_into_scalars`` is set:

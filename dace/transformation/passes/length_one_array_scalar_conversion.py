...
"""Rewrite length-1 ``Array`` data to true ``Scalar`` form.

Used by the HLFIR Fortran frontend as post-generation cleanup: a ``Scalar``
on the SDFG signature binds to a plain Python ``int`` / ``float`` whereas a
length-1 ``Array`` needs a 1-element numpy buffer.
"""
import re
...
class ConvertLengthOneArraysToScalars(ppl.Pass):
    """Rewrite every length-1 ``Array`` (shape ``(1,)``) to a true ``Scalar``
    and drop the ``[0]`` accessors that referenced it from interstate-edge
    assignments, conditional-block guards, loop-region conditions and memlet
    subsets.

    :param recursive: Recurse into nested SDFGs (only their TRANSIENT length-1
        arrays are rewritten -- a non-transient nested-SDFG arg is part of its
        parent's signature and rewriting it would change the caller's contract).
    :param transient_only: Restrict the top-level rewrite to transient arrays.

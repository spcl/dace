...
class SchedulePhase():
    ...
    def set_toplevel_to_GPU(self, sdfg: SDFG, type: Type):
        ...
        for state in sdfg.states():
            scope_dict = self.sdfg_scope_dict[
                state] if state in self.sdfg_scope_dict else None  # None -> within nested SDFGs
            ...
            for node in state.nodes():
                ...
                if scope_dict and node in scope_dict and scope_dict[node] is None:  # toplevel node -> change schedule
                    ...
                    if self.verbose:
                        kind = "libnode" if isinstance(node, nodes.LibraryNode) else "map"
                        print(f"Phase1: set {kind} {node} to sequential schedule")

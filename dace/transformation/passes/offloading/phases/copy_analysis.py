...
class CopyAnalysisPhase():
    ...
    def _parse_to_IR(self, sdfg: SDFG, cfr: ControlFlowRegion, curr_node: OffloadingIRNode) -> OffloadingIRNode:
        ...
        for block in cfr.bfs_nodes():
            ...
            for edge in cfr.in_edges(block):
                arrays = OrderedSet(data_name for data_name in edge.data.used_arrays(sdfg.arrays)
                                    if helpers.is_array(data_name, sdfg))
    ...
    def __populate_close_node_sets(self, IR: OffloadingIRNode):
        ...
        IR.close.cpu_set = OrderedSet(array_name for array_name in location_on_gpu if not location_on_gpu[array_name])
    ...
    def get_arrays_used_by_edge(self, sdfg: SDFG, state: SDFGState, edge: MultiConnectorEdge,
        ...
        if edge.data and not edge.data.is_empty():
            ...
            if helpers.is_array(data_name, sdfg):  # array access on edge
                return OrderedSet([data_name])

            elif helpers.is_view(data_name,
    ...
    def get_data_locations_of_map(self, sdfg: SDFG, state: SDFGState,
        ...
        def _add_data(data_name: str, gpu_set: OrderedSet[str], cpu_set: OrderedSet[str],
            ...
            if data_name in gpu_set:  # has already been accessed on GPU
                if not is_gpu:  # is now accessed on CPU
                    raise RuntimeError(
                        "GPU->CPU within map. This should never happen. If outer map is GPU then inner data must also be on GPU (seq map runs as kernel)"
                    )
        ...
        def _recursive_helper(sdfg: SDFG, state: SDFGState, map_entry: nodes.MapEntry, gpu_set: OrderedSet[str],
                              cpu_set: OrderedSet[str], is_gpu: bool):
            ...
            # input & output nodes
            input_and_output = helpers.get_data_used_by_incoming_access_nodes(
                sdfg, state, map_entry) | helpers.get_data_used_by_outgoing_access_nodes(
            ...
            for node in map_nodes:
                if isinstance(node, nodes.MapEntry):  # recurse on inner map
                    ...
                    raise RuntimeError(
                        f"Error: Unknown node type inside map {map_entry}: {node.label} of type {node.__class__.__name__} in state {state}"
                    )

...
def write_co2_from_co1(callsite_stream, name, src_name, dst_name, beg1, beg2, width, height, nodedesc):
...
def write_vecin_from_co2(callsite_stream, name, src_name, dst_name, beg1, beg2, width, height, nodedesc):
    src_storage_name = "CO1"
...
def write_tensor_copy(callsite_stream, memlet, src_name, dst_name, src_storage, dst_storage, nodedesc):
    ...
    if len(subset.ranges) == 1:
        ...
        if src_storage.name == "Ascend_Global" and dst_storage.name == "Ascend_VECIN":
            ...
            write_co2_from_co1(callsite_stream, memlet.data, dst_name, src_name, beg1, beg2, width, height, nodedesc)
        elif src_storage.name == "Ascend_CO2" and dst_storage.name == "Ascend_VECIN":
            write_vecin_from_co2(callsite_stream, memlet.data, dst_name, src_name, beg1, beg2, width, height, nodedesc)
        elif src_storage.name == "Ascend_CO2" and dst_storage.name == "Ascend_Global":
            raise NotImplementedError("CO2 -> Global copy not implemented yet.")
            #write_global_from_co2(callsite_stream, memlet.data, dst_name, src_name, beg1, beg2, width, height, nodedesc)
        else:

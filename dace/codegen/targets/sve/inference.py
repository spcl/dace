...
"""
    SVE Type inference: This is almost the same as the regular `type_inference.py`, but has little tweaks to allow for more aggressive inference in SVE.
    For example, comparing a vector and a scalar produces a bool vector.
    Also, function calls return the type of their result (needed because some math functions can be defined arbitrarily).
"""
...
def _IfExp(t, symbols, inferred_symbols):
    ...
    res = dtypes.result_type_of(type_body, type_orelse)

    if isinstance(type_test,
                  dtypes.vector) and not isinstance(res, dtypes.vector):
        return dtypes.vector(res, -1)
    return res
...
def _Compare(t, symbols, inferred_symbols):
    l = []
    l.append(_dispatch(t.left, symbols, inferred_symbols))
    for o, e in zip(t.ops, t.comparators):
        if o.__class__.__name__ not in cppunparse.CPPUnparser.cmpops:
            continue
        l.append(_dispatch(e, symbols, inferred_symbols))
    return dtypes.vector(dtypes.typeclass(np.bool), -1) if isinstance(
        dtypes.result_type_of(*l), dtypes.vector) else dtypes.typeclass(np.bool)
...
def _Call(t, symbols, inferred_symbols):
    ...
    return inf_type or arg_types[0] if len(
        arg_types) == 1 else dtypes.result_type_of(*arg_types)
...
def _Subscript(t, symbols, inferred_symbols):
    ...
    # Subscript of a pointer returns the base type (as vector in SVE)
    if isinstance(inferred_type, dtypes.pointer):
        return dtypes.vector(inferred_type.base_type, -1)

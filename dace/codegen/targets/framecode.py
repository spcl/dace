...
class DaCeCodeGenerator(object):
    ...
    def __init__(self, sdfg: SDFG):
        ...
        self.to_allocate: DefaultDict[
            ...
            List[Tuple[SDFG, Optional[SDFGState], Optional[nodes.AccessNode],
                       bool, bool, bool]]] = collections.defaultdict(list)
        ...
        # first handle root
        sdfg.reset_cfg_list()
        self._symbols_and_constants[sdfg.cfg_id] = sdfg.free_symbols.union(
            sdfg.constants_prop.keys())
        # then recurse
        for nested, state in sdfg.all_nodes_recursive():
            if isinstance(nested, nodes.NestedSDFG):
                ...
                parent_constants = self._symbols_and_constants[
                    nsdfg.parent_sdfg.cfg_id]
    ...
    def generate_constants(self, sdfg: SDFG, callsite_stream: CodeIOStream):
        ...
        for cstname, (csttype, cstval) in sdfg.constants_prop.items():
            if isinstance(csttype, data.Array):
                const_str = "constexpr " + csttype.dtype.ctype + " " + cstname + "[" + str(
                    cstval.size) + "] = {"
    ...
    def generate_fileheader(self,
        ...
        def _emit_definitions(dtype: dtypes.typeclass,
                              wrote_something: bool) -> bool:
            if isinstance(dtype, dtypes.pointer):
                wrote_something = _emit_definitions(dtype._typeclass,
                                                    wrote_something)
...
''', sdfg)
    ...
    def generate_footer(self, sdfg: SDFG, global_stream: CodeIOStream,
        ...
        initparamnames = sdfg.init_signature(
            for_call=True, free_symbols=self.free_symbols(sdfg))
...
''', sdfg)
    ...
    def generate_external_memory_management(self, sdfg: SDFG,
                                            callsite_stream: CodeIOStream):
        """
        ...
        ext_arrays: Dict[dtypes.StorageType, List[Tuple[
            SDFG, str, data.Data]]] = collections.defaultdict(list)
...
{{''', sdfg)
            ...
            for subsdfg, aname, arr in arrays:
                ...
                callsite_stream.write(
                    f'{allocname} = decltype({allocname})(ptr + {sym2cpp(offset)});',
                    subsdfg)
    ...
    def generate_state(self,
        ...
        self.allocate_arrays_in_scope(sdfg, cfg, state, global_stream,
                                      callsite_stream)
        ...
        for instr in self._dispatcher.instrumentation.values():
            if instr is not None:
                instr.on_state_begin(sdfg, cfg, state, callsite_stream,
                                     global_stream)
        ...
        if generate_state_footer:
            ...
            self.deallocate_arrays_in_scope(sdfg, state.parent_graph, state,
                                            global_stream, callsite_stream)

            # Invoke all instrumentation providers
            for instr in self._dispatcher.instrumentation.values():
                if instr is not None:
                    instr.on_state_end(sdfg, cfg, state, callsite_stream,
                                       global_stream)

    def generate_states(self, sdfg: SDFG, global_stream: CodeIOStream,
                        callsite_stream: CodeIOStream) -> Set[SDFGState]:
        states_generated = set()

        opbar = progress.OptionalProgressBar(
            len(sdfg.states()), title=f'Generating code (SDFG {sdfg.cfg_id})')
    ...
    def determine_allocation_lifetime(self, top_sdfg: SDFG):
        ...
        access_instances: Dict[int, Dict[str,
                                         List[Tuple[SDFGState,
                                                    nodes.AccessNode]]]] = {}
        for sdfg in top_sdfg.all_sdfgs_recursive():
            shared_transients[sdfg.cfg_id] = sdfg.shared_transients(
            ...
            instances: Dict[str, List[Tuple[
                SDFGState, nodes.AccessNode]]] = collections.defaultdict(list)
            ...
            for state in cfg_analysis.blockorder_topological_sort(
                    sdfg, ignore_nonstate_blocks=True):
        ...
        for sdfg, name, desc in top_sdfg.arrays_recursive(
            ...
            first_state_instance, first_node_instance = access_instances[
                sdfg.cfg_id].get(name, [(None, None)])[0]
            last_state_instance, last_node_instance = access_instances[
                sdfg.cfg_id].get(name, [(None, None)])[-1]

            # Cases
            if top_lifetime in (dtypes.AllocationLifetime.Persistent,
            ...
            if (name in shared_transients[sdfg.cfg_id]
                    or top_lifetime is dtypes.AllocationLifetime.SDFG):
                ...
                for cfg in sdfg.all_control_flow_regions():
                    block_syms = cfg.used_symbols(all_symbols=True,
                                                  with_contents=False)
            ...
            if (not isinstance(curscope, nodes.EntryNode)
                    and utils.is_nonfree_sym_dependent(
                        first_node_instance, desc, first_state_instance,
                        fsymbols)):
                ...
                if first_state_instance != last_state_instance:
                    ...
                    instances: List[Tuple[
                        SDFGState, nodes.AccessNode]] = access_instances[
                            sdfg.cfg_id][name]

                    # A view gets "allocated" everywhere it appears
                    if isinstance(desc, data.View):
                        for s, n in instances:
                            self.to_allocate[s].append(
                                (sdfg, s, n, False, True, False))
                            self.to_allocate[s].append(
                                (sdfg, s, n, False, False, True))
                        self.where_allocated[(sdfg, name)] = cursdfg
                        continue

                    if any(inst not in reachability[sdfg.cfg_id]
                           [first_state_instance] for inst in instances):
                        first_state_instance, last_state_instance = _get_dominator_and_postdominator(
                            sdfg, instances)
                        ...
                        self.to_allocate[curscope].append(
                            (sdfg, None, nodes.AccessNode(name), True, False,
                             False))
                    else:
                        self.to_allocate[curscope].append(
    ...
    def allocate_arrays_in_scope(self, sdfg: SDFG, cfg: ControlFlowRegion,
        ...
        for tsdfg, state, node, declare, allocate, _ in self.to_allocate[
                scope]:
            ...
            self._dispatcher.dispatch_allocate(
                tsdfg, cfg if state is None else state.parent_graph, state,
                state_id, node, desc, function_stream, callsite_stream,
                declare, allocate)
    ...
    def deallocate_arrays_in_scope(self, sdfg: SDFG, cfg: ControlFlowRegion,
        ...
        for tsdfg, state, node, _, _, deallocate in self.to_allocate[scope]:
            ...
            self._dispatcher.dispatch_deallocate(tsdfg, state.parent_graph,
                                                 state, state_id, node, desc,
                                                 function_stream,
                                                 callsite_stream)
    ...
    def generate_code(
        ...
        self.allocate_arrays_in_scope(sdfg, sdfg, sdfg, global_stream,
                                      callsite_stream)
        ...
        for cname, (ctype, _) in sdfg.constants_prop.items():
            if isinstance(ctype, data.Array):
                self.dispatcher.defined_vars.add(cname,
                                                 disp.DefinedType.Pointer,
                                                 ctype.dtype.ctype)
            else:
                self.dispatcher.defined_vars.add(cname,
        ...
        for isvarName, isvarType in interstate_symbols.items():
            if isvarType is None:
                raise TypeError(
                    f'Type inference failed for symbol {isvarName}')
            ...
            if (schedule in (dtypes.ScheduleType.FPGA_Device,
                             dtypes.ScheduleType.FPGA_Multi_Pumped)
                    and config.Config.get('compiler', 'fpga',
                                          'vendor').lower() == 'intel_fpga'):
                # Emit OpenCL type
        ...
        if len(states_generated) != len(sdfg.states()):
            raise RuntimeError(
                ...
                "\n  Generated: {}\n  Missing: {}".format(
                    ...
                    [s.label
                     for s in (set(sdfg.states()) - states_generated)]))

        # Deallocate transients
        self.deallocate_arrays_in_scope(sdfg, sdfg, sdfg, global_stream,
                                        callsite_stream)
        ...
        for line in generated_code.split('\n'):
            ...
            if len(label) > 0:
                if label[0] not in gotos:
                    last_line = ''
                    continue
                if f'goto {label[0]};' in last_line and goto_ctr[
                        label[0]] == 1:  # goto followed by label
...
def _get_dominator_and_postdominator(sdfg: SDFG,
                                     accesses: List[Tuple[SDFGState,
                                                          nodes.AccessNode]]):
    """
    ...
    """
    alldoms: Dict[ControlFlowBlock,
                  Set[ControlFlowBlock]] = collections.defaultdict(
                      lambda: set())
    allpostdoms: Dict[ControlFlowBlock,
                      Set[ControlFlowBlock]] = collections.defaultdict(
                          lambda: set())
    idom: Dict[ControlFlowRegion, Dict[ControlFlowBlock,
                                       ControlFlowBlock]] = {}
    ipostdom: Dict[ControlFlowRegion, Dict[ControlFlowBlock,
                                           ControlFlowBlock]] = {}
    utils.get_control_flow_block_dominators(sdfg, idom, alldoms, ipostdom,
                                            allpostdoms)
    ...
    while any(start_state not in alldoms[n] for n in states):
        if idom[start_state] is start_state:
            raise NotImplementedError(
                f'Could not find an appropriate dominator for allocation of "{data_name}"'
            )
    ...
    while any(end_state not in allpostdoms[n] for n in states):
        if ipostdom[end_state] is end_state:
            raise NotImplementedError(
                f'Could not find an appropriate post-dominator for deallocation of "{data_name}"'
            )

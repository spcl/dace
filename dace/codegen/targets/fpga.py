...
class FPGACodeGen(TargetCodeGenerator):
    ...
    def generate_modules(self, sdfg, state, kernel_name, subgraphs, parameters,
                         scalar_parameters, symbol_parameters,
                         top_level_local_data, module_stream, entry_stream,
                         host_stream):
        ...
        for subgraph in subgraphs:
            ...
            self.generate_module(sdfg, state, module_name, subgraph,
                                 parameters[subgraph] + scalar_parameters,
    ...
    def _generate_Reduce(self, sdfg, dfg, state_id, node, function_stream,
        ...
        # Set initial value to zero. This is helpful for Intel Compiler
        all_axes_collapsed = (len(axes) == input_dims)
        if all_axes_collapsed:
            accumulator = "_{}_accumulator".format(output_memlet.data)
            callsite_stream.write("{} {} = 0;".format(output_type, accumulator),
                                  sdfg, state_id, node)
        ...
        for i in range(end_braces):
            ...
            if i == end_braces - 1 and all_axes_collapsed:
                ...
                if offset:
                    dst_expr += " + " + offset
                def_type = self._dispatcher.defined_vars.get(
                    output_memlet.data)
                callsite_stream.write(

...
def iterate_multibank_interface_ids(array: dt.Array,
                                    interface_ids: Union[int,
                                                         List[Tuple[int,
                                                                    int]]]):
    """
...
def get_multibank_ranges_from_subset(subset: Union[subsets.Subset, str],
    ...
    try:
        ...
        raise ValueError(
            f"Only constant evaluatable indices allowed for multibank-memlets on the bank index."
        )
    return (low, high + 1)
...
def fpga_ptr(name: str,
    ...
    if (desc is not None and is_multibank_array_with_distributed_index(desc)):
        ...
        if (subset_info == None):
            raise ValueError(
                "Cannot generate name for bank without subset info")
        elif (isinstance(subset_info, int)):
            name = f"{mem_type}{subset_info}_{name}"
        elif (isinstance(subset_info, subsets.Subset)):
            if (sdfg == None):
                raise ValueError(
                    "Cannot generate name for bank using subset if sdfg not provided"
                )
            low, high = get_multibank_ranges_from_subset(subset_info, sdfg)
            if (low + 1 != high):
                raise ValueError(
                    "ptr cannot generate names for subsets accessing more than one memory bank"
                )
        ...
        if not self._in_device_code:
            ...
            for is_output, arg_name, arg, interface_id in state_parameters:
                ...
                if (isinstance(arg, dt.Array)):
                    for bank, _ in iterate_multibank_interface_ids(
                            arg, interface_id):
...
""")
                    ...
                    if scope == subgraph or n.desc(scope).transient:
                        ...
                        if is_multibank_array_with_distributed_index(desc):
                            ...
                            if n.data in array_to_banks_used_in:
                                array_to_banks_used_in[n.data].update(
                                    current_banks_in)
                            else:
                                array_to_banks_used_in[
                                    n.data] = current_banks_in
                            if n.data in array_to_banks_used_out:
                                array_to_banks_used_out[n.data].update(
                                    current_banks_out)
                            else:
                                array_to_banks_used_out[
                ...
                if (isinstance(desc, dt.Array)
                    ...
                    if data_name in data_to_interface:
                        ...
                        if is_multibank_array_with_distributed_index(desc):
                            tmp_interface_ids = []
                            if is_output:
                                banks_looked_at = array_to_banks_used_out[
                                    data_name]
                            else:
                                banks_looked_at = array_to_banks_used_in[
                                    data_name]
    ...
    def _emit_copy(self, sdfg, state_id, src_node, src_storage, dst_node,
        ...
        if (host_to_device or device_to_host
            ...
            is_src_using_multibank = (
                src_is_subset
                and is_multibank_array_with_distributed_index(src_nodedesc))
            is_dst_using_multibank = (
                not src_is_subset
                and is_multibank_array_with_distributed_index(dst_nodedesc))
    ...
    def copy_memory(self, sdfg, dfg, state_id, src_node, dst_node, edge,
        ...
        if (isinstance(src_node, dace.sdfg.nodes.AccessNode)
            ...
            src_is_multibank = is_multibank_array_with_distributed_index(
                src_array)
            dst_is_multibank = is_multibank_array_with_distributed_index(
                dst_array)

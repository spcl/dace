            ...
            for node in scope.nodes():
                if(isinstance(node, nd.NestedSDFG)):
                    if unique_functions and not unique_functions_hash and node.unique_name != "":
                        backup.append((node, True, deepcopy(node.unique_name), deepcopy(node.symbol_mapping)))
                        node.unique_name = f"{node.unique_name}_{param}{paramval}"
                    else:
                        backup.append((node, False, deepcopy(node.sdfg.name), deepcopy(node.symbol_mapping)))
                        node.sdfg.name = f"{node.sdfg.name}_{param}{paramval}"
                    for nstate in node.sdfg.nodes():
                        backup.extend(nsdfg_prepare_unroll(nstate, paramname, paramval))

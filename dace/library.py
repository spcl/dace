...
def register_expansion(library_node: LibraryNode, expansion_name: str):
    ...
    :param library_node: The library node class the expansion implements.
    :param expansion_name: Name to register the implementation under. ``'ai'`` is reserved, since
                           every library node already provides an AI-generated expansion under that
                           name (see :mod:`dace.libraries.ai`).
    """
...
def get_environment(env_name):
    try:
        return dace.library._DACE_REGISTERED_ENVIRONMENTS[env_name]
    except KeyError:
        pass
    # Environments generated by the 'ai' expansion live in a directory outside the package, and are
    # only re-registered once that library is imported. An SDFG loaded from disk may reference one
    # before anything has imported it.
    if env_name.startswith('dace_ai_env_'):
        # Importing by name so that the module-level `dace` binding is not shadowed
        importlib.import_module('dace.libraries.ai')  # Registers previously generated environments
        if env_name in dace.library._DACE_REGISTERED_ENVIRONMENTS:
            return dace.library._DACE_REGISTERED_ENVIRONMENTS[env_name]

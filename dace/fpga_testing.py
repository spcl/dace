            ...
            with temporary_config():
                ...
                if xilinx:
                    ...
                    for sdfg_name in sdfg_names:
                        build_folder = Path(Config.get(
                            "default_build_folder")) / sdfg_name / "build"
                        ...
                        if run_synthesis:
                            print_status(
                                f"{base_name} [Xilinx]: Running high-level "
                            ...
                            print_success(f"{base_name} [Xilinx]: High-level "
                                          ...
                                          f"{sdfg_name}.")
                            open(build_folder / "synthesis.out",
                                 "w").write(syn_out)
                            open(build_folder / "synthesis.err",
                                 "w").write(syn_err)
                            ...
                            if assert_ii_1:
                                ...
                                for m in re.finditer(r"Final II = ([0-9]+)",
                                                     hls_log):
                                    loops_found = True
                                    if int(m.group(1)) != 1:
                                        dump_logs((syn_out, syn_err))
                                        print_error(
                                            f"{base_name} [Xilinx]: Failed "
                                            f"to achieve II=1.")
                                        return False
                                if not loops_found:
                                    dump_logs((syn_out, syn_err))

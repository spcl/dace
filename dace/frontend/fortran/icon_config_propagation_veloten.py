...
if __name__ == "__main__":
    ...
    base_dir_veloten = f"{base_icon_path}/src/"
    fortran_files = find_path_recursive(base_dir_veloten)

    # Construct the primary velocity tendencies AST.
    parse_cfg = ParseConfig(
        sources=[Path(f) for f in fortran_files],
        entry_points=[('mo_velocity_advection', 'velocity_tendencies')], #[('radiation_interface', 'radiation')],
        config_injections=None,#config_injection_list('dace/frontend/fortran/conf_files'),
    )

...
class IntrinsicTransformation:
    ...
    @abstractmethod
    def replace(
        func_name: ast_internal_classes.Name_Node, args: ast_internal_classes.Arg_List_Node, line
...
class DirectReplacement(IntrinsicTransformation):
    ...
    def replace_int_kind(args: ast_internal_classes.Arg_List_Node, line):
        return ast_internal_classes.Int_Literal_Node(
            value=str(math.ceil((math.log2(math.pow(10, int(args.args[0].value))) + 1) / 8)), line_number=line
        )
...
class LoopBasedReplacementTransformation(IntrinsicNodeTransformer):
    ...
    def _parse_array(
        self, node: ast_internal_classes.Execution_Part_Node, arg: ast_internal_classes.FNode
...
class SumProduct(LoopBasedReplacementTransformation):
    ...
    def _summarize_args(
        ...
        par_Decl_Range_Finder(
            self.argument_variable, self.loop_ranges, [], [], self.count, new_func_body, self.scope_vars, True
        )
    ...
    def _generate_loop_body(self, node: ast_internal_classes.FNode) -> ast_internal_classes.BinOp_Node:
        ...
        return ast_internal_classes.BinOp_Node(
            ...
            op="=",
            rval=ast_internal_classes.BinOp_Node(
                lval=node.lval, op=self._result_update_op(), rval=self.argument_variable, line_number=node.line_number
            ),
            line_number=node.line_number,
        )
...
class AnyAllCountTransformation(LoopBasedReplacementTransformation):
    ...
    def _summarize_args(
        ...
        rangeslen_left = []
        par_Decl_Range_Finder(
            self.first_array, self.loop_ranges, [], rangeslen_left, self.count, new_func_body, self.scope_vars, True
        )
        ...
        rangeslen_right = []
        par_Decl_Range_Finder(
            self.second_array, loop_ranges_right, [], rangeslen_right, self.count, new_func_body, self.scope_vars, True
        )
...
class Count(LoopBasedReplacement):
    ...
    class Transformation(AnyAllCountTransformation):
        ...
        def _result_loop_update(self, node: ast_internal_classes.FNode):
            ...
            return ast_internal_classes.BinOp_Node(
                lval=copy.deepcopy(node.lval), op="=", rval=update, line_number=node.line_number
            )
...
class MinMaxValTransformation(LoopBasedReplacementTransformation):
    ...
    def _initialize_result(self, node: ast_internal_classes.FNode) -> ast_internal_classes.BinOp_Node:
        ...
        return ast_internal_classes.BinOp_Node(
            lval=node.lval, op="=", rval=self._result_init_value(self.argument_variable), line_number=node.line_number
        )

    def _generate_loop_body(self, node: ast_internal_classes.FNode) -> ast_internal_classes.BinOp_Node:

        cond = ast_internal_classes.BinOp_Node(
            lval=self.argument_variable, op=self._condition_op(), rval=node.lval, line_number=node.line_number
        )
        body_if = ast_internal_classes.BinOp_Node(
            lval=node.lval, op="=", rval=self.argument_variable, line_number=node.line_number
        )
        return ast_internal_classes.If_Stmt_Node(
            cond=cond,
            body=body_if,
...
class Merge(LoopBasedReplacement):
    class Transformation(LoopBasedReplacementTransformation):
        ...
        def _summarize_args(
            ...
            par_Decl_Range_Finder(
                self.first_array, self.loop_ranges, [], [], self.count, new_func_body, self.scope_vars, True
            )

            loop_ranges = []
            par_Decl_Range_Finder(
                self.second_array, loop_ranges, [], [], self.count, new_func_body, self.scope_vars, True
            )
            ...
            if self.mask_first_array is not None:
                loop_ranges = []
                par_Decl_Range_Finder(
                    self.mask_first_array, loop_ranges, [], [], self.count, new_func_body, self.scope_vars, True
                )
            ...
            if self.mask_second_array is not None:
                loop_ranges = []
                par_Decl_Range_Finder(
                    self.mask_second_array, loop_ranges, [], [], self.count, new_func_body, self.scope_vars, True
                )
        ...
        def _generate_loop_body(self, node: ast_internal_classes.FNode) -> ast_internal_classes.BinOp_Node:
            ...
            copy_first = ast_internal_classes.BinOp_Node(
                lval=copy.deepcopy(self.destination_array), op="=", rval=self.first_array, line_number=node.line_number
            )

            copy_second = ast_internal_classes.BinOp_Node(
                lval=copy.deepcopy(self.destination_array), op="=", rval=self.second_array, line_number=node.line_number
            )
            ...
            return ast_internal_classes.If_Stmt_Node(
                cond=self.mask_cond, body=body_if, body_else=body_else, line_number=node.line_number
            )
...
class MathFunctions(IntrinsicTransformation):
    ...
    def generate_scale(arg: ast_internal_classes.Call_Expr_Node):
        ...
        rval = ast_internal_classes.Call_Expr_Node(
            name=ast_internal_classes.Name_Node(name="pow"), type="INTEGER", args=[const_two, i], line_number=line
        )
    ...
    INTRINSIC_TO_DACE = {
        ...
        "FLOOR": {"REAL": MathTransformation("floor", "INTEGER"), "DOUBLE": MathTransformation("floor", "INTEGER")},

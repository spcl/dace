...
from fparser.two.Fortran2003 import Module_Stmt, Function_Stmt, Subroutine_Stmt, Derived_Type_Stmt, \
    ...
    Intrinsic_Function_Reference, Kind_Selector, \
    Derived_Type_Def, Module, Function_Reference, Structure_Constructor, Call_Stmt, \
    Intrinsic_Name, Access_Stmt, Enum_Def, Expr, Enumerator, Real_Literal_Constant, Actual_Arg_Spec, \
    Parenthesis, Level_2_Expr, Level_3_Expr, Execution_Part, \
    ...
    Data_Stmt_Value_List, Section_Subscript_List, Explicit_Shape_Spec, \
    Component_Initialization, If_Then_Stmt, Else_If_Stmt, Else_Stmt, If_Construct, \
    Level_4_Expr, Level_5_Expr, Add_Operand, Mult_Operand, Assignment_Stmt, Loop_Control, \
    If_Stmt, Or_Operand, End_Module_Stmt, \
    Data_Stmt, Data_Stmt_Set, Data_Stmt_Value, Write_Stmt, Data_Component_Def_Stmt, Allocate_Stmt, \
    Goto_Stmt, Continue_Stmt, Format_Stmt, Stmt_Function_Stmt, Internal_Subprogram_Part, \
    Private_Components_Stmt, Language_Binding_Spec, Type_Attr_Spec, Suffix, Equiv_Operand, Case_Value_Range_List
...
from dace.frontend.fortran.ast_desugaring_v2.analysis import interface_specs, generic_specs, \
    ...
    ident_spec, _lookup_dataref, _does_type_signature_match, _track_local_consts
from dace.frontend.fortran.ast_desugaring_v2.pruning import consolidate_uses
from dace.frontend.fortran.ast_desugaring_v2.types import NUMPY_TYPES, TYPE_SPEC, SPEC, SPEC_TABLE, ConstTypeInjection, ConstInjection, ConstInstanceInjection, \
    LITERAL_CLASSES, LITERAL_TYPES, numpy_type_to_literal
from dace.frontend.fortran.ast_desugaring_v2.utils import NAMED_STMTS_OF_INTEREST_CLASSES, SCOPE_OBJECT_TYPES, \
    find_name_of_stmt, find_scope_ancestor, find_named_ancestor, \
    find_name_of_node, append_children, remove_self, replace_node, set_children, prepend_children, \
...
def make_practically_constant_global_vars_constants(ast: Program) -> Program:
    ...
    never_assigned: Set[SPEC] = {
        k
        for k, v in ident_map.items() if isinstance(v, Entity_Decl) and not find_type_of_entity(v, alias_map).const
...
def const_eval_nodes(ast: Program) -> Program:
    EXPRESSION_CLASSES = (LITERAL_CLASSES, Expr, Equiv_Operand, Add_Operand, Or_Operand, Mult_Operand, Level_2_Expr,
                          Level_3_Expr, Level_4_Expr, Level_5_Expr, Intrinsic_Function_Reference)
    ...
    NON_EXPRESSION_CLASSES = (Explicit_Shape_Spec, Loop_Control, Call_Stmt, Function_Reference, Initialization,
                              Component_Initialization, Section_Subscript_List, Write_Stmt, Allocate_Stmt)
...
def _type_injection_applies_to_instance(item: ConstTypeInjection, defn_spec: SPEC, comp_spec: SPEC,
...
def _find_items_applicable_to_instance(items: Iterable[ConstInjection], inst_ref: Union[Name, Part_Ref, Data_Ref,
                                                                                        Entity_Decl],
...
def inject_const_evals(ast: Program, inject_consts: Optional[List[ConstInjection]] = None) -> Program:
    ...
    for scope_spec, items in items_by_scopes.items():
        ...
        drefs: List[Data_Ref] = [
            dr for dr in walk(scope, Data_Ref)
            if find_type_dataref(dr, find_scope_spec(dr), alias_map).spec != ('CHARACTER', )
        ]
        names: List[Name] = walk(scope, Name)
        allocateds: List[Intrinsic_Function_Reference] = [
            c for c in walk(scope, Intrinsic_Function_Reference) if c.children[0].string == 'ALLOCATED'
        ]

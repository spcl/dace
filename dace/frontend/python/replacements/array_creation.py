...
def _numpy_full(
    ...
    if any(isinstance(s, str) for s in shape):
        raise DaceSyntaxError(
            pv,
            None,
            f'Data-dependent shape {shape} is currently not allowed. Only constants and symbolic values can be used.',
        )
...
def _ndarray_fill(
    ...
    if isinstance(value, sp.Expr):
        raise NotImplementedError(
            f"{arr}.fill is not implemented for symbolic expressions ({value})."
        )  # Look at `full`.
...
def _numpy_ones_like(
    pv: ProgramVisitor, sdfg: SDFG, state: SDFGState, a: str, dtype: dtypes.typeclass = None, shape: Shape = None
):
    """Creates and array of the same shape and dtype as a and initializes it
    with ones.
    """
...
def _arange(
    ...
    if isinstance(start, str):
        raise TypeError(
            f'Cannot compile numpy.arange with a scalar start value "{start}" (only constants and symbolic '
            'expressions are supported). Please use numpy.linspace instead.'
        )
    if isinstance(stop, str):
        raise TypeError(
            f'Cannot compile numpy.arange with a scalar stop value "{stop}" (only constants and symbolic '
            'expressions are supported). Please use numpy.linspace instead.'
        )
    if isinstance(step, str):
        raise TypeError(
            f'Cannot compile numpy.arange with a scalar step value "{step}" (only constants and symbolic '
            'expressions are supported). Please use numpy.linspace instead.'
        )

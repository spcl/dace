...
static bool parseSimpleSection(hlfir::DesignateOp dg, SectionShape &out) {
    ...
    for (bool t : tripFlags) if (t) tripletCount++;
...
struct FoldCopyInOutPass
    ...
    void tryFold(hlfir::CopyInOp cin) {
        ...
        getOperation().walk([&](hlfir::CopyOutOp op) {
            if (op.getOperand(1) == cin.getResult(1))
                copyOuts.push_back(op);
        });
    ...
    void rewriteAccess(hlfir::DesignateOp useDg,
                       mlir::Value parent,
                       const SectionShape &sec,
                       mlir::OpBuilder &b) {
        ...
        if (loConst && *loConst == 1) {
            ...
            mlir::Value one = b.create<mlir::arith::ConstantOp>(
                loc, b.getIndexType(), b.getIndexAttr(1));
            ...
            for (auto idx : aliasIdx) {
                ...
                if (loMinusOne) {
                    if (!idx.getType().isIndex())
                        shifted = b.create<fir::ConvertOp>(
                            loc, b.getIndexType(), idx);
                    shifted = b.create<mlir::arith::AddIOp>(
                        loc, shifted, loMinusOne);
                }
                ...
                if (isT) {
                    ...
                    auto shift = [&](mlir::Value v) {
                        ...
                        if (!v.getType().isIndex())
                            vc = b.create<fir::ConvertOp>(
                                loc, b.getIndexType(), v);
                        return (mlir::Value)b.create<mlir::arith::AddIOp>(
                            loc, vc, loMinusOne);
                    };
        ...
        auto newOp = b.create<hlfir::DesignateOp>(
            ...
            /*is_triplet=*/(newTripFlags.empty() ? mlir::DenseBoolArrayAttr{}

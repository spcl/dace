...
size_t CommonPrefix(It begin, It end, size_t start_index, ExtractKey&& extract_key, ElementKey&& element_key) {
  ...
  for (++begin; begin != end; ++begin) {
    ...
    if (element_key(largest_match_list[start_index]) != element_key(current_list[start_index])) return start_index;
...
struct SortStarter {
  ...
  static void sort(It begin, It end, std::ptrdiff_t num_elements, ExtractKey& extract_key,
    ...
    if (StdSortIfLessThanThreshold<StdSortThreshold>(begin, end, num_elements, extract_key)) return;

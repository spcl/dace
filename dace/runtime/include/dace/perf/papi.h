...
#define CHECK_BOUNDS
// Disable runtime byte movement recording. Defining this can reduce cache line
// ping pong.
...
// Define a threshold for flushing (= don't flush at every supersection, but
// only if the buffer is filled to a certain percentage.)
 ...
 public:
  ...
  void initialize_elements() {
    ...
    for (size_t i = 0; i < m_size; ++i) {
      // The following 2 lines should have the same effect (line 1 is
      // copy-assigned, line 2 is using the placement operator new). The 2nd
      // line should be more efficient
  ...
  void clear() {
    if (align_offset == std::numeric_limits<size_t>::max()) return;
    for (size_t i = 0; i < m_size; ++i) {
      (*this)[i].~T();
    }
  }
...
class thread_lock_context_t {
 public:
  thread_lock_context_t() : notified(true), iteration(0) {}
  std::mutex mutex;
...
int64_t getThreadID() {
  ...
  // More than 64 CPUs are put into processor groups, so the normal
...
class ThreadLockReleaser {
 public:
  ...
  size_t getIteration() const { return m_iteration; }
...
// This class provides an interface to lock threads to prevent multiple
// concurrent measurements on the same core
class ThreadLockProvider {
 public:
  ...
  ThreadLockReleaser enqueue() {
    ...
    // This instance is now blocking the thread. When the return value is
    // destroyed, it will no longer block
 ...
 public:
  ...
  void report(Report& rep, int* event_override = nullptr) const {
    ...
    if (m_flags == ValueSetType::Default || m_flags == ValueSetType::Copy) {
      ...
      // Markers have no values attached to them, so they are not included in
      // the report entry_name = "papi_loop_start";
      return;
    } else if (m_flags == ValueSetType::marker_supersection_start) {
      // entry_name = "papi_supersection_start (node " +
...
// Class to store the value sets during execution and writing it to disk after
// execution
template <int... events>
class PAPIValueStore {
 protected:
 ...
 public:
  ...
  ~PAPIValueStore() {
    if (this->m_insertion_position > 0) flush();
  }

  // Does a couple of empty runs (starting and immediately stopping counters) to
  // get a rough estimate of overheads
  ...
  // Provides a thread-safe implementation to increase a counter representing
  // the bytes moved
  ...
  size_t getNewSlotPosition() {
    ...
    do {
      ...
      if (pos >= m_values.size()) {
        ...
        // Ideally, we only let the main thread flush (to avoid excess memory
        // movements, especially with NUMA) However, it's hard to guarantee
        // termination like this. (What if the main thread is done and will
        // never call getNewSlotPosition())
        if (m_vec_mutex.try_lock()) {
          // We got the lock, so we can flush
          flush();
          m_vec_mutex.unlock();
        } else {
          // We didn't get the lock, which means that somebody else is already
          // flushing. Since we lost a lot of time already, we can just use the
          // lock to wait instead of spinlocking.
  ...
  PAPIValueSet<events...>& getNewValueSet(
    ...
    if (correct_subset) {
      // If we have a good subset (same order, same start, same entries), we can
      // skip this expensive operation.
...
class PAPIPerf {
 public:
  PAPIPerf(const bool multiplexing = false) : m_event_set(PAPI_NULL) {
...
#endif
    // Since we allow storing less, we have to make sure there's no stale data
    // in the value store. Set to an invalid value, i.e. max.

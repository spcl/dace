...
class ArrayStreamViewThreadlocal {
 ...
 public:
  ...
  void push(const T& element) { m_array[m_elements++] = element; }
  ...
  /*
          void push_if(const dace::vec<T, 4>& element, const dace::vec<int, 4>&
     mask) { for (int v = 0; v < 4; ++v) { if (mask[v]) { m_array[m_elements++]
     = element[v];
                  }
              }
          }

          void push_if(const dace::vec<T, 8>& element, const dace::vec<int, 8>&
     mask) { for (int v = 0; v < 8; ++v) { if (mask[v]) { m_array[m_elements++]
     = element[v];
                  }
              }
          }

...
struct const_int_range<RangeBegin, RangeEnd, RangeSkip> {
  ...
  static DACE_CONSTEXPR DACE_HDFI size_t len(size_t dim) { return (RangeEnd - RangeBegin + RangeSkip - 1) / RangeSkip; }
...
struct const_int_range<RangeBegin, RangeEnd, RangeSkip, ArgRanges...> {
  ...
  static constexpr int size =
      const_int_range<ArgRanges...>::size * const_int_range<RangeBegin, RangeEnd, RangeSkip>::size;
  ...
  static DACE_CONSTEXPR DACE_HDFI int index_value(const size_t range_value, const size_t dimension) {
    const int _ranges[] = {RangeBegin, RangeEnd, RangeSkip, ArgRanges...};
    if (dimension == dims - 1)
      return _ranges[3 * dimension] + (range_value % len(dimension)) * _ranges[3 * dimension + 2];
    auto value = range_value;
    for (auto dim = dimension + 1; dim < dims; ++dim) {
      value /= len(dim);
    }
    return _ranges[3 * dimension] + (value % len(dimension)) * _ranges[3 * dimension + 2];
  }
  ...
  static DACE_CONSTEXPR DACE_HDFI std::array<int, dims> index_values(const size_t range_value) {
    ...
    for (int dim = dims - 1; dim >= 0; --dim) {
      values[dim] = _ranges[3 * dim] + (value % len(dim)) * _ranges[3 * dim + 2];
      value /= len(dim);
    }
    return values;
  }
...
class int_range {
 ...
 private:
  ...
  DACE_HDFI int _calc_length() {
    ...
    for (size_t i = 0; i < kDims; ++i) {
      auto length =
          (std::get<1>(_ranges[i]) - std::get<0>(_ranges[i]) + std::get<2>(_ranges[i]) - 1) / std::get<2>(_ranges[i]);
 ...
 public:
  DACE_HDFI int_range(ArgRanges&&... ranges) : _ranges({ranges...}), _total_length(_calc_length()) {
    // -std=c++1z
  ...
  DACE_HDFI int index_value(const size_t range_value, const size_t dimension) const {
    if (dimension == kDims - 1)
      return std::get<0>(_ranges[dimension]) +
             (range_value % _range_lengths[dimension]) * std::get<2>(_ranges[dimension]);
    auto value = range_value;
    for (auto dim = dimension + 1; dim < kDims; ++dim) {
      value /= _range_lengths[dim];
    }
    return std::get<0>(_ranges[dimension]) + (value % _range_lengths[dimension]) * std::get<2>(_ranges[dimension]);
  }
  DACE_HDFI std::array<int, kDims> index_values(const size_t range_value) const {
    std::array<int, kDims> values;
    auto value = range_value;
    for (int dim = kDims - 1; dim >= 0; --dim) {
      values[dim] = std::get<0>(_ranges[dim]) + (value % _range_lengths[dim]) * std::get<2>(_ranges[dim]);
      value /= _range_lengths[dim];
    }
    return values;
  }

...
template <typename T>
struct is_typeless_pi {
  static constexpr bool value = false;
};
#define MAKE_TYPELESS_PI(type)          \
  template <>                           \
  struct is_typeless_pi<type> {         \
...
struct typeless_pi_exp {
  ...
  DACE_CONSTEXPR DACE_HDFI operator float() const {
    using std::pow;
    return mult * pow(static_cast<float>(M_PI), exp);
  }

  DACE_CONSTEXPR DACE_HDFI operator double() const {
    using std::pow;
    return mult * std::pow(static_cast<double>(M_PI), exp);
  }
  ...
  DACE_CONSTEXPR DACE_HDFI operator long double() const {
    using std::pow;
    return mult * std::pow(static_cast<long double>(M_PI), exp);
  }
#endif

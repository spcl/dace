...
struct CopyND<T, VECLEN, ALIGNED, COPYDIM, OTHER_COPYDIMS...> {
  ...
  struct ConstSrc {
    ...
    static DACE_HDFI void Copy(const T *src, T *dst, const int &dst_stride,
                               const Args &...dst_otherdims) {
#if !defined(__CUDA_ARCH__) && !defined(__HIP_DEVICE_COMPILE__)
      // Memcpy specialization
      if (sizeof...(OTHER_COPYDIMS) == 0 && SRC_STRIDE == 1 &&
          dst_stride == 1) {
      ...
      for (int i = 0; i < COPYDIM; ++i)
        CopyND<T, VECLEN, ALIGNED, OTHER_COPYDIMS...>::template ConstSrc<
            OTHER_SRCDIMS...>::Copy(src + i * SRC_STRIDE, dst + i * dst_stride,
                                    dst_otherdims...);
    }
    ...
    static DACE_HDFI void Accumulate(const T *src, T *dst, ACCUMULATE acc,
      ...
      for (int i = 0; i < COPYDIM; ++i)
        CopyND<T, VECLEN, ALIGNED, OTHER_COPYDIMS...>::template ConstSrc<
            OTHER_SRCDIMS...>::Accumulate(src + i * SRC_STRIDE,
                                          dst + i * dst_stride, acc,
                                          dst_otherdims...);
    }

    template <typename ACCUMULATE, typename... Args>
    static DACE_HDFI void Accumulate_atomic(const T *src, T *dst,
                                            ACCUMULATE acc,
                                            const int &dst_stride,
                                            const Args &...dst_otherdims) {
      __DACE_UNROLL
      for (int i = 0; i < COPYDIM; ++i)
        CopyND<T, VECLEN, ALIGNED, OTHER_COPYDIMS...>::template ConstSrc<
            OTHER_SRCDIMS...>::Accumulate_atomic(src + i * SRC_STRIDE,
                                                 dst + i * dst_stride, acc,
                                                 dst_otherdims...);
    }
  };
  ...
  struct ConstDst {
    template <typename... Args>
    static DACE_HDFI void Copy(const T *src, T *dst, const int &src_stride,
      ...
      __DACE_UNROLL
      for (int i = 0; i < COPYDIM; ++i)
        CopyND<T, VECLEN, ALIGNED, OTHER_COPYDIMS...>::template ConstDst<
            OTHER_DSTDIMS...>::Copy(src + i * src_stride, dst + i * DST_STRIDE,
                                    src_otherdims...);
    }
    ...
    static DACE_HDFI void Accumulate(const T *src, T *dst, ACCUMULATE acc,
      ...
      for (int i = 0; i < COPYDIM; ++i)
        CopyND<T, VECLEN, ALIGNED, OTHER_COPYDIMS...>::template ConstDst<
            OTHER_DSTDIMS...>::Accumulate(src + i * src_stride,
                                          dst + i * DST_STRIDE, acc,
                                          src_otherdims...);
    }
    ...
    static DACE_HDFI void Accumulate_atomic(const T *src, T *dst,
      ...
      for (int i = 0; i < COPYDIM; ++i)
        CopyND<T, VECLEN, ALIGNED, OTHER_COPYDIMS...>::template ConstDst<
            OTHER_DSTDIMS...>::Accumulate_atomic(src + i * src_stride,
                                                 dst + i * DST_STRIDE, acc,
                                                 src_otherdims...);
    }
  };
  ...
  struct Dynamic {
    ...
    static DACE_HDFI void Accumulate_atomic(const T *src, T *dst,
      ...
      __DACE_UNROLL
      for (int i = 0; i < COPYDIM; ++i)
        CopyND<T, VECLEN, ALIGNED, OTHER_COPYDIMS...>::Dynamic::
            Accumulate_atomic(src + i * src_stride, dst + i * dst_stride, acc,
                              otherdims...);
    }
  };
};
...
struct CopyND<T, VECLEN, ALIGNED> {
  ...
  struct ConstSrc {
    ...
    template <
        typename ACCUMULATE, typename... Args, typename T2 = T,
        std::enable_if_t<std::is_trivially_copyable<T2>::value, bool> = true>
    static DACE_HDFI void Accumulate(const T *src, T *dst, ACCUMULATE acc) {
      *(vec<T, VECLEN> *)dst =
          acc(*(vec<T, VECLEN> *)dst, *(vec<T, VECLEN> *)src);
    }
    ...
    static DACE_HDFI void Accumulate_atomic(const T *src, T *dst,
                                            ACCUMULATE acc) {
      wcr_custom<T>::reduce_atomic(acc, (vec<T, VECLEN> *)dst,
                                   *(vec<T, VECLEN> *)src);
    }
  ...
  struct ConstDst {
    ...
    template <
        typename ACCUMULATE, typename T2 = T,
        std::enable_if_t<std::is_trivially_copyable<T2>::value, bool> = true>
    static DACE_HDFI void Accumulate(const T *src, T *dst, ACCUMULATE acc) {
      *(vec<T, VECLEN> *)dst =
          acc(*(vec<T, VECLEN> *)dst, *(vec<T, VECLEN> *)src);
...
struct CopyNDDynamic {
  ...
  struct ConstSrc {
    template <typename... Args>
    static DACE_HDFI void Copy(const T *src, T *dst, const int &copydim,
                               const int &dst_stride,
                               const Args &...otherdims) {
#if !defined(__CUDA_ARCH__) && !defined(__HIP_DEVICE_COMPILE__)
      // Memcpy specialization
      ...
      __DACE_UNROLL
      for (int i = 0; i < copydim; ++i)
        CopyNDDynamic<T, VECLEN, ALIGNED, N - 1>::template ConstSrc<
            OTHER_SRCDIMS...>::Copy(src + i * SRC_STRIDE, dst + i * dst_stride,
                                    otherdims...);
    }
    ...
    template <typename ACCUMULATE, typename... Args>
    static DACE_HDFI void Accumulate(const T *src, T *dst, ACCUMULATE acc,
                                     const int &copydim, const int &dst_stride,
                                     const Args &...otherdims) {
      __DACE_UNROLL
      for (int i = 0; i < copydim; ++i)
        CopyNDDynamic<T, VECLEN, ALIGNED, N - 1>::template ConstSrc<
    ...
    template <typename ACCUMULATE, typename... Args>
    static DACE_HDFI void Accumulate_atomic(const T *src, T *dst,
                                            ACCUMULATE acc, const int &copydim,
                                            const int &dst_stride,
                                            const Args &...otherdims) {
      __DACE_UNROLL
      for (int i = 0; i < copydim; ++i)
  ...
  struct ConstDst {
    ...
    static DACE_HDFI void Copy(const T *src, T *dst, const int &copydim,
      ...
      __DACE_UNROLL
      for (int i = 0; i < copydim; ++i)
        CopyNDDynamic<T, VECLEN, ALIGNED, N - 1>::template ConstDst<
            OTHER_DSTDIMS...>::Copy(src + i * src_stride, dst + i * DST_STRIDE,
                                    otherdims...);
    }
    ...
    template <typename ACCUMULATE, typename... Args>
    static DACE_HDFI void Accumulate(const T *src, T *dst, ACCUMULATE acc,
                                     const int &copydim, const int &src_stride,
                                     const Args &...otherdims) {
      __DACE_UNROLL
      for (int i = 0; i < copydim; ++i)
        CopyNDDynamic<T, VECLEN, ALIGNED, N - 1>::template ConstDst<
  ...
  struct Dynamic {
    ...
    static DACE_HDFI void Copy(const T *src, T *dst, const int &copydim,
                               ...
                               const Args &...otherdims) {
      static_assert(sizeof...(otherdims) == (N - 1) * 3,
                    "Dimensionality mismatch in dynamic copy");
...
struct CopyNDDynamic<T, VECLEN, ALIGNED, 0> {
  ...
  struct ConstSrc {
    ...
    template <
        typename ACCUMULATE, typename... Arg, typename T2 = T,
        std::enable_if_t<std::is_trivially_copyable<T2>::value, bool> = true>
    static DACE_HDFI void Accumulate_atomic(const T *src, T *dst,
                                            ACCUMULATE acc) {
      wcr_custom<T>::reduce_atomic(acc, (vec<T, VECLEN> *)dst,

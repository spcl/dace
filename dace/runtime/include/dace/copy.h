...
namespace dace {
template <typename T, int VECLEN, typename std::enable_if<std::is_trivially_copyable<T>::value, bool>::type = true>
...
inline void InitArray(T* ptr, const U& value, int size) {
  for (int i = 0; i < size; ++i) *ptr++ = T(value);
}
...
template <typename T, int VECLEN, int ALIGNED, int COPYDIM, int... OTHER_COPYDIMS>
struct CopyND<T, VECLEN, ALIGNED, COPYDIM, OTHER_COPYDIMS...> {
  ...
  struct ConstSrc {
    ...
    static DACE_HDFI void Accumulate(const T* src, T* dst, ACCUMULATE acc, const int& dst_stride,
    ...
    template <typename ACCUMULATE, typename... Args>
    static DACE_HDFI void Accumulate_atomic(const T* src, T* dst, ACCUMULATE acc, const int& dst_stride,
                                            const Args&... dst_otherdims) {
      __DACE_UNROLL
      for (int i = 0; i < COPYDIM; ++i)
  ...
  struct ConstDst {
    ...
    template <typename ACCUMULATE, typename... Args>
    static DACE_HDFI void Accumulate(const T* src, T* dst, ACCUMULATE acc, const int& src_stride,
    ...
    template <typename ACCUMULATE, typename... Args>
    static DACE_HDFI void Accumulate_atomic(const T* src, T* dst, ACCUMULATE acc, const int& src_stride,
                                            const Args&... src_otherdims) {
      __DACE_UNROLL
      for (int i = 0; i < COPYDIM; ++i)
  ...
  struct Dynamic {
    template <typename... Args>
    static DACE_HDFI void Copy(const T* src, T* dst, const int& src_stride, const int& dst_stride,
      ...
      __DACE_UNROLL
      for (int i = 0; i < COPYDIM; ++i)
        CopyND<T, VECLEN, ALIGNED, OTHER_COPYDIMS...>::Dynamic::Copy(src + i * src_stride, dst + i * dst_stride,
                                                                     otherdims...);
    }

    template <typename ACCUMULATE, typename... Args>
    static DACE_HDFI void Accumulate(const T* src, T* dst, ACCUMULATE acc, const int& src_stride, const int& dst_stride,
                                     const Args&... otherdims) {
      __DACE_UNROLL
      for (int i = 0; i < COPYDIM; ++i)
        CopyND<T, VECLEN, ALIGNED, OTHER_COPYDIMS...>::Dynamic::Accumulate(src + i * src_stride, dst + i * dst_stride,
                                                                           acc, otherdims...);
    }

    template <typename ACCUMULATE, typename... Args>
    static DACE_HDFI void Accumulate_atomic(const T* src, T* dst, ACCUMULATE acc, const int& src_stride,
                                            const int& dst_stride, const Args&... otherdims) {
      __DACE_UNROLL
      for (int i = 0; i < COPYDIM; ++i)
...
struct CopyND<T, VECLEN, ALIGNED> {
  ...
  struct ConstDst {
    ...
    template <typename ACCUMULATE, typename T2 = T,
              std::enable_if_t<!std::is_trivially_copyable<T2>::value, bool> = true>
    static DACE_HDFI void Accumulate(const T* src, T* dst, ACCUMULATE acc) {
      for (int i = 0; i < VECLEN; ++i) dst[i] = acc(dst[i], src[i]);
    }
...
struct CopyNDDynamic {
  ...
  struct ConstSrc {
    template <typename... Args>
    static DACE_HDFI void Copy(const T* src, T* dst, const int& copydim, const int& dst_stride,
                               const Args&... otherdims) {
#if !defined(__CUDA_ARCH__) && !defined(__HIP_DEVICE_COMPILE__)
      // Memcpy specialization
    ...
    template <typename ACCUMULATE, typename... Args>
    static DACE_HDFI void Accumulate(const T* src, T* dst, ACCUMULATE acc, const int& copydim, const int& dst_stride,
                                     const Args&... otherdims) {
      __DACE_UNROLL
      for (int i = 0; i < copydim; ++i)
    ...
    template <typename ACCUMULATE, typename... Args>
    static DACE_HDFI void Accumulate_atomic(const T* src, T* dst, ACCUMULATE acc, const int& copydim,
                                            const int& dst_stride, const Args&... otherdims) {
      __DACE_UNROLL
      for (int i = 0; i < copydim; ++i)
  ...
  struct ConstDst {
    template <typename... Args>
    static DACE_HDFI void Copy(const T* src, T* dst, const int& copydim, const int& src_stride,
                               const Args&... otherdims) {
#if !defined(__CUDA_ARCH__) && !defined(__HIP_DEVICE_COMPILE__)
      // Memcpy specialization
      if (N == 1 && src_stride == 1 && DST_STRIDE == 1) {
    ...
    template <typename ACCUMULATE, typename... Args>
    static DACE_HDFI void Accumulate(const T* src, T* dst, ACCUMULATE acc, const int& copydim, const int& src_stride,
                                     const Args&... otherdims) {
      __DACE_UNROLL
      for (int i = 0; i < copydim; ++i)
    ...
    template <typename ACCUMULATE, typename... Args>
    static DACE_HDFI void Accumulate_atomic(const T* src, T* dst, ACCUMULATE acc, const int& copydim,
                                            const int& src_stride, const Args&... otherdims) {
      __DACE_UNROLL
      for (int i = 0; i < copydim; ++i)
  ...
  struct Dynamic {
    template <typename... Args>
    static DACE_HDFI void Copy(const T* src, T* dst, const int& copydim, const int& src_stride, const int& dst_stride,
                               const Args&... otherdims) {
      ...
      __DACE_UNROLL
      for (int i = 0; i < copydim; ++i)
        CopyNDDynamic<T, VECLEN, ALIGNED, N - 1>::Dynamic::Copy(src + i * src_stride, dst + i * dst_stride,
                                                                otherdims...);
    }
    ...
    static DACE_HDFI void Accumulate(const T* src, T* dst, ACCUMULATE acc, const int& copydim, const int& src_stride,
      ...
      __DACE_UNROLL
      for (int i = 0; i < copydim; ++i)
        CopyNDDynamic<T, VECLEN, ALIGNED, N - 1>::Dynamic::Accumulate(src + i * src_stride, dst + i * dst_stride, acc,
                                                                      otherdims...);
    }

    template <typename ACCUMULATE, typename... Args>
    static DACE_HDFI void Accumulate_atomic(const T* src, T* dst, ACCUMULATE acc, const int& copydim,
                                            const int& src_stride, const int& dst_stride, const Args&... otherdims) {
      static_assert(sizeof...(otherdims) == (N - 1) * 3, "Dimensionality mismatch in dynamic copy");
      __DACE_UNROLL
      for (int i = 0; i < copydim; ++i)
        CopyNDDynamic<T, VECLEN, ALIGNED, N - 1>::Dynamic::Accumulate_atomic(src + i * src_stride, dst + i * dst_stride,
                                                                             acc, otherdims...);
    }
  };
};
...
struct CopyNDDynamic<T, VECLEN, ALIGNED, 0> {
  ...
  struct ConstSrc {
    ...
    template <typename ACCUMULATE, typename... Arg, typename T2 = T,
              std::enable_if_t<std::is_trivially_copyable<T2>::value, bool> = true>
    static DACE_HDFI void Accumulate_atomic(const T* src, T* dst, ACCUMULATE acc) {
      wcr_custom<T>::reduce_atomic(acc, (vec<T, VECLEN>*)dst, *(vec<T, VECLEN>*)src);
    }
    ...
    static DACE_HDFI void Accumulate_atomic(const T* src, T* dst, ACCUMULATE acc, Args... args) {
      for (int i = 0; i < VECLEN; ++i) wcr_custom<T>::reduce_atomic(acc, &dst[i], src[i]);
    }
  };
  ...
  struct ConstDst {
    ...
    template <typename T2 = T, std::enable_if_t<!std::is_trivially_copyable<T2>::value, bool> = true>
    static DACE_HDFI void Copy(const T* src, T* dst) {
      for (int i = 0; i < VECLEN; ++i) dst[i] = src[i];
    }

    ...
    * ``construct_from_json``
    ...
    def decorator(cls):
        ...
        init_docstring = "__init__({})\n\n".format(
            ", ".join(name + "=" + repr(prop._default) for name, prop in cls.__properties__.items())
        )
...
@onnx_representation(
    onnx.defs.OpSchema.FormalParameter, type_str='type_str', param_type='option', homogeneous="is_homogeneous"
)
class ONNXParameter:
...
@onnx_representation(
    ...
    attributes=lambda proto: {str(k): convert_onnx_proto(v) for k, v in get_proto_attr(proto, "attributes").items()},
    type_constraints=lambda proto: {
        str(cons.type_param_str): convert_onnx_proto(cons) for cons in get_proto_attr(proto, "type_constraints")
    },
)
class ONNXSchema:

...
class PureMaxPool2D(ONNXForward):
    ...
    def forward(node: ONNXOp, state: SDFGState, sdfg: SDFG) -> typing.Union[nodes.Node, SDFG]:
        ...
        inner_image_memlet = propagation.propagate_memlet(new_state, image_memlet, inner_me, False)
        outer_image_memlet = propagation.propagate_memlet(new_state, inner_image_memlet, outer_me, False)
        ...
        # hook up outputs
        output_memlet = dace.Memlet("Y[b, c, out_x, out_y]", wcr="lambda x, y: max(x, y)")
        inner_output_memlet = propagation.propagate_memlet(new_state, output_memlet, inner_me, False)
        outer_output_memlet = propagation.propagate_memlet(new_state, inner_output_memlet, outer_me, False)
        new_state.add_edge(compute_tasklet, "output", inner_mx, None, output_memlet)

        write_Y = new_state.add_write("Y")
        new_state.add_edge_pair(outer_mx, inner_mx, write_Y, inner_output_memlet, outer_output_memlet)
...
class PureConv2D(ONNXForward):
    ...
    def forward_can_be_applied(node: ONNXOp, state: SDFGState, sdfg: SDFG) -> bool:
        ...
        if node.strides is not None and (not all(s == 1 for s in node.strides) or len(node.strides) != image_dims):
            return False
    ...
    def forward(node: ONNXOp, state: SDFGState, sdfg: SDFG) -> typing.Union[nodes.Node, SDFG]:
        ...
        @dace.program
        def broadcast(x: dtype[batch_size, num_filters, output_size_y, output_size_x], y: dtype[num_filters]):

            for b, m, out_x, out_y in dace.map[0:batch_size, 0:num_filters, 0:output_size_y, 0:output_size_x]:
                with dace.tasklet:
                    inp << y[m]
        ...
        @dace.program
        def zero_init(x: dtype[batch_size, num_filters, output_size_y, output_size_x]):
            for b, m, out_x, out_y in dace.map[0:batch_size, 0:num_filters, 0:output_size_y, 0:output_size_x]:
                with dace.tasklet:
                    outp >> x[b, m, out_x, out_y]
                    outp = 0
        ...
        if B is None:
            ...
            def conv(X, Y, W):
                zero_init(Y)
                for b, m, out_x, out_y, cin, hx, hy in dace.map[0:batch_size, 0:num_filters, 0:output_size_y,
        ...
        accessnodes_X = lambda s: [n for n in s.nodes() if isinstance(n, nodes.AccessNode) and n.data == "X"]

        compute_state = s1 if len(accessnodes_X(s1)) > len(accessnodes_X(s2)) else s2
        ...
        me = entry_nodes[0]
        for i in range(1, 4):
            me, _ = MapCollapse.apply_to(nsdfg, outer_map_entry=me, inner_map_entry=entry_nodes[i], permissive=True)
...
class PureBatchNormalization(ONNXForward):
    ...
    def forward(node: ONNXOp, state: SDFGState, sdfg: SDFG) -> typing.Union[nodes.Node, SDFG]:
        ...
        def prog(X, scale, B, in_mean, in_var, Y, out_mean, out_var, saved_mean, saved_var):
            saved_mean[:] = np.add.reduce(X, axis=axis) / N

            saved_mean_broadcastable = dace.define_local(broadcast_shape, dtype)
            ...
            saved_var[:] = np.add.reduce(X_minus_mean * X_minus_mean, axis=axis) / N
        ...
        write_mean = new_state.add_read(in_edge_with_name(node, state, "in_mean").data.data)
        new_state.add_edge(read_mean, None, write_mean, None, sdfg.make_array_memlet(mean_data_name))

        var_data_name = out_edge_with_name(node, state, "out_var").data.data
        read_var = new_state.add_read(var_data_name)
        write_var = new_state.add_read(in_edge_with_name(node, state, "in_var").data.data)
        new_state.add_edge(read_var, None, write_var, None, sdfg.make_array_memlet(var_data_name))
...
class PureGlobalAveragePool(ONNXForward):
    ...
    def forward(node: ONNXOp, state: SDFGState, sdfg: SDFG) -> typing.Union[nodes.Node, SDFG]:
        ...
        map_entry_1, map_exit_1 = new_state.add_map(name='grid_map',
                                                    ndrange={
                                                        'a': f'0:{x_data.shape[0]}',
                                                        'b': f'0:{x_data.shape[1]}'
                                                    })
        ...
        new_state.add_memlet_path(input_access,
                                  ...
                                  tasklet,
                                  dst_conn='x',
                                  memlet=dace.Memlet(data=x_name, subset="a,b,c,d"))
        ...
        new_state.add_memlet_path(writeout_tasklet,
                                  ...
                                  output_access,
                                  src_conn='out',
                                  memlet=dace.Memlet(data=y_name, subset="a,b", dynamic=True, volume=0))

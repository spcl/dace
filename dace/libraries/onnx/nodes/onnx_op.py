...
def register_op_repo_replacement(cls: Type[ONNXOp], cls_name: str, dace_schema: ONNXSchema):
...
for schema in _get_schemas_from_version(12):
    ...
    def __init__(self, name, *args, location=None, **op_attributes):
        super(ONNXOp, self).__init__(
            ...
            inputs={inp.name
                    for inp in self.schema.inputs if inp.param_type == ONNXParameterType.Single},
            outputs={out.name
                     for out in self.schema.outputs if out.param_type == ONNXParameterType.Single})
        ...
        if len(missing_arguments) > 0:
            ...
            raise TypeError(get_missing_arguments_message("__init__()", missing_arguments, "keyword-only argument"))
    ...
    class Expansion(ExpandTransformation):
        ...
        def expansion(cls, node, state: SDFGState, sdfg: SDFG):
            ...
            if not isinstance(result, SDFG):
                ...
                cls.environments = map(dace.library.get_environment, result.environments)
            return result
            ...
            class Expansion(ExpandTransformation):
                ...
                def expansion(cls, node, state, sdfg, **kwargs):
                    ...
                    if cls.forward_impl.forward_can_be_applied(node, state, sdfg):
                        ...
                        # fall back to ORT
                        log.info('Falling back to onnxruntime expansion for library node "{}". '
                                 'Reason: forward_can_be_applied returned False'.format(node.label))
                        result = expand_node(node, state, sdfg)
                        if not isinstance(result, SDFG):
                            # when we return an SDFG the the environments will be determined recursively by codegen.
                            cls.environments = map(dace.library.get_environment, result.environments)

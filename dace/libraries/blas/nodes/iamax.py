...
class Iamax(dace.sdfg.nodes.LibraryNode):
    ...
    def validate(self, sdfg, state):
        ...
        if len(in_edges) != 1 or len(out_edges) != 1:
            raise ValueError("IAMAX expects one input and one output")
        ...
        if len(squeezed.size()) != 1:
            raise ValueError("IAMAX only supported on 1-D arrays")
        stride_x = desc_x.strides[sqdims[0]]
        n = squeezed.num_elements()
        if out_edges[0].data.subset.num_elements() != 1:
            raise ValueError("Output of IAMAX must be a single element")
        return (desc_x, stride_x), desc_res, n

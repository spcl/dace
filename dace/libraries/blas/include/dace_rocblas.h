...
class _RocblasConstants {
 public:
  ...
  _RocblasConstants() {
    ...
    (void)hipMemcpy(half_pone_, &half_pone, sizeof(__half) * 1, hipMemcpyHostToDevice);
    (void)hipMalloc(&float_pone_, sizeof(float) * 1);
    float float_pone = 1.0f;
    (void)hipMemcpy(float_pone_, &float_pone, sizeof(float) * 1, hipMemcpyHostToDevice);
    (void)hipMalloc(&double_pone_, sizeof(double) * 1);
    double double_pone = 1.0;
    (void)hipMemcpy(double_pone_, &double_pone, sizeof(double) * 1, hipMemcpyHostToDevice);
    (void)hipMalloc(&complex64_pone_, sizeof(hipComplex) * 1);
    hipComplex complex64_pone = make_hipFloatComplex(1.0f, 0.0f);
    (void)hipMemcpy(complex64_pone_, &complex64_pone, sizeof(hipComplex) * 1, hipMemcpyHostToDevice);
    (void)hipMalloc(&complex128_pone_, sizeof(hipDoubleComplex) * 1);
    hipDoubleComplex complex128_pone = make_hipDoubleComplex(1.0, 0.0);
    (void)hipMemcpy(complex128_pone_, &complex128_pone, sizeof(hipDoubleComplex) * 1, hipMemcpyHostToDevice);

...
def get_arglist(module: 'dace.frontend.ml.torch.DaceModule') -> Tuple[List[str], List[str]]:
    ...
    :return: A tuple of (input_names, output_names) where each is a list of cleaned
...
def compile_and_init_sdfgs(
    ...
    :param module: The DaCe module to compile SDFGs for.
    :param dummy_inputs: The dummy inputs to use for shape inference and initialization.
    :return: If the module has no backward pass: (compiled_sdfg, state_ptr).
             If the module has a backward pass: (compiled_fwd_sdfg, fwd_state_ptr,
             compiled_bwd_sdfg, bwd_state_ptr). Where state_ptr is a torch.Tensor
             containing the pointer to the SDFG state.
    """

...
class ExpandPgemvMKL(ExpandTransformation):
    ...
    def expansion(node, parent_state, parent_sdfg, **kwargs):
        ...
        code = (f"if (!__state->__mkl_scalapack_grid_init) {{\n"
                ...
                f"MKL_INT gcols = 1;\n"
                # f"MKL_INT a_rows = (trans == 'T' ? {node._n} : {node._m});\n"
                # f"MKL_INT a_cols = (trans == 'T' ? {node._m} : {node._n});\n"
                ...
                f"MKL_INT bcols = 1;\n"
                # f"MKL_INT a_brows = (trans == 'T' ? _a_block_sizes[1] : _a_block_sizes[0]);\n"
                f"MKL_INT a_brows = _a_block_sizes[1];\n"
                # f"MKL_INT a_bcols = (trans == 'T' ? _a_block_sizes[0] : _a_block_sizes[1]);\n"
                ...
                # f"MKL_INT a_lld = (trans == 'T' ? a_mloc : a_nloc);\n"
                # f"printf(\"a_rows=%d, a_cols=%d, a_brows=%d, a_bcols=%d, a_lld=%d\\n\", a_rows, a_cols, a_brows, a_bcols, a_lld);\n"
                ...
                # f"printf(\"b_rows=%d, b_cols=%d, b_brows=%d, b_bcols=%d, b_lld=%d\\n\", b_rows, b_cols, b_mloc, b_bcols, b_lld);\n"
                ...
                # f"printf(\"c_rows=%d, c_cols=%d, c_brows=%d, c_bcols=%d, c_lld=%d\\n\", grows, gcols, mloc, bcols, c_lld);\n"

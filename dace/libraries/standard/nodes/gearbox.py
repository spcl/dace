...
class ExpandGearbox(dace.transformation.ExpandTransformation):
    ...
    def expansion(node: "Gearbox", parent_state: dace.SDFGState, parent_sdfg: dace.SDFG):
        ...
        entry, exit = state.add_map(
            node.name, {f"_{node.name}_i": f"0:{node.size}", vec_it: f"0:{gear_factor}"}, schedule=node.schedule
        )
        ...
        if is_elementwise:
            ...
            if is_pack:
                ...
                unroll_entry, unroll_exit = read_state.add_map(
                    f"{node.name}_elementwise", {elem_it: f"0:{small_veclen}"}, schedule=node.schedule, unroll=True
                )
                unroll_tasklet = read_state.add_tasklet(
                    f"{node.name}_elementwise", {"unpack_in"}, {"buffer_out"}, "buffer_out = unpack_in"
                )
                ...
                read_state.add_memlet_path(
                    ...
                    buffer_write,
                    src_conn="buffer_out",
                    memlet=dace.Memlet(f"{buffer_name_inner}[{vec_it} * {small_veclen} + {elem_it}]"),
                )
                ...
                unroll_tasklet = write_state.add_tasklet(
                    f"{node.name}_elementwise", {"buffer_in"}, {"pack_out"}, "pack_out = buffer_in"
                )
...
buffer_out = wide""",
            ...
            state.add_memlet_path(
                buffer_read, entry, tasklet, dst_conn="buffer_in", memlet=dace.Memlet(f"{buffer_name}[0]")
            )
            state.add_memlet_path(
                tasklet,
                exit,
            ...
            state.add_memlet_path(
                tasklet, exit, buffer_write, src_conn="buffer_out", memlet=dace.Memlet(f"{buffer_name}[0]")
            )
...
class Gearbox(dace.sdfg.nodes.LibraryNode):
    ...
    def validate(self, sdfg: dace.SDFG, state: dace.SDFGState):
        ...
        if isinstance(in_desc.dtype, dace.vector) and in_desc.dtype.base_type == out_desc.dtype:
            ...
            isinstance(in_desc.dtype, dace.vector)

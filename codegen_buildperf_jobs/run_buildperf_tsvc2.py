...
legacy one over the TSVC2 corpus, on the daint.alps cluster -- the tsvc2 sibling of
...
are the corpus adapter (the vendored ``tsvc2_perf`` + ``tsvc_corpus`` harness, for kernel
discovery, deterministic input allocation and sizing) and the correctness oracle (TSVC has no
numpy reference, so the LEGACY output is the reference the experimental output is compared
against, via ``engine.arrays_close``).
...
Two presets:
  S      single-core, the canonical real TSVC sizes (LEN_1D=32000 / LEN_2D=256) -- a
         quick single-core signal.
  paper  full-node threads, the ~2GB working-set sizing tsvc2_perf.py measures at.

A per-kernel build/compile/run failure is caught, recorded as an ERROR row, and the sweep
continues.

    python3 run_buildperf_tsvc2.py --preset S --codegen both --out tsvc2.tsv
    python3 run_buildperf_tsvc2.py --preset paper --only s000 --out probe.tsv
    python3 run_buildperf_tsvc2.py --list-kernels
"""
import os
...
# The tsvc harness vendored into THIS folder (tsvc2_perf, tsvc_corpus) + readability_metrics
...
CORPUS = base.CORPUS  # 'tsvc2'
DEVICE = 'cpu'        # the TSVC corpus is CPU-only; the readable comparison is CPU codegen
CODEGENS = ('legacy', 'experimental')

#: The swept extent for the quick single-core S signal, per regime: the canonical
#: real-world TSVC sizes (LEN_1D=32000 for 1d kernels, LEN_2D=256 for 2d kernels).
#: paper uses base.size_for_kernel (the ~2GB working set tsvc2_perf.py measures at).
S_SWEPT = {'1d': 32_000, '2d': 256}
...
#: (readability_metrics.FRAME_METRIC_FIELDS); `corpus` is literally "tsvc2".
TSV_FIELDS = [
...
def sizes_for(kernel, preset):
    """(LEN_1D, LEN_2D) for the preset: paper = base.size_for_kernel (the ~2GB target);
    S = the canonical real TSVC size for the kernel's regime (a quick single-core signal)."""
    if preset == 'paper':
        return base.size_for_kernel(kernel)
    return tsvc.regime_sizes(kernel.regime, S_SWEPT[kernel.regime])


def call_kwargs(arrays, sparams, sym):
    """The SDFG call dict: fresh array copies (each run mutates in place) + scalar
    parameters + LEN_* symbol values."""
    return {**{n: a.copy() for n, a in arrays.items()}, **sparams, **sym}
...
# --------------------------------------------------------------------------
def perf_job(name, l1, l2, preset, codegen, reps, timeout):
    ...
    kernel, arrays, sym, sparams = base._inputs(name, l1, l2)
    sdfg = pipelined_sdfg(kernel.program, sdfg_name(name, preset, codegen))
    codegen_ms, compile_ms = engine.compile_sdfg_timed(sdfg)
    times = engine.time_sdfg(sdfg, call_kwargs(arrays, sparams, sym), reps, time_budget_s=0.8 * timeout)
    return dict(codegen_ms=codegen_ms, compile_ms=compile_ms, runtime_ms=(min(times) if times else None))


def correctness_job(name, l1, l2, preset, want_experimental):
    ...
    kernel, arrays, sym, sparams = base._inputs(name, l1, l2)
    legacy = pipelined_sdfg(kernel.program, sdfg_name(name, preset, 'legacy'))
    legacy_call = call_kwargs(arrays, sparams, sym)
    ...
    if want_experimental:
        ...
        try:
            k2, arrays2, sym2, sparams2 = base._inputs(name, l1, l2)  # identical inputs (deterministic seed)
            experimental = pipelined_sdfg(k2.program, sdfg_name(name, preset, 'experimental'))
            exp_call = call_kwargs(arrays2, sparams2, sym2)
...
def select_kernels(only, explicit):
    """The TSVC2 corpus kernel names (from the vendored tsvc_corpus), optionally filtered
    by an explicit list or a substring."""
...
def process_kernel(writer, out_handle, name, args, threads, cxx, phase):
    ...
    corr_ok, corr = engine.run_isolated(correctness_job, (name, l1, l2, preset, 'experimental' in codegens),
                                        timeout=timeout)
    if not corr_ok:
        corr = {}
    ...
    for codegen in codegens:
        ok, payload = engine.run_isolated(perf_job, (name, l1, l2, preset, codegen, reps, timeout), timeout=timeout)
...
def main():
    ...
    ap.add_argument('--preset',
                    choices=('S', 'paper'),
                    default='S',
                    help='S = single-core, canonical TSVC sizes (quick signal); paper = full-node threads, ~2GB sizes')

...
#: The build/size/readability metrics the perf drivers embed per row (in TSV column order).
#: code_bytes is the on-disk generated-source size; the rest come from readability_score.
FRAME_METRIC_FIELDS = ('code_bytes', 'nloc', 'max_nesting', 'tokens', 'max_ccn')


def frame_source_path(build_folder, name):
    """The generated C++ frame for an SDFG named `name` built under `build_folder`.

    DaCe writes the CPU frame at ``<build_folder>/src/cpu/<name>.cpp`` (see
    dace/codegen/targets/cpu.py). With ``cache='name'`` the build_folder already ends in
    the SDFG name, so this is the single, stable source the drivers score inline."""
    return os.path.join(build_folder, 'src', 'cpu', f'{name}.cpp')


def frame_metrics(path):
    """Build/size/readability metrics for ONE generated frame, as a dict over
    FRAME_METRIC_FIELDS. Never raises: every field comes back None when the frame is
    missing or unreadable, so a driver can write blank TSV cells and keep going.

    code_bytes is the generated source's byte size (a size proxy that needs no lizard);
    nloc / max_nesting / tokens / max_ccn are exactly readability_score's, so tokens and
    max_ccn are None without lizard while nloc (its local fallback), max_nesting and
    code_bytes are always available."""
    out = {field: None for field in FRAME_METRIC_FIELDS}
    if not path or not os.path.isfile(path):
        return out
    try:
        out['code_bytes'] = os.path.getsize(path)
    except OSError:
        pass
    score = readability_score(path)
    if score.get('ok'):
        for metric in ('nloc', 'max_nesting', 'tokens', 'max_ccn'):

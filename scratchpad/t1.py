...
def setup_architecture():
    ...
    generate_arg_cfg(cluster_tcdm_size=hex(TCDM_SIZE),
                     num_cluster_x=THREAD_GROUP_DIMS[0],
                     num_cluster_y=THREAD_GROUP_DIMS[1],
...
def hbm_to_np_impl(array_name: str, interleave_handler: InterleaveHandler, element_size_in_bytes: int, dtype: str,
                   parsed_sections: Dict[str, Dict[int, List[str]]], numpy_buffer: np.ndarray):
    assert len(numpy_buffer.shape) == 2
    for i in range(numpy_buffer.shape[0]):
        for j in range(numpy_buffer.shape[1]):
            value = get_address_and_read_from_file(i=i,
                                                   j=j,
                                                   interleave_handler=interleave_handler,
...
def np_to_hbm(A_handler, M, N, K):
    ...
    make_preload_elf_hbm_interleaved_new("output.elf", [A_handler],
                                         KMN=[K, M, N],
...
def run_sdfg_in_tempdir(combo):
    ...
    compiled_sdfg(A=A_host, B=B_host, C=C_host, M=M_val, N=N_val, K=K_val)
...
def parse_dump_file(file_path: str, num_channels: int, arrays: Iterable[str]) -> Dict[str, Dict[int, List[str]]]:
...
if __name__ == "__main__":
    ...
    parsed_sections: Dict[str, Dict[int, List[str]]] = parse_dump_file(
        file_path=GVSOC_PATH + "/dump_0",
        num_channels=8,
        arrays={
            arr_name
            for arr_name, arr in sdfg.arrays.items() if arr.transient is False and isinstance(arr, dace.data.Array)
        })

    #print(parsed_sections)

    hbm_to_np_impl(array_name="A",

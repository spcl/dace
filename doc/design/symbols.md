...
Each SDFG owns its own registry. The same name in inner and outer SDFGs is
allowed and they remain independent objects. `add_nested_sdfg` propagates
free symbols via `add_symbol`, preserving the inner registry's invariants.

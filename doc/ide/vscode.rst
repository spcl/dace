...
- The entire SDFG can be brought in to view using the :code:`zoom to fit` button |zoom-to-fit-button|.
- The hierarchical nature of SDFGs allows individual hierarchies to be collapsed or folded
  to view the graph on different levels of detail. The :code:`collapse all` button |collapse-all-button|
  can be used to collapse all hierarchy levels, and the :code:`expand all` button
...
- By selecting graph elements in the main SDFG view, more information about them,
  ...
  at the same time using the :code:`box select` tool |box-select-button|, or by holding down the Shift key
  when selecting elements.
- The :code:`Go to source` button opens the source code file that generated the SDFG (if available)
  and navigates to the line(s) of code corresponding to the selected graph element.
- The :code:`Go to Generated Code` button opens source code file containing the code generated by
...
.. image:: ./images/generate_code.gif
    :width: 800

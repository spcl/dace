        ...
        for state, (reads, writes) in state_dict.items():
            ...
            for access_node in access_nodes:
                if access_node.data not in rename_dict[original_name].keys():
                    rename_dict[original_name][access_node.data] = {
                        access_node}
                else:
...
def test_scalar_write_shadow_split():
    ...
    init_state.add_edge(init_tasklet, 'out', init_write,
                        None, dace.Memlet('tmp'))
    tmp1_tasklet = loop_1_1.add_tasklet(
        'tmp1', {'a', 'b'}, {'out'}, 'out = a * b')
    ...
    loop_1_1.add_edge(tmp1_tasklet, 'out', tmp1_write,
                      None, dace.Memlet('tmp'))
    ...
    intermediate_tasklet = intermediate.add_tasklet(
        "overwrite", {}, {"t"}, "t = 0")
    intermediate.add_edge(intermediate_tasklet, "t",
                          tmp_1, None, dace.Memlet("tmp"))
    ...
    name_sets["tmp"] = [{init_write}, {
        tmp1_write, loop1_read_tmp}, {tmp_1, tmp_2}]
...
def test_scalar_write_shadow_fused():
    ...
    sdfg.add_edge(init_state, guard_1,
                  dace.InterstateEdge(assignments={'i': 0}))
    sdfg.add_edge(guard_1, loop_1, dace.InterstateEdge(
        condition='i < (N - 1)'))
    sdfg.add_edge(loop_1, guard_1, dace.InterstateEdge(
        assignments={'i': 'i + 1'}))
    sdfg.add_edge(guard_1, intermediate,
                  dace.InterstateEdge(condition='i >= (N - 1)'))
    sdfg.add_edge(intermediate, guard_2,
                  dace.InterstateEdge(assignments={'i': 0}))
    sdfg.add_edge(guard_2, loop_2, dace.InterstateEdge(
        condition='i < (N - 1)'))
    sdfg.add_edge(loop_2, guard_2, dace.InterstateEdge(
        assignments={'i': 'i + 1'}))
    sdfg.add_edge(guard_2, end_state, dace.InterstateEdge(
        condition='i >= (N - 1)'))
    init_tasklet = init_state.add_tasklet('init', {}, {'out'}, 'out = 0')
    init_write = init_state.add_write('tmp')
    init_state.add_edge(init_tasklet, 'out', init_write,
                        None, dace.Memlet('tmp'))
    tmp1_tasklet = loop_1.add_tasklet(
        'tmp1', {'a', 'b'}, {'out'}, 'out = a * b')
    loop1_tasklet_1 = loop_1.add_tasklet(
        'loop1_1', {'ap', 't'}, {'a'}, 'a = ap + 2 * t')
    loop1_tasklet_2 = loop_1.add_tasklet(
        'loop1_2', {'bp', 't'}, {'b'}, 'b = bp - 2 * t')
    ...
    loop_1.add_edge(tmp1_tasklet, 'out', tmp1_read_write,
                    None, dace.Memlet('tmp'))
    loop_1.add_edge(tmp1_read_write, None, loop1_tasklet_1,
                    't', dace.Memlet('tmp'))
    loop_1.add_edge(tmp1_read_write, None, loop1_tasklet_2,
                    't', dace.Memlet('tmp'))
    loop_1.add_edge(a1_read, None, loop1_tasklet_1,
                    'ap', dace.Memlet('A[i + 1]'))
    loop_1.add_edge(b1_read, None, loop1_tasklet_2,
                    'bp', dace.Memlet('B[i + 1]'))
    ...
    tmp2_tasklet = loop_2.add_tasklet(
        'tmp2', {'a', 'b'}, {'out'}, 'out = a / b')
    loop2_tasklet_1 = loop_2.add_tasklet(
        'loop2_1', {'ap', 't'}, {'a'}, 'a = ap + t * t')
    loop2_tasklet_2 = loop_2.add_tasklet(
        'loop2_2', {'bp', 't'}, {'b'}, 'b = bp - t * t')
    ...
    loop_2.add_edge(tmp2_tasklet, 'out', tmp2_read_write,
                    None, dace.Memlet('tmp'))
    loop_2.add_edge(tmp2_read_write, None, loop2_tasklet_1,
                    't', dace.Memlet('tmp'))
    loop_2.add_edge(tmp2_read_write, None, loop2_tasklet_2,
                    't', dace.Memlet('tmp'))
    ...
    loop_2.add_edge(loop2_tasklet_1, 'a', a2_write,
                    None, dace.Memlet('A[i + 1]'))
    loop_2.add_edge(loop2_tasklet_2, 'b', b2_write,
...
def test_scalar_write_shadow_interstate_self():
    ...
    sdfg.add_edge(intermediate, guard_2,
                  dace.InterstateEdge(assignments={'i': 0}))
    sdfg.add_edge(guard_2, loop_2_1, dace.InterstateEdge(
        condition='i < (N - 1)'))
    tmp2_edge = dace.InterstateEdge(assignments={'j': 'tmp'})
    sdfg.add_edge(loop_2_1, loop_2_2, tmp2_edge)
    sdfg.add_edge(loop_2_2, guard_2, dace.InterstateEdge(
        assignments={'i': 'i + 1'}))
    sdfg.add_edge(guard_2, end_state, dace.InterstateEdge(
        condition='i >= (N - 1)'))
    ...
    loop_1_2.add_edge(loop1_tasklet_2, 'b', loop1_write_b,
                      None, dace.Memlet('B[i]'))
    tmp2_tasklet = loop_2_1.add_tasklet(
        'tmp2', {'a', 'b'}, {'out'}, 'out = a / b')
    tmp2_write = loop_2_1.add_write('tmp')
    a2_read = loop_2_1.add_read('A')
    b2_read = loop_2_1.add_read('B')
    loop_2_1.add_edge(a2_read, None, tmp2_tasklet,
                      'a', dace.Memlet('A[i + 1]'))
    loop_2_1.add_edge(b2_read, None, tmp2_tasklet,
                      'b', dace.Memlet('B[i + 1]'))
    loop_2_1.add_edge(tmp2_tasklet, 'out', tmp2_write,
                      None, dace.Memlet('tmp'))
    loop2_tasklet_1 = loop_2_2.add_tasklet(
        'loop2_1', {'ap', 't'}, {'a'}, 'a = ap + t * t')
    loop2_tasklet_2 = loop_2_2.add_tasklet(
        'loop2_2', {'bp', 't'}, {'b'}, 'b = bp - t * t')
    ...
    loop_2_2.add_edge(loop2_read_tmp, None, loop2_tasklet_1,
                      't', dace.Memlet('tmp'))
    loop_2_2.add_edge(loop2_read_tmp, None, loop2_tasklet_2,
                      't', dace.Memlet('tmp'))
    loop_2_2.add_edge(loop2_read_a, None, loop2_tasklet_1,
                      'ap', dace.Memlet('A[i]'))
    loop_2_2.add_edge(loop2_read_b, None, loop2_tasklet_2,
                      'bp', dace.Memlet('B[i]'))
    loop_2_2.add_edge(loop2_tasklet_1, 'a', loop2_write_a,
                      None, dace.Memlet('A[i + 1]'))
    loop_2_2.add_edge(loop2_tasklet_2, 'b', loop2_write_b,
                      None, dace.Memlet('B[i + 1]'))
    ...
    try:
        ...
        assert (False)
    name_sets: Dict[str, List[Set[AccessNode]]] = {}
    name_sets['A'] = [{a1_read, a2_read, loop1_read_a,
                       loop1_write_a, loop2_read_a, loop2_write_a}]
    name_sets['B'] = [{b1_read, b2_read, loop1_read_b,
                       loop1_write_b, loop2_read_b, loop2_write_b}]
    name_sets["tmp"] = [{init_write}, {tmp1_write,
                                       loop1_read_tmp}, {tmp2_write, loop2_read_tmp}]
    access_nodes: Dict[str, Dict[SDFGState, Tuple[Set[AccessNode], Set[AccessNode]]]
                       ] = results[FindAccessNodes.__name__][sdfg.sdfg_id]
    assert_rename_sets(name_sets, access_nodes)
...
def test_scalar_write_shadow_interstate_pred():
    ...
    sdfg.add_edge(loop_1_3, guard_1, dace.InterstateEdge(
        assignments={'i': 'i + 1'}))
    sdfg.add_edge(guard_1, intermediate,
                  dace.InterstateEdge(condition='i >= (N - 1)'))
    sdfg.add_edge(intermediate, guard_2,
    ...
    sdfg.add_edge(loop_2_3, guard_2, dace.InterstateEdge(
        assignments={'i': 'i + 1'}))
    sdfg.add_edge(guard_2, end_state, dace.InterstateEdge(
        condition='i >= (N - 1)'))
    init_tasklet = init_state.add_tasklet('init', {}, {'out'}, 'out = 0')
    ...
    loop_1_1.add_edge(tmp1_tasklet, 'out', tmp1_write,
                      None, dace.Memlet('tmp'))
    loop1_tasklet_1 = loop_1_3.add_tasklet(
        'loop1_1', {'ap', 't'}, {'a'}, 'a = ap + 2 * t')
    loop1_tasklet_2 = loop_1_3.add_tasklet(
        'loop1_2', {'bp', 't'}, {'b'}, 'b = bp - 2 * t')
    ...
    loop_1_3.add_edge(loop1_read_tmp, None, loop1_tasklet_1,
                      't', dace.Memlet('tmp'))
    loop_1_3.add_edge(loop1_read_tmp, None, loop1_tasklet_2,
                      't', dace.Memlet('tmp'))
    loop_1_3.add_edge(loop1_read_a, None, loop1_tasklet_1,
                      'ap', dace.Memlet('A[i + 1]'))
    loop_1_3.add_edge(loop1_read_b, None, loop1_tasklet_2,
                      'bp', dace.Memlet('B[i + 1]'))
    loop_1_3.add_edge(loop1_tasklet_1, 'a', loop1_write_a,
                      None, dace.Memlet('A[i]'))
    loop_1_3.add_edge(loop1_tasklet_2, 'b', loop1_write_b,
                      None, dace.Memlet('B[i]'))
    tmp2_tasklet = loop_2_1.add_tasklet(
        'tmp2', {'a', 'b'}, {'out'}, 'out = a / b')
    tmp2_write = loop_2_1.add_write('tmp')
    ...
    loop_2_1.add_edge(tmp2_tasklet, 'out', tmp2_write,
                      None, dace.Memlet('tmp'))
    loop2_tasklet_1 = loop_2_3.add_tasklet(
        'loop2_1', {'ap', 't'}, {'a'}, 'a = ap + t * t')
    loop2_tasklet_2 = loop_2_3.add_tasklet(
        'loop2_2', {'bp', 't'}, {'b'}, 'b = bp - t * t')
    ...
    loop_2_3.add_edge(loop2_read_tmp, None, loop2_tasklet_1,
                      't', dace.Memlet('tmp'))
    loop_2_3.add_edge(loop2_read_tmp, None, loop2_tasklet_2,
                      't', dace.Memlet('tmp'))
    loop_2_3.add_edge(loop2_read_a, None, loop2_tasklet_1,
                      'ap', dace.Memlet('A[i]'))
    loop_2_3.add_edge(loop2_read_b, None, loop2_tasklet_2,
                      'bp', dace.Memlet('B[i]'))
    loop_2_3.add_edge(loop2_tasklet_1, 'a', loop2_write_a,
                      None, dace.Memlet('A[i + 1]'))
    loop_2_3.add_edge(loop2_tasklet_2, 'b', loop2_write_b,
                      None, dace.Memlet('B[i + 1]'))
...
def test_loop_fake_shadow_symbolic():
    ...
    loop2.add_edge(loop2_tasklet_2, 'b', loop2_access_b,
                   None, dace.Memlet('B'))
    ...
    sdfg.add_edge(loop2, guard, dace.InterstateEdge(
        assignments={'i': 'i + 1'}))
...
def test_nested_loops():
    ...
    sdfg.add_edge(guard_1, guard_0, dace.InterstateEdge(
        condition="j < N", assignments={"i": "i + 1"}))
    sdfg.add_edge(guard_0, guard_1, dace.InterstateEdge(
        condition="not (i < N)", assignments={"j": "j + 1"}))
    sdfg.add_edge(guard_1, guard_2, dace.InterstateEdge(
        condition="not(j < N)", assignments={"k": "0"}))
    sdfg.add_edge(guard_2, loop_body_1, dace.InterstateEdge(condition="k < N"))
    sdfg.add_edge(loop_body_1, guard_2, dace.InterstateEdge(
        assignments={"k": "k + 1"}))
    sdfg.add_edge(guard_2, guard_3, dace.InterstateEdge(
        condition="not(k < N)", assignments={"l": "l + 1"}))
...
def test_nested_loops2_fission():
    ...
    sdfg.add_edge(loop_body_0, guard_1, dace.InterstateEdge(
        assignments={"j": "j + 1"}))
    sdfg.add_edge(guard_1, guard_2, dace.InterstateEdge(
        condition="not(j < N)", assignments={"k": "0"}))
    sdfg.add_edge(guard_2, loop_body_1, dace.InterstateEdge(condition="k < N"))
    ...
    sdfg.add_edge(guard_2, guard_3, dace.InterstateEdge(
        condition="not(k < N)", assignments={"l": "l + 1"}))
    sdfg.add_edge(init, guard_3, dace.InterstateEdge(assignments={"l": "0"}))
    sdfg.add_edge(guard_3, guard_1, dace.InterstateEdge(
        condition="l < N", assignments={"j": "0"}))
    sdfg.add_edge(guard_3, end, dace.InterstateEdge(condition="not (l < N)",))

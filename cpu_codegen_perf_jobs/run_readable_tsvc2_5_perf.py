...
"""Benchmark the experimental *readable* C++ code generator against the legacy one
over the TSVC2.5 corpus, on the daint.alps cluster -- the tsvc2_5 sibling of
``run_readable_perf.py`` (which covers NPBench+PolyBench). Same structure, same
TSV, same crash-isolated measurement; the only differences are the corpus adapter
(the copied ``tsvc2_5_perf`` + ``tsvc_2_5_corpus`` harness, used for kernel discovery,
deterministic input allocation and per-kernel symbolic sizing) and the correctness
oracle (TSVC has no numpy reference, so the LEGACY output is the reference the
experimental output is compared against, via ``engine.arrays_close``).

The pipeline under test is identical to run_readable_perf.py's:
``dace + simplify + LoopToMap + MapFusion + (len-1 -> scalar)``
(``engine.pipeline_parallel`` + ``ConvertLengthOneArraysToScalars``); the ONLY thing
that varies between the two lanes is ``compiler.cpu.implementation`` (legacy vs
experimental). Readability is expected to be perf-neutral, so the runtime comparison
is a regression guard, not a claimed win.

Two presets:
  S      single-core, the corpus's stock (small) SIZES defaults -- a quick regression signal.
...
Per kernel x codegen it emits one TSV row with the SAME header the plot generator
reads: kernel, corpus (literally "tsvc2_5"), codegen, preset, threads, codegen_ms
(DaCe C++ codegen), compile_ms (the cxx compiler on its own), runtime_ms (best-of-N),
speedup (legacy_runtime / this_runtime), correctness, error. A per-kernel
build/compile/run failure is caught, recorded as an ERROR row, and the sweep continues.

    python3 run_readable_tsvc2_5_perf.py --preset S --codegen both --out tsvc2_5.tsv
    python3 run_readable_tsvc2_5_perf.py --preset paper --only heat3d --out probe.tsv
    python3 run_readable_tsvc2_5_perf.py --list-kernels
"""
import os

# MPI/OpenMP anti-hang defaults BEFORE importing dace / the copied harness (a dace
...
# The tsvc harness copied into THIS folder (tsvc2_5_perf, tsvc_2_5_corpus) is authoritative,
# so keep this dir ahead of the sibling on sys.path. The sibling performance_regression_jobs
# supplies the shared infra we deliberately do NOT duplicate (engine, native_harness,
# adapters); it is APPENDED so it can never shadow this dir's local copies. Both paths are
# derived from this file's own location -- no hardcoded absolute paths.
...
#: TSV columns -- the EXACT header the plot generator reads (order matters), identical
#: to run_readable_perf.py. codegen_ms (DaCe C++ code generation) and compile_ms (the cxx
#: compiler, on its own) are kept SEPARATE so a stacked plot can show each; `corpus` is
#: literally "tsvc2_5"; `threads` records the OMP width the row was measured at; `cxx` the host
#: C++ compiler the row was built with (g++ / clang++); `phase` which sweep phase produced it
#: (single_core / multi_core), so one merged TSV from submit_daint_readable.sbatch stays
#: groupable; `error` carries a failure message so an ERROR row is self-explanatory.
TSV_FIELDS = [
...
# mirrors run_readable_perf.py exactly.
# --------------------------------------------------------------------------
...
def sdfg_name(name, preset, codegen):
    ...
    return f'readable_{CORPUS}_{name}_{preset}_{codegen}'
...
def correctness_job(name, sizes, preset, want_experimental):
    ...
    via engine.arrays_close. Returns {'legacy': bool|None, 'experimental': bool|None, 'error': str}."""
    engine.configure_dace_process()
...
def select_kernels(only, explicit):
    """The TSVC2.5 corpus kernel names (bare function names, from the copied
...
def resolve_phase(explicit, threads):
    ...
    still labels itself correctly. This corpus is CPU-only -- the gpu phase never runs it."""
    if explicit:
        return explicit
...
# --------------------------------------------------------------------------
# Per-kernel driver + TSV assembly (mirrors run_readable_perf.py).
# --------------------------------------------------------------------------

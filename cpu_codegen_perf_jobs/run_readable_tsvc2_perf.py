...
over the TSVC2 corpus, on the daint.alps cluster -- the tsvc2 sibling of
...
(the copied ``tsvc2_perf`` + ``tsvc_corpus`` harness, used for kernel discovery,
deterministic input allocation and sizing) and the correctness oracle (TSVC has no
numpy reference, so the LEGACY output is the reference the experimental output is
compared against, via ``engine.arrays_close``).
...
reads: kernel, corpus (literally "tsvc2"), codegen, preset, threads, codegen_ms
...
build/compile/run failure is caught, recorded as an ERROR row, and the sweep continues.
    ...
    python3 run_readable_tsvc2_perf.py --preset S --codegen both --out tsvc2.tsv
    python3 run_readable_tsvc2_perf.py --preset paper --only s000 --out probe.tsv
    python3 run_readable_tsvc2_perf.py --list-kernels
"""
import os
...
# The tsvc harness copied into THIS folder (tsvc2_perf, tsvc_corpus) is authoritative,
...
#: literally "tsvc2"; `threads` records the OMP width the row was measured at; `cxx` the host
...
def select_kernels(only, explicit):
    """The TSVC2 corpus kernel names (from the copied tsvc_corpus), optionally filtered

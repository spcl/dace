...
def test_configuration(a_trans: bool, b_trans: bool, a_padding: int, b_padding: int, M: int, K: int,
    ...
    if a_trans:
        a_strides = (1, int(math.ceil(M / a_padding) * a_padding))
        total_a = int(a_strides[1] * K)
    else:
        a_strides = (int(math.ceil(K / a_padding) * a_padding), 1)
        total_a = int(a_strides[0] * M)

    if b_trans:
        b_strides = (1, int(math.ceil(K / b_padding) * b_padding))
        total_b = int(b_strides[1] * N)
    else:
        b_strides = (int(math.ceil(N / b_padding) * b_padding), 1)
        total_b = int(b_strides[0] * K)

...
if __name__ == '__main__':
    ...
    tuner = optim.OnTheFlyMapFusionTuner(sdfg, i=384, j=384, measurement=dace.InstrumentationType.GPU_Events)
    tuner.optimize(apply=True)

    tuner = optim.SubgraphFusionTuner(sdfg, i=384, j=384, measurement=dace.InstrumentationType.GPU_Events)
    tuner.optimize(apply=True)

    sdfg_path = Path(os.environ["HOME"]) / "projects/tuning-dace/hg-fvt-384-tuned.sdfg"
    sdfg.save(sdfg_path)

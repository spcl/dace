...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        sdfg1c = auto_optimize.auto_optimize(one_mm_compute.to_sdfg(simplify=True), device=dace.dtypes.DeviceType.GPU)
        sdfg2 = auto_optimize.auto_optimize(two_mm.to_sdfg(simplify=True), device=dace.dtypes.DeviceType.GPU)
        sdfg2c = auto_optimize.auto_optimize(two_mm_compute.to_sdfg(simplify=True), device=dace.dtypes.DeviceType.GPU)
        sdfg3 = auto_optimize.auto_optimize(three_mm.to_sdfg(simplify=True), device=dace.dtypes.DeviceType.GPU)
        sdfg3c = auto_optimize.auto_optimize(three_mm_compute.to_sdfg(simplify=True), device=dace.dtypes.DeviceType.GPU)
    ...
    if rank == 0:
        ...
        write_time(str(datetime.now()),"1mm_compute", "dace_gpu", size, (S, S, S), runtimes, file_name, field_names, append=True)
    ...
    if rank == 0:
        ...
        print(f"Median compute runtime: {np.median(runtimes)} seconds")
        write_time(str(datetime.now()),"2mm_compute", "dace_gpu", size, (S, S, S, S), runtimes, file_name, field_names, append=True)
    ...
    if rank == 0:
        ...
        print(f"Median compute runtime: {np.median(runtimes)} seconds")
        write_time(str(datetime.now()),"3mm_compute", "dace_gpu", size, (S, S, S, S, S), runtimes, file_name, field_names, append=True)

...
to_kernel = data""")
    ...
    state.add_memlet_path(mem,
                          ...
                          memlet=dace.Memlet("A_device[n0 * P + n1, k]", dynamic=True, allow_oob=True))
...
if n0 * {P} + n1 < {N}:                                               
    ...
    state.add_memlet_path(mem_read,
                          ...
                          dst_conn="prev_c",
                          memlet=dace.Memlet(
                              "C_device[n0 * P + n1, m]".format(vec_width)))

    state.add_memlet_path(tasklet,
                          exit_map,
                          mem,
                          src_conn="to_memory",
                          memlet=dace.Memlet("C_device[n0 * P + n1, m]", dynamic=True))

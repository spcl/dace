...
def make_read_val():
    ...
    a_val_mem = body.add_array("A_val_mem", (nnz, ),
                               dtype,
                               storage=StorageType.FPGA_Global)
    compute_pipe = body.add_stream("compute_pipe",
                                   dtype,
    ...
    body.add_memlet_path(a_val_mem,
                         tasklet,
                         dst_conn="a_in",
...
def make_main_state(sdfg):
    ...
    a_row = state.add_array("A_row_device", (H + 1, ),
                            ...
                            transient=True,
                            storage=StorageType.FPGA_Global)
    row_to_val_out = state.add_stream("row_to_val",
                                      itype,
                                      transient=True,
                                      storage=StorageType.FPGA_Local)
    row_to_col_out = state.add_stream("row_to_col",
                                      itype,
                                      transient=True,
                                      storage=StorageType.FPGA_Local)
    row_to_x_out = state.add_stream("row_to_x",
                                    itype,
                                    transient=True,
                                    storage=StorageType.FPGA_Local)
    row_to_compute_out = state.add_stream("row_to_compute",
                                          itype,
                                          transient=True,
                                          storage=StorageType.FPGA_Local)
    ...
    state.add_memlet_path(read_row_tasklet,
                          row_to_val_out,
                          memlet=dace.memlet.Memlet.simple(row_to_val_out,
                                                           "0",
                                                           num_accesses=-1),
                          src_conn="to_val_pipe")
    state.add_memlet_path(read_row_tasklet,
                          row_to_col_out,
                          memlet=dace.memlet.Memlet.simple(row_to_col_out,
                                                           "0",
                                                           num_accesses=-1),
                          src_conn="to_col_pipe")
    state.add_memlet_path(read_row_tasklet,
                          row_to_x_out,
                          memlet=dace.memlet.Memlet.simple(row_to_x_out,
                                                           "0",
                                                           num_accesses=-1),
                          src_conn="to_x_pipe")
    state.add_memlet_path(read_row_tasklet,
                          row_to_compute_out,
                          memlet=dace.memlet.Memlet.simple(row_to_compute_out,
                                                           "0",
                                                           num_accesses=-1),
    ...
    a_col = state.add_array("A_col_device", (nnz, ),
                            ...
                            transient=True,
                            storage=StorageType.FPGA_Global)
    row_to_col_in = state.add_stream("row_to_col",
                                     itype,
                                     transient=True,
                                     storage=StorageType.FPGA_Local)
    col_to_x_out = state.add_stream("col_to_x",
                                    itype,
                                    transient=True,
                                    storage=StorageType.FPGA_Local)
    read_col_sdfg = make_read_col()
    read_col_tasklet = state.add_nested_sdfg(read_col_sdfg, sdfg,
                                             {"A_col_mem", "row_pipe"},
                                             {"col_pipe"})
    ...
    state.add_memlet_path(row_to_col_in,
                          read_col_tasklet,
                          memlet=dace.memlet.Memlet.simple(row_to_col_in,
                                                           "0",
                                                           num_accesses=-1),
                          dst_conn="row_pipe")
    state.add_memlet_path(read_col_tasklet,
                          col_to_x_out,
                          memlet=dace.memlet.Memlet.simple(col_to_x_out,
                                                           "0",
                                                           num_accesses=-1),
                          src_conn="col_pipe")
    ...
    x = state.add_array("x_device", (W, ),
                        ...
                        transient=True,
                        storage=StorageType.FPGA_Global)
    row_to_x_in = state.add_stream("row_to_x",
                                   itype,
                                   transient=True,
                                   storage=StorageType.FPGA_Local)
    col_to_x_in = state.add_stream("col_to_x",
                                   itype,
                                   transient=True,
                                   storage=StorageType.FPGA_Local)
    x_to_compute_out = state.add_stream("x_to_compute",
                                        dtype,
                                        transient=True,
                                        storage=StorageType.FPGA_Local)
    read_x_sdfg = make_read_x()
    read_x_tasklet = state.add_nested_sdfg(read_x_sdfg, sdfg,
                                           {"x_mem", "col_pipe", "row_pipe"},
                                           {"compute_pipe"})
    ...
    state.add_memlet_path(col_to_x_in,
                          read_x_tasklet,
                          dst_conn="col_pipe",
                          memlet=dace.memlet.Memlet.simple(col_to_x_in,
                                                           "0",
                                                           num_accesses=-1))
    state.add_memlet_path(row_to_x_in,
                          read_x_tasklet,
                          dst_conn="row_pipe",
                          memlet=dace.memlet.Memlet.simple(row_to_x_in,
                                                           "0",
                                                           num_accesses=-1))
    state.add_memlet_path(read_x_tasklet,
                          x_to_compute_out,
                          src_conn="compute_pipe",
                          memlet=dace.memlet.Memlet.simple(x_to_compute_out,
                                                           "0",
                                                           num_accesses=-1))
    ...
    val_to_compute_in = state.add_stream("val_to_compute",
                                         ...
                                         transient=True,
                                         storage=StorageType.FPGA_Local)
    x_to_compute_in = state.add_stream("x_to_compute",
                                       dtype,
                                       transient=True,
                                       storage=StorageType.FPGA_Local)
    result_to_write_out = state.add_stream("result_to_write",
                                           dtype,
                                           transient=True,
                                           storage=StorageType.FPGA_Local)
    compute_sdfg = make_compute_sdfg()
    compute_tasklet = state.add_nested_sdfg(compute_sdfg, sdfg,
                                            {"row_pipe", "a_pipe", "x_pipe"},
                                            {"b_pipe"})
    ...
    state.add_memlet_path(val_to_compute_in,
                          compute_tasklet,
                          dst_conn="a_pipe",
                          memlet=dace.memlet.Memlet.simple(val_to_compute_in,
                                                           "0",
                                                           num_accesses=-1))
    state.add_memlet_path(x_to_compute_in,
                          compute_tasklet,
                          dst_conn="x_pipe",
                          memlet=dace.memlet.Memlet.simple(x_to_compute_in,
                                                           "0",
                                                           num_accesses=-1))
    state.add_memlet_path(compute_tasklet,
                          result_to_write_out,
                          src_conn="b_pipe",
                          memlet=dace.memlet.Memlet.simple(result_to_write_out,
                                                           "0",
                                                           num_accesses=-1))

    # Write back values of b
    result_to_write_in = state.add_stream("result_to_write",
                                          dtype,
                                          transient=True,
                                          storage=StorageType.FPGA_Local)
    ...
    state.add_memlet_path(result_to_write_in,
                          write_tasklet,
                          dst_conn="b_pipe",
                          memlet=dace.memlet.Memlet.simple(result_to_write_in,
                                                           "0",
                                                           num_accesses=-1))

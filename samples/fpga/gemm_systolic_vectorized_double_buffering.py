...
def make_compute(sdfg, state, vec_width=1, double=False):
    ...
    # Instantiate buffer for A and initialize it
    # This is done before entering the main computational map, to fill up the pipeline
    sdfg.add_array("A_reg",
                   shape=(2, ),
                   dtype=dace.float32,
                   transient=True,
                   storage=dace.dtypes.StorageType.FPGA_Registers)
    init_A = state.add_access("A_reg")
    init_entry, init_exit = state.add_map(
        "init",
        {
            "n1": "0:P"  # TODO: we can use less iteration here
        },
        schedule=dace.ScheduleType.FPGA_Device)
    init_tasklet = state.add_tasklet(
        "init_A", {"a_in", "a_top_in"}, {"to_buffer", "to_next"}, """\
val = a_in.pop() if p > 0 else a_top_in.pop()
if n1 == P - p - 1:
    to_buffer = val
if p < P - 1:
    to_next = val""")
    ...
    state.add_memlet_path(init_tasklet,
                          ...
                          init_A,
                          src_conn="to_buffer",
                          memlet=dace.Memlet(f"A_reg[0]", dynamic=True))
    state.add_memlet_path(init_tasklet,
                          init_exit,
                          A_pipe_out,
                          src_conn="to_next",
    ...
    entry_m, exit_m = state.add_map(
        "m", {"m": "0:M/{}".format((vec_width // 2) if double else vec_width)},
        schedule=dace.ScheduleType.FPGA_Device)
    ...
    state.add_memlet_path(init_A, entry_n0, memlet=dace.Memlet())

    update_A = state.add_write("A_reg")

    # For C result we are going to use vectorized data type
    sdfg.add_array("C_buffer",
                   [M / ((vec_width // 2) if double else vec_width)],
                   dtype=vec_type,
                   transient=True,
    ...
    # Buffer the element of A that will be used in the next iteration of the map over k
    # every PE: reads input data, buffer the data assigned to it, forwards the data
    # We should read P times and if we are not working on the last tile of A.
    # This is stored on (k+1)%2
    buffer_a_tasklet = state.add_tasklet(
        "double_buffer_a", {"a_in", "a_top_in"}, {"a_reg", "a_out"}, """\
if (n0 < N/P - 1 or k < K -1)  and m < P:
    ...
    state.add_memlet_path(buffer_a_tasklet,
                          update_A,
                          memlet=dace.Memlet("A_reg[(k+1)%2]", dynamic=True),
                          src_conn="a_reg")
    state.add_memlet_path(buffer_a_tasklet,
                          exit_m,
                          exit_k,
    ...
    compute_tasklet = state.add_tasklet(
        "multiply_add", {"a_in", "b_in", "b_top_in", "c_in"},
        {"b_out", "c_out"}, """\
c_prev = c_in
if k == 0:
    c_prev = 0
...
if p < P - 1:
    ...
    state.add_memlet_path(
        ...
        compute_tasklet,
        dst_conn="a_in",
        memlet=dace.Memlet("A_reg[k % 2]"))  # use the buffered data
...
if n1 <= p:
    ...
    compute_entry, compute_exit = state.add_map(
        ...
        schedule=(dace.ScheduleType.FPGA_Double
                  if double else dace.ScheduleType.FPGA_Device),
        unroll=True)
    ...
    state.add_memlet_path(compute_entry,
                          B_pipe_top_in,
                          memlet=dace.memlet.Memlet())
    state.add_memlet_path(C_pipe_top_out,
                          compute_exit,
                          memlet=dace.memlet.Memlet())

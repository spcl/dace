...
if __name__ == "__main__":
    ...
    image = np.ndarray(shape=(BATCH_SIZE, IMAGE_SIZE, IMAGE_SIZE,
                              NUM_CHANNELS),
                       dtype=np.float32)
    ...
    conv1_weights = tf.Variable(
        tf.random_normal([5, 5, NUM_CHANNELS, 32],
                         stddev=0.1,
                         seed=SEED,
                         dtype=tf.float32))
    ...
    softmax = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=label_node,
                                                             logits=logits)

...
| `lever2_memcpy_idiom.cpp` | per-row `memcpy` idiom vs an inlined copy | **REFUTED here** (0.86–1.08×, inside the floor) though the idiom demonstrably fires. It measured 2.6× on another box, so it is microarch/glibc-specific — kept precisely so it can be re-run elsewhere. **This is the codegen axis behind heat_3d + `InsertExplicitCopies`:** `dace::CopyND<…>::Copy(…)` bottoms out in `memcpy` (`runtime/include/dace/copy.h`), so replacing it with the `=` operator (an inlined assignment loop) is exactly memcpy-vs-inline. `InsertExplicitCopies` also fissions the copy into its own loop, but *loop fission/fusion is a semantic SDFG transform, not codegen* — the only codegen (pure-syntax) component is this memcpy↔`=` choice. Kernel-level heat_3d was 0.790× (1.27× faster with `=`), sign-consistent with this lever being microarch-dependent |

    ...
    for i, j in dace.map[0:N:16, 0:N:16]:  # Thread-block map
        for _ in dace.map[0:32]:  # Warp map
            ...
            for k in range(0, N, 16):
                atile = dace.ndarray(
                    [16, 16], dtype=dace.float16, storage=dace.StorageType.TensorCore_A
                )
                btile = dace.ndarray(
                    [16, 16], dtype=dace.float16, storage=dace.StorageType.TensorCore_B
                )
...
if __name__ == "__main__":
    ...
    A = np.random.rand(2048, 2048).astype(np.float16)
    B = np.random.rand(2048, 2048).astype(np.float16)
    C = np.random.rand(2048, 2048).astype(np.float32)

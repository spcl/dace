...
def print_memlets_of_map_node(node: Union[nodes.MapEntry, nodes.MapExit], state: SDFGState):
    ...
    for inedge in state.in_edges(node):
        if isinstance(state.parent.data(inedge.data.data), Array):
...
def main():
    ...
    map_entry, map_exit, map_state = None, None, None
    for node, state in sdfg.all_nodes_recursive():
        if isinstance(node, nodes.MapEntry) and node.label == args.map:
            map_entry = node
            map_state = state
        if isinstance(node, nodes.MapExit) and node.label == args.map:
            map_exit = node

    print("*** Map Entry ***")
    print_memlets_of_map_node(map_entry, map_state)
    print("*** Map Exit ***")
    print_memlets_of_map_node(map_exit, map_state)

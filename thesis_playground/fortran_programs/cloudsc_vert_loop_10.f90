...
SUBROUTINE inner_loops(&
    ...
    DO JK=NCLDTOP,KLEV
        ...
        DO JL=KIDIA,KFDIA   ! LOOP CLASS 3
            ...
            IF((LDCUM(JL) == 1).AND.PLUDE(JL,JK) > RLMIN.AND.PLU(JL,JK+1)> ZEPSEC) THEN
                ! ZCONVSRCE(JL,NCLDQL) = ZALFAW*PLUDE(JL,JK)
                ! ZCONVSRCE(JL,NCLDQI) = (1.0 - ZALFAW)*PLUDE(JL,JK)
                ZCONVSRCE(JL,4) = ZALFAW*PLUDE(JL,JK)
                ZCONVSRCE(JL,3) = (1.0 - ZALFAW)*PLUDE(JL,JK)
                ZSOLQA(JL,NCLDQL,NCLDQL) = ZSOLQA(JL,NCLDQL,NCLDQL)+ZCONVSRCE(JL,4)
                ZSOLQA(JL,NCLDQI,NCLDQI) = ZSOLQA(JL,NCLDQI,NCLDQI)+ZCONVSRCE(JL,3)
            ELSE
            ...
            IF (LDCUM(JL)==1) ZSOLQA(JL,NCLDQS,NCLDQS) = ZSOLQA(JL,NCLDQS,NCLDQS) + PSNDE(JL,JK)*ZDTGDP(JL)

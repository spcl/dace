...
# fast-canon) on TSVC2, distributed over nodes * ntasks-per-node ranks total.
#
...
# Both self-partition kernels by SLURM_PROCID/SLURM_NTASKS and write into
# <results-dir>/tsvc2/<kernel>/<compiler>_<host>_default/; the final
# single-process --tables-only passes are the cross-rank aggregation step (see
# engine.write_tables / write_compile_tables). If you have already run the
# runtime sweep, drop the first srun line and keep only the compile one.
#
# Same isolation/timing/crash handling as tsvc2_perf.py -- this reuses its
# sizing/native/correctness machinery directly (see example_slurm_tsvc2.sh for
# the full rationale).
#
# Submit with:  sbatch example_slurm_tsvc2_compile.sh
...
# --cxx <name-or-abs-path> pins the compiler for DaCe's own codegen (default:
# clang++ on PATH, else g++); the SAME compiler is used to build the SDFG in
# both the runtime and the compile-speed lanes, so the two metrics are
# comparable. --cpu-bind=cores keeps each rank pinned to its own cores.
srun --cpu-bind=cores python3 tsvc2_perf.py --reps 100 --cxx=clang++
srun --cpu-bind=cores python3 tsvc2_compile_perf.py --compile-reps 5 --cxx=clang++

...
for CXX in $CXXES; do
  ...
  srun --cpu-bind=verbose,cores --distribution=block:block python3 tsvc2_5_perf.py --reps "$REPS" --cxx="$CXX" || echo "[compile] runtime sweep failed for $CXX"
  srun --cpu-bind=verbose,cores --distribution=block:block python3 tsvc2_5_compile_perf.py --compile-reps "$COMPILE_REPS" --cxx="$CXX" || echo "[compile] compile sweep failed for $CXX"
done

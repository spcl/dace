...
#
# Sizing --ntasks-per-node: TSVC2 has 151 kernels x 14 lanes each (6 DaCe +
# 8 native -- 4 vendors x serial/autopar, though a vendor with no compiler
# installed just skips its 2 lanes); more ranks per node means more
...
# By default DaCe's own C++ codegen uses the latest LLVM/clang++ found on
# PATH, falling back to the latest GCC/g++ -- add --cxx <name-or-abs-path>
# below to pin a specific compiler instead (e.g. --cxx g++-13 or
# --cxx /opt/llvm-18/bin/clang++). DaCe-lane results are namespaced by
# compiler+hostname (see engine.result_tag), so mixing compilers/nodes in one
# --results-dir never corrupts a resumed sweep.
#
...
# compiler on PATH (or, for Intel oneAPI/NVIDIA HPC SDK, their standard
# /opt install locations) -- --cxx does not affect these lanes at all. A
# vendor with no compiler installed is just skipped for that lane; native
# results are namespaced only by hostname (engine.native_result_tag), so
# they aren't needlessly re-measured every time --cxx changes.

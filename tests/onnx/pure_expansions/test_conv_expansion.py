...
@pytest.mark.parametrize(
    "num_in_channels, kernel_size, num_filters, bias",
...
def test_conv_simple(num_in_channels, kernel_size, num_filters, bias):
    ...
    if bias:
        ...
        @dace.program
        def conv(
            X_: dace.float32[tuple(X.shape)], W_: dace.float32[tuple(W.shape)], Z_: dace.float32[tuple(torch_Z.shape)]
        ):

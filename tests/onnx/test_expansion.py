...
from dace.libraries.onnx.nodes.onnx_op import ONNXAdd, ONNXConv

import torch

def test_add():
    sdfg = dace.SDFG("test_expansion")
    sdfg.add_array("X_arr", (100, 90), dace.float32)
    sdfg.add_array("W_arr", (100, 90), dace.float32)
    sdfg.add_array("Z_arr", (100, 90), dace.float32)
    ...
    state.add_node(c)
    state.add_edge(access_X, None, c, "A", sdfg.get_array_memlet("X_arr"))
    state.add_edge(access_W, None, c, "B", sdfg.get_array_memlet("W_arr"))
    state.add_edge(c, "C", access_Z, None, sdfg.get_array_memlet("Z_arr"))

    X = np.random.rand(100, 90).astype(np.float32)
    W = np.random.rand(100, 90).astype(np.float32)

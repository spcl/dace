...
def make_sdfg() -> dace.SDFG:
    ...
    for target, rng in [("t1", "0:10"), ("t2", "0:10"), ("b", "0:1")]:
        exit_node = state.add_mapped_tasklet(
            f"comp_{target}",
            map_ranges={"__i": rng},
...
def apply_pair(sdfg: dace.SDFG, order_by_transformation: bool) -> tuple[int, int]:
...
def test_order_by_transformation_applies_the_same_matches():
    ordered, unordered = make_sdfg(), make_sdfg()

    applied_ordered, enumerations_ordered = apply_pair(ordered, True)
    applied_unordered, enumerations_unordered = apply_pair(unordered, False)

...
def test_struct_to_container_group(container_variant: str):
    ...
    # Ensure interface version is valid (cant pass strucs from python frontend)
    interface_sdfg = copy.deepcopy(sdfg)
    StructToContainerGroups(
        ...
        verbose=False,
    ).apply_pass(interface_sdfg, {})
    interface_sdfg.validate()
    interface_sdfg.compile()

    # Ensure non interface is valid and numerically verifies
    StructToContainerGroups(
        flattening_mode=ContainerGroupFlatteningMode.StructOfArrays,
        simplify=True,
        validate=True,

...
def test_map_with_tasklets(l: str = 'Python'):
    sdfg = _make_sdfg(l)
    strict_reduced = [
        xf for xf in strict_transformations()
        if xf.__name__ != 'SimpleTaskletFusion'
    ]
    sdfg.apply_transformations_repeated(strict_reduced, strict=True)
    sdfg.save('pre_tfusion.sdfg')
    sdfg.apply_transformations_repeated(SimpleTaskletFusion)
    sdfg.save(f'post_tfusion.sdfg')
    func = sdfg.compile()

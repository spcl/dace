...
def test_move_if_into_loop_basic():
    ...
    for cval in (1, 0):
        ...
        loops = _loops(sdfg)
        assert len(loops) == 1
        assert any(isinstance(n_, ConditionalBlock)
                   for n_ in loops[0].all_control_flow_regions(recursive=True))
...
def test_move_if_into_loop_nested_guards_cascade():
    ...
    for cval, dval in ((1, 2.0), (0, 2.0), (1, 50.0), (0, 50.0)):
        ref = np.full(n, 4.0)
        copy.deepcopy(base)(a=a.copy(), b=ref, c=np.array([cval], np.int32),
                            d=np.array([dval], np.float64), N=n)
        ...
        inner_conds = [r for r in loops[0].all_control_flow_regions(recursive=True)
                       if isinstance(r, ConditionalBlock)]
        assert len(inner_conds) >= 2
        assert not any(isinstance(r, ConditionalBlock)
                       for r in sdfg.nodes() if isinstance(r, ConditionalBlock))

        out = np.full(n, 4.0)
        sdfg(a=a.copy(), b=out, c=np.array([cval], np.int32),
             d=np.array([dval], np.float64), N=n)

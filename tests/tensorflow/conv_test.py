...
inp_shape = [3, 3, 10, 10]
filter_shape = [5, 5, 3, 11]
strides = [1, 1, 1, 1]

inp = tf.placeholder(tf.float32, inp_shape)
filter = tf.placeholder(tf.float32, filter_shape)
outp = tf.nn.conv2d(inp, filter, strides, padding="VALID", data_format="NCHW")

test_in = np.random.uniform(size=inp_shape).astype(np.float32)
test_filter = np.random.uniform(size=filter_shape).astype(np.float32)

sess_dace = TFSession()
sess_tf = tf.Session(config=tf.ConfigProto())
...
for p in paddings:
    for f in filters:
        for s in strides:
            ...
            outp = tf.nn.conv2d(inp, filter, s, padding=p, data_format="NCHW")
            out_backprop = tf.placeholder(tf.float32, outp.shape)
            inp_gradients = gen_nn_ops.conv2d_backprop_input(
                inp_shape, filter, out_backprop, s, padding=p, data_format="NCHW")
...
for p in paddings:
    for f in filters:
        for s in strides:
            ...
            filter = tf.placeholder(tf.float32, f)
            outp = tf.nn.conv2d(input_placeholder, filter, s, padding=p, data_format="NCHW")
            out_backprop = tf.placeholder(tf.float32, outp.shape)
            filter_gradients = gen_nn_ops.conv2d_backprop_filter(
                input_placeholder, f, out_backprop, s, padding=p, data_format="NCHW")
            test_grads = np.random.uniform(size=outp.shape).astype(np.float32)
            #test_grads = np.full(outp.shape, 0.01).astype(np.float32)
            test_input = np.random.uniform(size=tuple(inp_shape)).astype(
                np.float32)
            #test_input = np.full(inp_shape, 2).astype(np.float32)
            ...
            try:
                ...
                print(output_tf)
                print(output_dace)
                print('diff: ',output_tf - output_dace)
                print(
                    tf.linalg.norm(output_tf -
                                   output_dace).eval(session=sess_tf))

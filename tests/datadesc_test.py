...
def test_total_elements():
    assert dace.float64[2, 3].total_elements == 6
    assert dace.data.Scalar(dace.float64).total_elements == 1
    assert dace.data.Stream(dace.int32, 4, shape=(5, )).total_elements == 5
    ...
    assert padded.total_elements == 40

    N = dace.symbol('N')
    assert dace.data.Array(dace.float64, [N, 3]).total_elements == 3 * N

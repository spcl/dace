...
def _construct_graph(tbsize_1=None, tbsize_2=None) -> dace.SDFG:
    ...
    sdfg.add_transient(
        'tmp', dims, dace.float64, storage=dace.StorageType.GPU_Global, lifetime=dace.AllocationLifetime.SDFG
    )
    ...
    ime1, imx1 = state.add_map(
        'kernel_a',
        dict(j='0:%s' % (size if tbsize_1 is None else int_ceil(size, tbsize_1))),
        dace.ScheduleType.GPU_Device,
    )
    ime2, imx2 = state.add_map(
        'kernel_b',
        dict(j='0:%s' % (size if tbsize_2 is None else int_ceil(size, tbsize_2))),
        dace.ScheduleType.GPU_Device,
    )
    ...
    if tbsize_1 is not None:
        ...
        state.add_memlet_path(
            tasklet1, tbmx1, imx1, tmp, src_conn='t', memlet=dace.Memlet.simple('tmp', f'i, j*{tbsize_1} + k')
        )
    else:
        state.add_memlet_path(
            a, fme, ime1, tasklet1, dst_conn='a', memlet=dace.Memlet.simple('A', f'i, {size} - 1 - j')
        )
    ...
    if tbsize_2 is not None:
        ...
        state.add_memlet_path(
            tasklet2, tbmx2, imx2, fmx, b, src_conn='b', memlet=dace.Memlet.simple('B', f'i, j*{tbsize_2} + k')
        )
    else:

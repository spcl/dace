...
def memlet_check_parameters(memlet, volume, dynamic, subsets):
    if memlet.volume != volume:
        raise RuntimeError('Expected volume of {}, got {}'.format(
    ...
    if len(subsets) != memlet.subset.dims():
        raise RuntimeError('Expected subset of dim {}, got {}'.format(
            len(subsets), memlet.subset.dims()))

    for i in range(len(subsets)):
        if subsets[i] != memlet.subset.ranges[i]:
            raise RuntimeError('Expected subset {} at dim {}, got {}'.format(
                subsets[i], i, memlet.subset.ranges[i]))


def state_check_executions(state, expected, expected_dynamic=False):
    if state.executions != expected:
        raise RuntimeError('Expected {} executions, got {}'.format(
...
def make_sdfg():
    ...
    a_in = state.add_array('A_in', [N],
                           dace.int32,
                           storage=StorageType.FPGA_Global)
    bound_pipe = state.add_stream('bound_in',
                                  dace.int32,
                                  transient=True,
                                  storage=StorageType.FPGA_Local)
    out_stream = state.add_stream('out_stream',
                                  dace.int32,
                                  transient=True,
                                  storage=StorageType.FPGA_Local)
    ...
    state.add_memlet_path(bound_pipe,
                          nest,
                          dst_conn='IN_bound',
                          memlet=Memlet.simple(bound_pipe, '0',
                                               num_accesses=-1))

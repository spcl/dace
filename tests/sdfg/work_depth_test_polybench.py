...
def kernel_module_name(stem: str) -> str:
    """PolyBench stems are not importable names: dashes become underscores and a leading digit
    takes a ``k`` prefix (``2mm`` -> ``k2mm``)."""
    name = stem.replace('-', '_')
    return 'k' + name if name[0].isdigit() else name


def _load_kernel(stem: str):
    """The kernel's ``dace.program``, resolved through the corpus collector (the kernels live in
    category subpackages, so there is no flat path to import by)."""
    kernel = polybench.collect(kernel_module_name(stem))[0]
    return vars(importlib.import_module(kernel.modpath))[_KERNEL_FUNCS[stem]]

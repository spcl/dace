...
def tuple_retval_sdfg() -> dace.SDFG:
    ...
    for iout, ops in enumerate(["+", "-"]):
        tskl = state.add_tasklet(
            "work", inputs={"__in0", "__in1"}, outputs={"__out"}, code=f"__out0 = __in0 {ops} __in1"
        )
        for isrc, src in enumerate(anames):
            state.add_edge(acnodes[src], None, tskl, f"__in{isrc}", dace.Memlet.simple(src, "0"))
        state.add_edge(
            tskl, "__out", state.add_write(f"__return_{iout}"), None, dace.Memlet.simple(f"__return_{iout}", "0")
        )
    return sdfg
...
def test_scalar_return_validation():
    ...
    sdfg = single_retval_sdfg()
    with pytest.raises(InvalidSDFGError, match='Can not use scalar "__return" as return value.'):
        sdfg.validate()

    sdfg = tuple_retval_sdfg()
    with pytest.raises(InvalidSDFGError, match='Can not use scalar "__return_(0|1)" as return value.'):
        sdfg.validate()

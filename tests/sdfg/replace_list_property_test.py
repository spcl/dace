...
import dace
from dace.sdfg.replace import replace_list_property_item
...
def test_bools_in_an_int_list_survive():
    ...
    for value in (False, True):
        result = replace_list_property_item(value, int, repl, symrepl)
...
def test_code_fragments_in_a_string_list_survive():
    ...
    assert replace_list_property_item(field, str, repl, symrepl) == field
...
def test_tuples_in_an_object_list_survive():
    ...
    repl, symrepl = _repl()
    node = (0, 1)
    result = replace_list_property_item(node, tuple, repl, symrepl)
    assert result == (0, 1)
...
def test_names_in_a_string_list_are_replaced():
    ...
    repl, symrepl = _repl()
    assert replace_list_property_item('i', str, repl, symrepl) == 'j'
    assert replace_list_property_item('k', str, repl, symrepl) == 'k'
...
def test_non_identifier_replacements_are_skipped():
    ...
    assert replace_list_property_item('i', str, repl, symrepl) == 'i'


def test_symbolic_lists_are_replaced():
    """``Array.offset`` is a ``ListProperty(sp.Basic)`` and must still be substituted into."""
    repl, symrepl = _repl()
    assert replace_list_property_item(sp.Symbol('N'), sp.Basic, repl, symrepl) == sp.Symbol('M')

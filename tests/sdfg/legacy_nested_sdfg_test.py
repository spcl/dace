...
def test_inline_rejects_window():
    """InlineSDFG does not apply to a nested SDFG that was not converted."""
    sdfg = _windowed_sdfg()
    num_sdfgs = len(list(sdfg.all_sdfgs_recursive()))
    assert sdfg.apply_transformations_repeated(InlineSDFG, validate=False) == 0
    assert len(list(sdfg.all_sdfgs_recursive())) == num_sdfgs

...
def unlaunched(monkeypatch):
    """Drop the launcher rank variables the surrounding environment exports.

    Cache-related configuration is pinned per test through ``set_temporary``;
    the environment only seeds configuration defaults at load time.
    """
...
def test_distaware_gives_each_rank_its_own_cache_root(unlaunched, rank_var):
    ...
    with dace.config.set_temporary('cache_distaware', value=True):
...
def test_ranks_do_not_share_a_build_folder_by_default(unlaunched, tmp_path):
    ...
    # Pin the leaf naming policy so the exact path holds
    with dace.config.set_temporary('default_build_folder', value=str(tmp_path)), \
         dace.config.set_temporary('cache', value='name'):
        unlaunched.setenv('OMPI_COMM_WORLD_RANK', '0')
        rank0 = sdfg.build_folder
        unlaunched.setenv('OMPI_COMM_WORLD_RANK', '1')
...
def test_distributed_compile_puts_every_rank_in_rank_0_folder(unlaunched, tmp_path):
    ...
    with dace.config.set_temporary('default_build_folder', value=str(tmp_path)), \
         dace.config.set_temporary('cache_distaware', value=True):
        unlaunched.setenv('OMPI_COMM_WORLD_RANK', '0')

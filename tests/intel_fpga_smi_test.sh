...
run_jacobi_2d() {
    ...
    #First start the receiver and then the sender
    echo -e "${YELLOW}Running receiver...${NC}"

    $PYTHON_BINARY ${TESTS_DIR}/${TEST_NAME}_receiver.py &
    receiver_pid=$!
    sleep 5;
    echo -e "${YELLOW}Running sender...${NC}"
    echo "$PYTHON_BINARY ${TESTS_DIR}/${TEST_NAME}_sender.py &"
    $PYTHON_BINARY ${TESTS_DIR}/${TEST_NAME}_sender.py &
    wait $receiver_pid
...
run_all() {
    ...
    # Jacobi 3d stencil generated by stencilflow (evaluates only if it compiles and run not correctness)
    #run_jacobi_2d  #Temporaly disabled, requires stencilflow to run
}

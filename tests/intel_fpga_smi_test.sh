...
run_sample0() {
    ...
    echo -e "\n" | $PYTHON_BINARY ${TESTS_DIR}/${TEST_NAME}_sender.py
    echo -e "\n" | $PYTHON_BINARY ${TESTS_DIR}/${TEST_NAME}_receiver.py

    #2: compile both for emulation
    # Sender
    cd .dacecache/${TEST_NAME}_sender/build
    make intelfpga_smi_compile_${TEST_NAME}_sender_emulator -j4 
    if [ $? -ne 0 ]; then
    ...
    #First start the receiver and then the sender
    echo -e "\n" | $PYTHON_BINARY ${TESTS_DIR}/${TEST_NAME}_receiver.py &
    receiver_pid=$!
    sleep 1;
    echo -e "\n" | $PYTHON_BINARY ${TESTS_DIR}/${TEST_NAME}_sender.py &

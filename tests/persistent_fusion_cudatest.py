...
def init_scalar(state, node, value):
    tasklet = state.add_tasklet(
        'set_%s' % node.data,
        dict(),
        dict(out=dace.Memlet.simple(node.data, '0')),
        '''
out = %d
        ''' % value
    )
...
s_init.add_memlet_path(
    root_in, frontier_out,
    memlet=dace.Memlet.simple(root_in.data, '0', other_subset_str='0')
)
...
s_init.add_memlet_path(
    root_in, map_entry, tasklet,
    dst_conn='root_idx',
    memlet=dace.Memlet.simple(root_in.data, '0')
)

s_init.add_memlet_path(
    tasklet, map_exit, result_out,
    src_conn='result_out',
...
def fill_update_state(state, front_in, front_in_count, front_out, front_out_count, s_frontier_io,
    ...
    front_enter, front_exit = state.add_map(
        'frontier_map',
        dict(x='0:count_val')
    )

    state.add_memlet_path(
        front_in_count, front_enter,
        dst_conn='count_val',
...
index_end = row[f_x + 1]
    ...
    neigh_enter, neigh_exit = state.add_map(
        'neighbor_map',
        dict(i='map_start:map_end')
    )

    state.add_memlet_path(
        t_find_range, temp_ids_io,
        src_conn='index_start',
        memlet=dace.Memlet.simple(temp_ids_io.data, '0')
    )

    state.add_memlet_path(
        t_find_range, temp_ide_io,
        src_conn='index_end',
        memlet=dace.Memlet.simple(temp_ide_io.data, '0')
    )

    state.add_memlet_path(
        temp_ids_io, neigh_enter,
        dst_conn='map_start',
        memlet=dace.Memlet.simple(temp_ids_io.data, '0')
    )

    state.add_memlet_path(
        temp_ide_io, neigh_enter,
        dst_conn='map_end',
        memlet=dace.Memlet.simple(temp_ide_io.data, '0')
    )

    state.add_memlet_path(
        row_index_in, front_enter, t_find_range,
        dst_conn='row',
        memlet=dace.Memlet.simple(row_index_in.data, '0:N', num_accesses=2)
    )

    state.add_memlet_path(
        front_in, front_enter, t_find_range,
        dst_conn='f_x',

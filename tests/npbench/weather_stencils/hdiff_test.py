...
def hdiff_jax_kernel(in_field, out_field, coeff, S):
...
def run_hdiff_autodiff():
    ...
    @dc.program
    def autodiff_kernel(in_field: dc.float64[I + 4, J + 4, K], out_field: dc.float64[I, J, K], coeff: dc.float64[I, J, K]):
        hdiff_kernel(in_field, out_field, coeff)
        return np.sum(out_field)
    
    # Add the backward pass to the SDFG
    sdfg = autodiff_kernel.to_sdfg()
    add_backward_pass(sdfg=sdfg, inputs=["in_field"], outputs=["__return"], autooptimize=False)
    sdfg(in_field, out_field, coeff, I=I, J=J, K=K, gradient_in_field=gradient_in_field, gradient___return=gradient___return)
    ...
    in_field_jax = in_field.astype(np.float64)
    out_field_jax = out_field.astype(np.float64)
    coeff_jax = coeff.astype(np.float64)
    S_jax = S.astype(np.float64)
    jax_grad_in_field = jax_grad(in_field_jax, out_field_jax, coeff_jax, S_jax)
    np.testing.assert_allclose(gradient_in_field, jax_grad_in_field, rtol=1e-5, atol=1e-8)

...
def test_ufunc_add_where():
    ...
    if not np.all(
        W
    ):  # If all of W is True, np.logical_not(W) would result in empty arrays
...
@dace.program
def ufunc_add_outer_where2(
    A: dace.int32[2, 2, 2, 2, 2], B: dace.int32[2, 2, 2, 2, 2], W: dace.bool_[2, 1, 2]
):
    return np.add.outer(A, B, where=W)

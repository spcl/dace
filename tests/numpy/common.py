        ...
        def test():
            ...
            def get_rand_arr(ddesc):
                ...
                if ddesc.dtype in [dace.float16, dace.float32, dace.float64]:
                    ...
                    res = rng.random(ddesc.shape).astype(getattr(np, ddesc.dtype.to_string())) + 1j * rng.random(
                        ddesc.shape
                    ).astype(getattr(np, ddesc.dtype.to_string()))
                    b = 0 if positive else -max_value
                    a = max_value
                    res = (b - a) * res + a
                    if non_zero:
            ...
            contextmgr = (
                pytest.warns(match="divide by zero encountered") if expect_div_by_zero else contextlib.nullcontext()
            )

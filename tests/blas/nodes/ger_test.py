...
def fpga_graph_column(veclen, n_tile, m_tile, precision, vendor, testCase="0"):
    ...
    res_stream = streaming.StreamWriteMatrixFull('r',
                                                 m,
                                                 n,
                                                 rowTile,
                                                 colTile,
    ...
    preState, postState = streaming.fpga_setup_connect_streamers(
        test_sdfg, test_state, ger_node, [x_stream, y_stream, A_stream],
        ['_x', '_y', '_A'], ger_node, [res_stream], ['_res'])
...
def fpga_graph_array_column(veclen,
                            n_tile,
                            m_tile,
                            precision,
                            vendor,
    ...
    copy_in_state.add_memlet_path(in_host_y,
                                  ...
                                  memlet=Memlet.simple(
                                      in_host_y, "0:{}".format(n / veclen)))
    copy_in_state.add_memlet_path(in_host_A,
                                  in_device_A,
                                  memlet=Memlet.simple(
                                      in_host_A, "0:{}".format(n * m / veclen)))
    ...
    fpga_state.add_memlet_path(A,
                               ...
                               dst_conn="_A",
                               memlet=Memlet.simple(
                                   A, "0:{}".format(n * m / veclen)))
    fpga_state.add_memlet_path(ger_node,
                               z,
                               src_conn="_res",
                               memlet=Memlet.simple(
                                   z, "0:{}".format(n * m / veclen)))
...
if __name__ == "__main__":
    ...
    if args.target == "pure":
        ...
        # Test streaming
        sdfg = fpga_graph_column(veclen, m_tile, n_tile, dace.float32,
                                 args.target, "0")
        run_test(sdfg.compile(), args.target)
        ...
        # Test streaming
        sdfg = fpga_graph_column(veclen, n_tile, m_tile, dace.float32,
                                 args.target, "0")
        run_test(sdfg.compile(), args.target)

        # Test array based
        sdfg = fpga_graph_array_column(veclen, n_tile, m_tile, dace.float32,
                                       args.target, "0")
        run_test(sdfg.compile(), args.target)

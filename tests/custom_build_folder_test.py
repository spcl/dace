...
def test_every_cache_mode_builds_under_the_rank_root(unlaunched, cache_mode):
    ...
    with dace.config.set_temporary('cache', value=cache_mode):
        ...
        unlaunched.setenv('DACE_cache_distaware', '0')

        assert sdfg.build_folder != ranked


def test_turning_distaware_off_puts_every_rank_in_one_folder(unlaunched):
    """The escape hatch has to work: one shared folder, for whoever arranges that a single rank builds."""
    sdfg = dace.SDFG('rankprobe')
    unlaunched.setenv('DACE_cache_distaware', '0')
...
@pytest.mark.parametrize('cache_mode', ['name', 'hash', 'unique', 'single'])
def test_no_launcher_build_folder_is_byte_identical_to_distaware_off(unlaunched, cache_mode):
    """Guards the on-by-default flip: every existing .dacecache path has to survive it untouched."""
    sdfg = dace.SDFG('rankprobe')

    with dace.config.set_temporary('cache', value=cache_mode):
        on = sdfg.build_folder
        unlaunched.setenv('DACE_cache_distaware', '0')

        assert sdfg.build_folder == on
...
def test_distributed_compile_puts_every_rank_in_rank_0_folder(unlaunched, tmp_path):
    ...
    built = customprog.to_sdfg()
    csdfg = sdfg_utils.distributed_compile(built, builder)
    built_in = built.build_folder
    ...
    sdfg = customprog.to_sdfg()
    assert sdfg.build_folder != built_in, "rank 1 was looking in rank 0's folder regardless"

    csdfg = sdfg_utils.distributed_compile(sdfg, loader)

    assert sdfg.build_folder == built_in
    del csdfg

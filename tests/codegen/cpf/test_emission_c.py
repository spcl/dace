...
"""End-to-end CPF rendering in C: SDFG in, self-contained C23 out, same numbers.

Every kernel here is the same shape as its counterpart in ``test_emission.py`` -- the patterns that
decide which printer, which allocation path and which lowering-table entry CPF reaches -- but
...
def render_c(program, name: str, simplify: bool = True):
    ...
    The SDFG returned is the PREPARED one CPF rendered: expanding a library node can introduce an
    extent symbol, which makes the two argument lists differ (see ``dace.codegen.cpf.Rendering``).
    """
...
def run_c(sdfg, code, arguments, name: str):
    """Build ``code`` as C and invoke it on ``arguments`` (in place, as CPF's entry point writes)."""
...
def test_matrix_multiply_renders_as_loops():
    ...
    for banned in ('cblas', 'dgemm', 'MKL', 'BLAS'):
        assert banned not in executable, f'CPF rendered a BLAS call ({banned})'
...
def test_rendering_is_deterministic():
    """The same SDFG renders to the same C text twice; CPF output exists to be read and diffed."""
...
def test_gpu_schedules_are_refused_with_a_reason():
    """CPF renders one host unit in either language."""
...
def test_language_selects_the_dialect(language):
    """``dace.cpf`` and ``render`` take the same argument, and ``'c++'`` stays the default."""
    sdfg = c_scale_add.to_sdfg(simplify=True)
    sdfg.name = 'mprc_language_%s' % ('cxx' if language == 'c++' else 'c')
    assert cpf(sdfg, language=language) == render_sdfg(sdfg, language=language).code
    if language == 'c++':
        assert cpf(sdfg) == cpf(sdfg, language='c++'), 'the default must not have moved'
...
_HELPER_PROBE = """
#include <stdint.h>
static inline int64_t cpf_probe_max_l(int64_t a, int64_t b) {{ return (a > b) ? a : b; }}
#define probe_max(a, b) _Generic((a) + (b), long: cpf_probe_max_l)(a, b)
{helper}
...
def test_index_helper_parses_its_arguments(call, expected):
    ...
    library = ctypes.CDLL(compile_standalone(code, 'cpf_idx_probe', language='c'))
...
def test_index_helper_computes_in_int64():
    ...
    library = ctypes.CDLL(compile_standalone(code, 'cpf_idx_width', language='c'))
    out = np.zeros(1, dtype=np.int64)
    library.probe.argtypes = [ctypes.c_void_p]
    library.probe.restype = None
    library.probe(ctypes.c_void_p(out.ctypes.data))
...
def test_a_c_minmax_resolution_folds_through_the_typed_helper_and_keeps_the_accumulator(operation):
    ...
    rendering = render_sdfg(wcr_sdfg(name, f'lambda p, q: {operation}(p, q)', length=6), language='c')
    sdfg, code = rendering.sdfg, rendering.code
...
def test_the_c_gate_rejects_each_cpp_construct(label, line):
    ...
    with pytest.raises(RuntimeError, match='not self-contained'):
        cpf_module.verify(line + '\n', 'probe', cpf_lowering.Dialect.STANDALONE_C)


#: What the C gate must NOT reject. A gate with a false positive gets disabled, so the two shapes
#: that look like a banned pattern are pinned here: ``constexpr`` on an OBJECT is how CPF emits an
...
def test_the_cpp_gate_still_allows_the_cpp_constructs():
    ...
    for _, line in LEAKS:
        cpf_module.verify(line + '\n', 'probe', cpf_lowering.Dialect.STANDALONE)
...
def test_thread_local_storage_allocates_with_aligned_alloc():
    ...
    """
    rendering = render_sdfg(thread_local_sdfg('cpf_c_threadlocal'), validate=False, language='c')
    ...
    assert_standalone(rendering.code, 'cpf_c_threadlocal', language='c')

    cpp = render_sdfg(thread_local_sdfg('cpf_cpp_threadlocal'), validate=False).code
    assert 'new (std::align_val_t(64))' in cpp, 'the C++ allocation changed shape'

    a = np.random.rand(64)
    b = np.zeros(64)
    call_standalone(build_standalone(rendering.code, 'cpf_c_threadlocal', language='c'), rendering.sdfg, {
        'a': a,
        'b': b
    })
...
def test_custom_conflict_resolution_needs_no_lambda_in_c():
    ...
    :func:`tests.codegen.cpf.test_emission.test_non_conflicting_wcr_renders_without_the_reduction_runtime`
    ...
    """
    rendering = render_sdfg(wcr_sdfg('cpf_c_resolve', 'lambda p, q: p * q + 1.0'), language='c')
...
def test_out_parameter_ufuncs_call_the_typed_helper_and_compute_what_numpy_does():
    ...
    address, since C has no references and the unit carries no dispatch macro. numpy is the oracle for
    which output receives which part."""

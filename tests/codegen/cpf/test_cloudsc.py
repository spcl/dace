...
"""CPF against a whole application: CloudSC.
...
Why a whole application is a different test from the sum of the small ones. A refusal CPF should
...
value here is coverage of CPF's REFUSALS as much as of its output: whatever CloudSC contains,
...
stored baseline: CPF's contract is stated relative to the SDFG, so the SDFG is the oracle. Both
...
#: Flags added to the CPF build so it matches the reference's floating-point regime. The reference

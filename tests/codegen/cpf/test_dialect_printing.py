...
def test_helpers_used_finds_calls_and_ignores_definitions(dialect):
    ...
    ceil, modulus = ('int_ceil', 'c_mod') if dialect is Dialect.STANDALONE else ('cpf_int_ceil_int64',
                                                                                 'cpf_c_mod_int32')
    assert cpf_lowering.helpers_used('y = %s(a, b) + %s(c, d);' % (ceil, modulus), dialect) == {ceil, modulus}
    assert cpf_lowering.helpers_used('y = my_c_mod(c, d) + a.c_mod(e);', dialect) == set()
...
def test_helpers_used_drives_the_definitions_a_unit_needs(dialect):
    ...
    code = printed(sympy.Mod(X, Y), dialect)

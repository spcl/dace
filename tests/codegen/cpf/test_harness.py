...
"""Self-test for the CPF acceptance harness.

The harness in ``conftest.py`` is the gate every later CPF phase is judged by, so it is verified
first and on its own terms -- against hand-written C++ shaped exactly like the output CPF is
...
The C++ below is written by hand, not generated: it is the specification of CPF's output shape --
...
import dace
from tests.codegen.cpf.conftest import (BANNED_PATTERNS, UNQUALIFIED_RUNTIME_FUNCTIONS, assert_matches,
                                        assert_standalone, build_standalone, call_standalone, compile_diagnostics,
                                        compile_standalone, entry_argtypes, host_compiler)

#: Hand-written stand-in for CPF output: ``B[i] = A[i] * 2 + N`` over a symbolic extent, in the
...
AXPY_MPR = textwrap.dedent("""
    ...
    extern "C" void cpf_axpy(double * __restrict__ A, double * __restrict__ B, int32_t N)
    {
...
#: The WCR form CPF is allowed to emit: an OpenMP ``reduction(op:var)`` clause, never an atomic
#: through a ``dace::wcr_fixed`` helper.
SUM_MPR = textwrap.dedent("""
    #include <cstdint>

    extern "C" void cpf_sum(double * __restrict__ A, double * __restrict__ out, int32_t N)
    {
        double acc = 0.0;
...
def axpy_sdfg():
    """SDFG whose arglist defines ``cpf_axpy``'s signature: arrays ``A``, ``B``, then symbol ``N``."""
    N = dace.symbol('N')
    sdfg = dace.SDFG('cpf_axpy')
...
#: the unqualified DaCe runtime function it lands on. Each of these needs an CPF mapping.
UNQUALIFIED_LEAKS = [
...
def test_unqualified_runtime_call_is_rejected(snippet, name):
    ...
    ``pyinterop.h`` / ``ITE.h``, which CPF does not include. Without this check a leak would
...
def test_emitted_definition_is_not_a_leak():
    """CPF emitting its OWN ``int_ceil`` is the fix, not a violation -- the check must allow it."""
    ...
    assert compile_diagnostics(code, name='cpf_intceil') == ''
...
def test_measured_leaks_are_all_listed():
    ...
    assert not uncovered, (f'measured unqualified runtime leaks {uncovered} are missing from '
                           'UNQUALIFIED_RUNTIME_FUNCTIONS, so CPF output carrying them would pass the gate')
...
def test_mpr_shaped_code_is_standalone(code):
    """The specification snippets pass the string gate -- CPF's target shape is reachable."""
    assert_standalone(code)
...
def test_dace_include_fails_the_bare_build():
    ...
    """
    leaked = '#include <dace/dace.h>\nextern "C" void cpf_leak() {}\n'
    with pytest.raises(AssertionError) as excinfo:
        compile_standalone(leaked, name='cpf_leak')
...
def test_entry_argtypes_follow_the_sdfg_arglist():
    ...
    sdfg = axpy_sdfg()
    assert list(sdfg.arglist()) == ['A', 'B', 'N'], (f'arglist order changed: {list(sdfg.arglist())}; the CPF entry '
...
def test_standalone_call_writes_in_place():
    """A built CPF-shaped kernel runs through ctypes and its writes land in the caller's array."""
    sdfg = axpy_sdfg()
    library = build_standalone(AXPY_MPR, name='cpf_axpy')
    n = 64
    a = np.arange(n, dtype=np.float64)
    b = np.zeros(n, dtype=np.float64)
    call_standalone(library, sdfg, {'A': a, 'B': b, 'N': n})
    assert_matches({'B': a * 2.0 + n}, {'B': b}, label='cpf_axpy')


def sum_sdfg():
    """SDFG whose arglist defines ``cpf_sum``'s signature, with a WCR accumulation into ``out``.
...
def test_reduction_clause_kernel_runs():
    """The only WCR form CPF may emit -- an OpenMP reduction clause -- builds bare and is correct."""
    sdfg = sum_sdfg()
    assert list(sdfg.arglist()) == ['A', 'out', 'N'], (f'arglist order changed: {list(sdfg.arglist())}; the CPF '
                                                       'entry point signature is defined by it')
    library = build_standalone(SUM_MPR, name='cpf_sum')
    n = 1024
    a = np.random.default_rng(0).random(n)
    out = np.zeros(1, dtype=np.float64)
    call_standalone(library, sdfg, {'A': a, 'out': out, 'N': n})
    assert_matches({'out': np.array([a.sum()])}, {'out': out}, label='cpf_sum')


@pytest.mark.parametrize('code,name', [(AXPY_MPR, 'cpf_axpy'), (SUM_MPR, 'cpf_sum')], ids=['axpy', 'sum'])
def test_mpr_shaped_code_compiles_without_warnings(code, name):
    ...
    assert diagnostics == '', f'{name}: CPF-shaped code produced compiler warnings\n{diagnostics}'

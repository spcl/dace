...
def make_sdfg() -> dace.SDFG:
    ...
    for name in "BC":
        sdfg.add_array(name=name, dtype=dace.float64, shape=(N, N), strides=(N, 1), transient=False)
    ...
    sdfg.add_array(name="A", dtype=dace.float64, shape=(N,), strides=(second_stride_A,), transient=False)
    ...
    sdfg.add_array(name="D", dtype=dace.float64, shape=(N, N), strides=(second_stride_D, 1), transient=False)

    # Simplest way to generate a mapped Tasklet, we will later modify it.
    state.add_mapped_tasklet(
        "computation",
        map_ranges={"__i0": "0:N", "__i1": "0:N"},
        inputs={"__in0": dace.Memlet("A[__i1]"), "__in1": dace.Memlet("B[__i0, __i1]")},
    ...
    state.add_edge(tmp_in, None, iedge.dst, iedge.dst_conn, dace.Memlet(f"{tmp_in.data}[0]"))
    ...
    state.add_edge(oedge.src, oedge.src_conn, tmp_out, None, dace.Memlet(data="tmp_out", subset="0"))
    state.add_edge(tmp_out, None, oedge.dst, oedge.dst_conn, dace.Memlet(data="C", subset="__i0, __i1"))

    # Now we create a new output that uses `tmp_out` but goes into `D`.
    #  The memlet on the inside will not use `D` but `tmp_out`.
    state.add_edge(
        tmp_out, None, oedge.dst, "IN_D", dace.Memlet(data=tmp_out.data, subset="0", other_subset="__i1, __i0")
    )
    state.add_edge(
        oedge.dst, "OUT_D", state.add_access("D"), None, dace.Memlet(data="D", subset="__i0, __i1", other_subset="0")
    )

...
def test_nest_state_subgraph_wcr_placement():
    ...
    helper must emit the WCR on the new private-AccessNode -> MapExit edge, NOT on
    the NSDFG -> AccessNode edge. Otherwise codegen drops the reduction.

    Build a Map+tasklet+WCR pattern, nest its body via the helper, verify the WCR
    sits on the right edge structurally AND that the result is still numerically
    correct.
    """

...
"""Anchor tests for the :class:`EOShift` library node.

``EOSHIFT`` does not appear in any current target Fortran workload,
so the lib node's pure expansion is a stub that raises
``NotImplementedError``.  These tests exercise many shape / shift /
boundary / dim combinations of the construction path.  Implementing
the expansion later will produce a clean test signal across all of
them.
"""
import pytest
...
def _build(in_shape, dtype, *, dim=1, shift=None, boundary=None):
    """Wire an EOShift lib node into a fresh SDFG with full-array memlets.

    :param in_shape: source array shape (also the output shape).
    :param dtype: element type.
    :param dim: Fortran 1-based shift axis.
    :param shift: ``None`` means the runtime symbol ``__shift``.
    :param boundary: ``None`` means the SymbolicProperty default (0).
    :returns: the constructed (unexpanded) SDFG.
    """
    label = f"eoshift_dim{dim}_{'_'.join(map(str, in_shape))}"
...
def test_eoshift_construct_validates_runtime_shift(shape, dim):
...
@pytest.mark.parametrize("shape,dim", _SHAPE_DIM_CASES)
def test_eoshift_construct_validates_compile_time_shift(shape, dim):
...
def test_eoshift_construct_handles_various_shift_magnitudes(shift):
    """Negative, zero, and large shifts all construct cleanly."""
    sdfg = _build((16, ), dace.float64, dim=1, shift=shift)
    sdfg.validate()


@pytest.mark.parametrize("boundary", [None, 0, 1, -1, 99])
def test_eoshift_construct_handles_various_boundaries(boundary):
    """Boundary values (default + explicit) all construct cleanly."""
    sdfg = _build((16, ), dace.float64, dim=1, shift=2, boundary=boundary)
    sdfg.validate()


@pytest.mark.parametrize("dtype", [dace.int32, dace.int64, dace.float32, dace.float64])
def test_eoshift_construct_validates_various_dtypes(dtype):
    """Lib node is dtype-agnostic at construct time."""
    sdfg = _build((8, 8), dtype, dim=1)
    sdfg.validate()


def test_eoshift_validate_rejects_out_of_range_dim():
    """``dim`` outside ``[1, rank]`` raises."""
    bad = dace.SDFG("eoshift_bad_dim")
    bad.add_array("v", [4, 4], dace.float64)
    bad.add_array("out", [4, 4], dace.float64)
    state = bad.add_state()
    node = EOShift("eoshift", dim=5)
    state.add_node(node)
...
def test_eoshift_validate_rejects_mismatched_shapes():
    """Input and output must carry the same shape."""
    bad = dace.SDFG("eoshift_shape_mismatch")
    bad.add_array("v", [4, 4], dace.float64)
    bad.add_array("out", [4, 5], dace.float64)
    state = bad.add_state()
    node = EOShift("eoshift", dim=1)
    state.add_node(node)
    state.add_edge(state.add_read("v"), None, node, "_x", dace.Memlet.from_array("v", bad.arrays["v"]))
    state.add_edge(node, "_out", state.add_write("out"), None, dace.Memlet.from_array("out", bad.arrays["out"]))
...
def test_eoshift_pure_expansion_raises_until_implemented():
    """The stub expansion refuses lowering with a clear message."""
    sdfg = _build((5, ), dace.float64, dim=1)
    with pytest.raises(NotImplementedError, match="EOShift pure expansion is not yet implemented"):

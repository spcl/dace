...
def _get_sdfg(implementation, gpu=True) -> dace.SDFG:
    ...
    libnode = MemsetLibraryNode(name="memset1", inputs={}, outputs={"_mset_out"})
    if implementation is not None:
        libnode.implementation = implementation

    # Only set a slice
    state.add_edge(libnode, "_mset_out", b1, None, dace.memlet.Memlet(f"{name}[50:100]"))
...
def _get_multi_dim_sdfg(implementation, gpu=True) -> dace.SDFG:
    ...
    libnode = MemsetLibraryNode(name="copy2", inputs={}, outputs={"_mset_out"})
    if implementation is not None:
        libnode.implementation = implementation

    # Only set a slice
    state.add_edge(libnode, "_mset_out", b1, None, dace.memlet.Memlet(f"{name}[40:50, 0:2, 0:2]"))

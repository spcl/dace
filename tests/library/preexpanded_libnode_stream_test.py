...
def _build_h2d_d2h_pre_expanded_sdfg():
    ...
    state.add_node(d2h)
    state.add_edge(a, None, h2d, '_in', dace.Memlet('host_in[0:16]'))
    state.add_edge(h2d, '_out', d, None, dace.Memlet('dev[0:16]'))
    state.add_edge(d, None, d2h, '_in', dace.Memlet('dev[0:16]'))
    state.add_edge(d2h, '_out', b, None, dace.Memlet('host_out[0:16]'))

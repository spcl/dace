...
def test_tile_binop_symbol_symbol_broadcasts(widths):
    ...
    sdfg = dace.SDFG(f"tile_binop_symsym_{'x'.join(str(w) for w in widths)}")
    sdfg.add_symbol("N", dace.float64)
    sdfg.add_symbol("M", dace.float64)
    sdfg.add_array("C", widths, dace.float64, transient=False)
    state = sdfg.add_state("main")
    node = TileBinop(name="tb_ss", widths=widths, op="+", kind_a="Symbol", kind_b="Symbol", expr_a="N", expr_b="M")
    state.add_node(node)
    full = ",".join(f"0:{w}" for w in widths)
    state.add_edge(node, "_c", state.add_access("C"), None, dace.Memlet(f"C[{full}]"))
    sdfg.expand_library_nodes()
    sdfg.validate()
    C = np.zeros(widths)
    sdfg(C=C, N=3.0, M=4.0)
    np.testing.assert_allclose(C, 7.0, rtol=0, atol=0)
...
def test_tile_binop_scalar_symbol_broadcasts():
    ...
    sdfg.add_array("C", widths, dace.float64, transient=False)
    state = sdfg.add_state("main")
    node = TileBinop(name="tb_cs", widths=widths, op="+", kind_a="Scalar", kind_b="Symbol", expr_b="M")
    state.add_node(node)
    state.add_edge(state.add_access("s"), None, node, "_a", dace.Memlet("s"))
    state.add_edge(node, "_c", state.add_access("C"), None, dace.Memlet("C[0:8]"))
    sdfg.expand_library_nodes()
    sdfg.validate()
    C = np.zeros(widths)
    sdfg(C=C, s=2.0, M=5.0)
    np.testing.assert_allclose(C, 7.0, rtol=0, atol=0)

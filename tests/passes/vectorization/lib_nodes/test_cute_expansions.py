...
def _expand_cute_tasklet(lib_node):
    """Return the raw tasklet from the lib node's ``cute`` expansion."""
    sdfg = dace.SDFG(f"cute_smoke_{lib_node.label}")
    state = sdfg.add_state("main")
    state.add_node(lib_node)
    cls = lib_node.implementations["cute"]
...
def _expand_merge_cute_with_dtype(lib_node, out_dtype):
    """Expand a :class:`TileMerge` ``cute`` body with a wired ``_o`` output of
    ``out_dtype`` (the fallback path reads the output dtype off the edge)."""
    sdfg = dace.SDFG(f"cute_merge_{lib_node.label}")
    state = sdfg.add_state("main")
...
def test_tile_load_cute_pos_inf_pad_mode():
    """``pad_mode='POS_INF'`` selects ``ct.PaddingMode.POSITIVE_INFINITY``."""
    body, _ = _expand_cute(TileLoad(name="L", widths=(8, ), pad_mode="POS_INF"))
    _assert_parses_as_python(body)
    assert "padding_mode=ct.PaddingMode.POSITIVE_INFINITY" in body
    assert "ct.PaddingMode.ZERO" not in body


def test_tile_load_cute_masked_drops_dangling_mask_input():
    ...
    tasklet = _expand_cute_tasklet(TileLoad(name="L", widths=(8, ), has_mask=True))
...
def test_tile_binop_cute_symbol_operand_inlines_expr():
    ...
    body, _ = _expand_cute(TileBinop(name="B", widths=(8, ), op="+", kind_b="Symbol", expr_b="alpha"))
    _assert_parses_as_python(body)
...
def test_tile_mask_gen_cute_1d_uses_arange_and_bid():
    ...
    body, _ = _expand_cute(TileMaskGen(name="M", widths=(8, ), iter_vars=("i", ), global_ubs=("N_ub", )))
    _assert_parses_as_python(body)
    assert "__pid0 = ct.bid(0)" in body
    assert "ct.arange(8, dtype=ct.int32)" in body
    assert "__pid0 * 8" in body
...
def test_tile_mask_gen_cute_K2_combines_per_dim_via_broadcast_and_amp():
    ...
    body, _ = _expand_cute(TileMaskGen(name="M", widths=(4, 8), iter_vars=("i", "j"), global_ubs=("M_ub", "N_ub")))
    _assert_parses_as_python(body)
...
def test_tile_gather_cute_1d_unmasked_emits_padding_value():
    """1D unmasked gather: single index tile + ``padding_value=0`` (no mask)."""
    body, lang = _expand_cute(TileGather(name="G", widths=(8, )))
    _assert_parses_as_python(body)
...
def test_tile_gather_cute_1d_masked_emits_mask_and_padding_value():
    """1D masked gather: ``mask=__mask, padding_value=0``."""
    body, _ = _expand_cute(TileGather(name="G", widths=(8, ), has_mask=True))
    _assert_parses_as_python(body)
    assert "ct.gather(__src, __idx_0, mask=__mask, padding_value=0)" in body


def test_tile_gather_cute_2d_masked_uses_index_tuple():
    """2D-source masked gather uses the ``(__idx_0, __idx_1)`` tuple form."""
    body, _ = _expand_cute(TileGather(name="G", widths=(8, ), source_ndim=2, has_mask=True))
    _assert_parses_as_python(body)
    assert "ct.gather(__src, (__idx_0, __idx_1), mask=__mask, padding_value=0)" in body


def test_tile_gather_cute_pad_value_emitted():
    ...
    body, _ = _expand_cute(TileGather(name="G", widths=(8, ), pad_value=1))
    _assert_parses_as_python(body)
    assert "padding_value=1" in body


def test_tile_gather_cute_nonunit_index_strides_raises():
    """Non-unit ``index_strides`` cannot be expressed by ``ct.gather`` (no
    per-lane stride) → ``NotImplementedError`` at expansion time."""
    with pytest.raises(NotImplementedError):
        _expand_cute(TileGather(name="G", widths=(8, ), index_strides=(2, )))


def test_tile_reduce_cute_unmasked_full_and_axis():
    """Unmasked reduction emits ``ct.sum(__src)`` / ``ct.sum(__src, axis=1)``."""
    full, _ = _expand_cute(TileReduce(name="R", widths=(8, ), op="+"))
    _assert_parses_as_python(full)
    assert "__output = ct.sum(__src)" in full

    axed, _ = _expand_cute(TileReduce(name="R", widths=(4, 8), op="+", axis=1))
    _assert_parses_as_python(axed)
    assert "ct.sum(__src, axis=1)" in axed


def test_tile_reduce_cute_unmasked_max_uses_ct_max():
    """``op='max'`` routes to ``ct.max``."""
    body, _ = _expand_cute(TileReduce(name="R", widths=(8, ), op="max"))
    _assert_parses_as_python(body)
    assert "__output = ct.max(__src)" in body


def test_tile_reduce_cute_masked_sum_preselects_identity_via_where():
    ...
    tasklet = _expand_cute_tasklet(TileReduce(name="R", widths=(4, 8), op="+", axis=1, has_mask=True))
    body = tasklet.code.as_string
    _assert_parses_as_python(body)
...
def test_tile_reduce_cute_masked_min_preselects_pos_inf_via_where():
    """Masked ``min`` reduction pre-selects ``+inf`` into masked lanes."""
    body, _ = _expand_cute(TileReduce(name="R", widths=(8, ), op="min", has_mask=True))
    _assert_parses_as_python(body)
    assert "ct.where(__mask, __src, float('inf'))" in body
    assert "ct.min(__masked_src)" in body


def test_tile_reduce_cute_masked_sum_fallback_blend(monkeypatch):
    ...
    body, _ = _expand_cute(TileReduce(name="R", widths=(8, ), op="+", has_mask=True))
    _assert_parses_as_python(body)
    assert "ct.where" not in body
...
def test_tile_reduce_cute_masked_prod_fallback_blend(monkeypatch):
...
def test_tile_reduce_cute_masked_min_without_where_raises(monkeypatch):
    """Masked ``min``/``max`` with ``ct.where`` absent raises (the ``inf*0``
    hazard makes the arithmetic blend unsafe — L-reduce-nomask)."""
    monkeypatch.setattr(_tile_reduce_mod, "_CT_HAS_WHERE", False)
    with pytest.raises(NotImplementedError):
        _expand_cute(TileReduce(name="R", widths=(8, ), op="min", has_mask=True))
    with pytest.raises(NotImplementedError):
        _expand_cute(TileReduce(name="R", widths=(8, ), op="max", has_mask=True))


def test_tile_merge_cute_primary_emits_ct_where():
    """Primary (CI default) TileMerge body is ``ct.where(__cond, __then, __else)``."""
    body, lang = _expand_cute(TileMerge(name="M", widths=(8, )))
    _assert_parses_as_python(body)
    assert body == "__output = ct.where(__cond, __then, __else)"
    assert lang == dace.dtypes.Language.Python


def test_tile_merge_cute_fallback_arith_blend_for_int(monkeypatch):
    ...
    body, _ = _expand_merge_cute_with_dtype(TileMerge(name="M", widths=(8, )), dace.int32)
    _assert_parses_as_python(body)
    assert "ct.where" not in body
...
def test_tile_merge_cute_fallback_float_raises(monkeypatch):
    """With ``ct.where`` absent and a float output dtype, TileMerge raises
    (``0.0 * inf = NaN`` would leak a non-finite unselected branch)."""
    monkeypatch.setattr(_tile_merge_mod, "_CT_HAS_WHERE", False)
    with pytest.raises(NotImplementedError):
        _expand_merge_cute_with_dtype(TileMerge(name="M", widths=(8, )), dace.float64)

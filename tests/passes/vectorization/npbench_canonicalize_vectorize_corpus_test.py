...
def _base(name):
    ...
    if name not in _BASE:
        c = _CORPUS[name]
        arrays, symbols = npbench.make_inputs(c)
        ref = npbench.reference_outputs(c, arrays)
        sdfg = npbench.fresh_sdfg(c)
        canonicalize(sdfg, validate=True)
        _BASE[name] = (sdfg, arrays, symbols, ref)
    return _BASE[name]
...
def test_npbench_corpus(name, phase):
    ...
    canon, arrays, symbols, ref = _base(name)
    sdfg = copy.deepcopy(canon)
    if phase == "canon_vec":
        _multidim_pass(name).apply_pass(sdfg, {})
    sdfg.validate()
    got = npbench.run_outputs(_CORPUS[name], sdfg, arrays, symbols)

...
def _icon_inputs(rng):
    return dict(
        e_bln=rng.random((NB_VAL, 3, NPROMA_VAL)),
        edge_idx=rng.integers(0, NPROMA_VAL, (NB_VAL, NPROMA_VAL, 3)).astype(np.int32),
        edge_blk=rng.integers(0, NB_VAL, (NB_VAL, NPROMA_VAL, 3)).astype(np.int32),
        z_kin_hor_e=rng.random((NB_VAL, NLEV_VAL, NPROMA_VAL)),
        z_ekinh=np.zeros((NB_VAL, NLEV_VAL, NPROMA_VAL)),
    )
...
def test_predicated_branch_body(kern, branch_mode):
    ...
    _run_compare(kern, _branch_inputs, BRANCH_PARAMS, branch_mode=branch_mode)

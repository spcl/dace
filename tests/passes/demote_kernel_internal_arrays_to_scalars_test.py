...
def _kernel_with_inner(inner: dace.SDFG, in_conns, out_conns) -> dace.SDFG:
    ...
    n = st.add_nested_sdfg(inner, dict.fromkeys(in_conns), dict.fromkeys(out_conns))
...
def test_kernel_internal_transient_demoted():
    ...
    s = inner.add_state('s')
    t1 = s.add_tasklet('t1', {'x': None}, {'y': None}, 'y = x + 1.0')
    t2 = s.add_tasklet('t2', {'x': None}, {'y': None}, 'y = x * 2.0')
...
def test_kernel_output_written_across_gpu_exit_not_demoted():
    ...
    t = st.add_tasklet('t', {'x': None}, {'y': None}, 'y = x')
...
def _build_block_reduction() -> dace.SDFG:
    ...
    wt = state.add_tasklet('writeout', {'inp': None}, {'out': None}, 'if i == 0: out = inp')
...
def test_cpp_tasklet_neighbor_not_demoted():
    ...
    tc = st.add_tasklet('cpp', {'inp': None}, {'outp': None}, 'outp = inp;', language=dtypes.Language.CPP)
    tp = st.add_tasklet('py', {'inp': None}, {'outp': None}, 'outp = inp')
...
def test_nested_sdfg_neighbor_does_not_crash():
    ...
    st = sdfg.add_state('m')
    e, x = st.add_map('k', dict(i='0:16'), schedule=GPU_DEVICE)
    t0 = st.add_tasklet('seed', {'x': None}, {'y': None}, 'y = x')
    n = st.add_nested_sdfg(inner, {'a': None}, {'b': None})
    buf = st.add_access('buf')

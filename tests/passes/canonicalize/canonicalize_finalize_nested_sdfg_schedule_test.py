...
def _build_nested_reduce_in_parallel_map():
    ...
    The outer map runs 4096 iterations, comfortably above
    ``compiler.cpu.parallel_min_work_per_region``, so the fork/join cost model keeps it parallel
    and the nested-parallelism rule is what the assertions below observe.
    """

...
# CI runs every converging kernel plus this sample of known-divergent ones, one per divergence
# family (node order, scan mint, descriptor churn, wcr mint, py_mod printing, npbench). The rest
# of ``_NOT_A_FIXED_POINT`` stays as documentation; grow this set to extend coverage.
_XFAIL_SAMPLE = {
    ('tsvc', 's1115_d_single'),
    ('tsvc', 's1119_d_single'),
    ('tsvc', 's118_d_single'),
...
def _cases(known: Dict[Tuple[str, str], str]):
    ...
    for suite, name in _KERNELS:
        if (suite, name) in known and (suite, name) not in _XFAIL_SAMPLE:
            continue

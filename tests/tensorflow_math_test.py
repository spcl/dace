...
outp_0 = tf.math.reduce_mean(inp, axis=[0, 2])
outp_2 = tf.math.reduce_mean(inp, axis=[-2, -1])
outp_4 = tf.math.reduce_mean(inp, axis=[0, -1], keepdims=True)
...
addn_test_0 = tf.math.add_n(inputs)

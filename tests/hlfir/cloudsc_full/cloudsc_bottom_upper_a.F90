...
FOE_DEWM_DT( PTARE ) = YDTHF%R2ES * ( &
     ...
     & YDTHF%R3IES*(1.0D0-FOEALFA(PTARE))*EXP(YDTHF%R3IES*(PTARE-YDCST%RTT)/(PTARE-YDTHF%R4IES)) &
...
!      ENDIF
  ...
  DO JL=KIDIA,KFDIA
    ...
    IF (ZQX(JL,JK,NCLDQR) > ZEPSEC) THEN
      ...
      IF (ZTP1(JL,JK) <= RTT .AND. ZTP1(JL,JK-1) > RTT) THEN
        ...
        IF (PRAINFRAC_TOPRFZ(JL) > 0.8D0) THEN
          LLRAINLIQ(JL) = .True.
        ELSE
          LLRAINLIQ(JL) = .False.
        ENDIF
  ...
  !----------------------------------------------------------------------
  ! ===============================================
  ! BOTTOM-UPPER-A: also drop 4.5 EVAPORATION section
  ! ===============================================
  ! BOTTOM-UPPER REPRODUCER: drop solver + flux + tendency
  ! (lines 3356-3710 of original cloudsc.F90), replace with
  ! a simple PCOVPTOT writeback so the output is meaningful.
  ! ========================================================
  DO JL=KIDIA,KFDIA

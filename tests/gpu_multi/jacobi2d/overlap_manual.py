...
def j_o_un(
    ...
    for rank in dace.map[0:size]:
        ...
        lA = dace.ndarray((lNy, N), dtype=A.dtype)
        lB = dace.ndarray((lNy, N), dtype=B.dtype)
        lAs = dace.ndarray((1, N), dtype=A.dtype)
        lAn = dace.ndarray((1, N), dtype=A.dtype)
        lBs = dace.ndarray((1, N), dtype=B.dtype)
        lBn = dace.ndarray((1, N), dtype=B.dtype)
        for i in dace.map[0:lNy]:
            lA[i, :] = A[rank * lNy + i, :]
            lB[i, :] = B[rank * lNy + i, :]

        for t in range(TSTEPS):
            # comp_boundary(lA, lB)
            # for i,j in dace.map[1:lNy+1, 0:N]:
...
if __name__ == "__main__":
    ...
    sdfg = j_o_un.to_sdfg(strict=True)
    ...
    # # lAs = np.zeros((number_of_gpus, n // number_of_gpus + 2, n), dtype=np_float)
    # # lAe = np.zeros((number_of_gpus, n // number_of_gpus + 2, n), dtype=np_float)
    # # lBb = np.zeros((number_of_gpus, n // number_of_gpus + 2, n), dtype=np_float)
    # # lBe = np.zeros((number_of_gpus, n // number_of_gpus + 2, n), dtype=np_float)

    # # lBi = np.zeros((number_of_gpus, n // number_of_gpus + 2, n), dtype=np_float)

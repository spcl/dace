...
def make_vecAdd_sdfg(symbol_name: str, sdfg_name:str, access_nodes_dict : dict, dtype = dace.float32):
    ...
    vecAdd_sdfg.add_array("device_x", shape=[n / vecWidth], dtype=vecType, storage=dace.dtypes.StorageType.FPGA_Global,
                   transient=True)
    vecAdd_sdfg.add_array("device_y", shape=[n / vecWidth], dtype=vecType, storage=dace.dtypes.StorageType.FPGA_Global,
                   transient=True)
    ...
    vecAdd_sdfg.add_array("device_z", shape=[n / vecWidth], dtype=vecType, storage=dace.dtypes.StorageType.FPGA_Global,
                   transient=True)
    ...
    copy_out_state.add_memlet_path(
        out_device, out_host,
        memlet=Memlet.simple(out_host, "0:{}/{}".format(n, vecWidth))
    )
    ...
    # vecAdd_sdfg.add_stream('x_stream', vecType, buffer_size=32,
    #                        storage=dace.dtypes.StorageType.FPGA_Local)
    # vecAdd_sdfg.add_stream('y_stream', vecType, buffer_size=32,
    #                        storage=dace.dtypes.StorageType.FPGA_Local)
    # vecAdd_sdfg.add_stream('z_stream', vecType, buffer_size=32,
    #                        storage=dace.dtypes.StorageType.FPGA_Local)
    #
    # x_stream_in = fpga_state.add_access('x_stream')
    # y_stream_in = fpga_state.add_access('y_stream')
    # z_stream_out = fpga_state.add_access('z_stream')
    ...
    vecAdd_tasklet = fpga_state.add_tasklet(
    ...
    # Connect memory to stream
    # fpga_state.add_memlet_path(x, x_stream_in, memlet=dace.Memlet.simple(
    #     x, "0:{}/{}".format(n, vecWidth)))
    # fpga_state.add_memlet_path(y, y_stream_in, memlet=dace.Memlet.simple(
    #     y, "0:{}/{}".format(n, vecWidth)))
    #
    # fpga_state.add_memlet_path(z_stream_out, z, memlet=dace.Memlet.simple(
    #     z, "0:{}/{}".format(n, vecWidth)))
...
def make_nested_sdfg_fpga():
    ...
    nested_sdfg = state2.add_nested_sdfg(to_nest, sdfg, {"v", "w"}, {"u"})

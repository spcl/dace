...
# When a NestedSDFG is created, the user can specify a mapping between internal symbols and their values, which could
# also be symbols in the parent SDFG. This mapping can be handled either through function argument
# (if nested SDFGs are generated as a function) or by explicitly aliasing code generated variables
# (e.g. `<type> nested_symbol = parent_symbol`).
...
def make_vecAdd_sdfg(dtype=dace.float32, vecWidth=4):
    ...
    vecAdd_sdfg.add_array('_x',
                          shape=[n / vecWidth],
                          dtype=vecType,
                          storage=dace.dtypes.StorageType.FPGA_Global)
    vecAdd_sdfg.add_array('_y',
                          shape=[n / vecWidth],
                          dtype=vecType,
                          storage=dace.dtypes.StorageType.FPGA_Global)
    vecAdd_sdfg.add_array('_res',
                          shape=[n / vecWidth],
                          dtype=vecType,
                          storage=dace.dtypes.StorageType.FPGA_Global)
...
def make_nested_sdfg_fpga():
    ...
    #nest the sdfg
    vecAdd_nested = fpga_state.add_nested_sdfg(vecAdd_node, sdfg, {"_x", "_y"},
                                               {"_res"}, {"n": "size"})
    ...
    fpga_state.add_memlet_path(x,
                               ...
                               dst_conn="_x",
                               memlet=Memlet.simple(
                                   x, "0:{}/{}".format(size, vecWidth)))
    fpga_state.add_memlet_path(y,
                               vecAdd_nested,
                               dst_conn="_y",
                               memlet=Memlet.simple(
                                   y, "0:{}/{}".format(size, vecWidth)))
    fpga_state.add_memlet_path(vecAdd_nested,
                               z,
                               src_conn="_res",
                               memlet=Memlet.simple(
                                   z, "0:{}/{}".format(size, vecWidth)))

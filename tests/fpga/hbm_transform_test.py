...
def _exec_hbmtransform(sdfg_source, assign, nest=False, num_apply=1, apply_to=None):
    sdfg = sdfg_source()
...
def create_multi_access_sdfg(name):
    ...
    @dace.program
    def sth(z: dace.float32[N], x: dace.float32[N], y: dace.float32[N], w: dace.float32[N], o1: dace.float32[N],
            o2: dace.float32[N]):
...
def validate_nd_sdfg(csdfg, m, n, divide_m=1, divide_n=1):
    A = np.zeros([divide_m * divide_n, n // divide_n, m // divide_m], np.float32)
    B = np.zeros([divide_m * divide_n, m // divide_m, n // divide_n], np.float32)
    Z = np.zeros([divide_m * divide_n, n // divide_n, m // divide_m], np.float32)
    expect = np.zeros([divide_m * divide_n, n // divide_n, m // divide_m], np.float32)
...
def test_axpy_unroll_mixed():
    csdfg, sdfg = _exec_hbmtransform(lambda: create_vadd_sdfg("axpy_mixed"), [("x", "DDR", "0"), ("y", "HBM", "0:2"),
                                                                              ("z", "HBM", "0:2")])
...
@xilinx_test(run_synthesis=False)
def test_nd_split():
    csdfg, sdfg = _exec_hbmtransform(lambda: create_nd_sdfg("nd_split"), [("x", "HBM", "0:10"), ("y", "HBM", "10:20"),
                                                                          ("z", "HBM", "20:30")])
...
def test_nd_split_inner():
    ...
    csdfg, sdfg = _exec_hbmtransform(lambda: create_nd_sdfg("nd_split_inner"), [("x", "HBM", "0:10"),
                                                                                ("y", "HBM", "10:20"),
                                                                                ("z", "HBM", "20:30")],
                                     apply_to=apply_to)
...
def test_gemv_blas_1():
    csdfg, sdfg = _exec_hbmtransform(lambda: create_gemv_blas_sdfg("gemv_1", 32), [("x", "HBM", "31:32"),
                                                                                   ("y", "HBM", "30:31"),
                                                                                   ("A", "HBM", "0:30")], True)
...
def test_gemv_blas_2():
    csdfg, sdfg = _exec_hbmtransform(lambda: create_gemv_blas_sdfg("gemv_2", 32), [("x", "HBM", "31:32"),
                                                                                   ("y", "HBM", "15:30"),
                                                                                   ("A", "HBM", "0:15")], True)
...
def test_multiple_applications():
    _, sdfg = _exec_hbmtransform(lambda: create_multi_access_sdfg("multi_access"), [("x", "HBM", "0:2"),
                                                                                    ("y", "HBM", "2:4"),
                                                                                    ("z", "HBM", "4:6"),
                                                                                    ("w", "HBM", "10:12"),
                                                                                    ("o1", "HBM", "6:8"),
...
def test_axpy_unroll_1():
    ...
    sdfg = _exec_hbmtransform(lambda: create_vadd_sdfg("axpy_unroll_1"), [("x", "DDR", "0"), ("y", "HBM", "0:1"),
                                                                          ("z", "DDR", "1")],
                              num_apply=0)
    sdfg.compile()  # We still have to compile for pytest, so the build folder exists
    return sdfg
...
def test_axpy_inconsistent_no_apply():
    sdfg = _exec_hbmtransform(lambda: create_vadd_sdfg("axpy_inconsistent"), [("x", "HBM", "0:2"), ("y", "DDR", "0"),
                                                                              ("z", "HBM", "0:3")],
                              num_apply=0)
    set_assignment(sdfg, [("x", "DDR", "0"), ("y", "HBM", "0:1"), ("z", "DDR", "1")])
    sdfg.compile()  # We still have to compile for pytest, so the build folder exists
    return sdfg

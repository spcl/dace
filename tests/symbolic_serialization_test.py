...
def test_pystr_to_symbolic_preserves_typed_symbols():
    ...
    plain_expr = symbolic.deserialize_symbolic('-1 + $N')
    plain_restored = symbolic.pystr_to_symbolic(plain_expr)
    typed = symbolic.deserialize_symbolic('-1 + symbol($N, dtype=dace.int64)')

    restored = symbolic.pystr_to_symbolic(typed)

    assert plain_restored is plain_expr
    assert symbolic.serialize_symbolic(plain_restored) == '-1 + $N'

...
def test(device: str):
    ...
    arrays.append(state.add_access('A_' + str(len(arrays))))
    state.add_edge(
        arrays[-2], None, arrays[-1], None, dace.memlet.Memlet.simple(arrays[-2], '5:N,2:N-2,N-10:N-7,1:N-1')
    )
    ...
    array_data = [
        np.random.rand(*[dace.symbolic.evaluate(s, {N: n}) for s in a.desc(sdfg).shape]).astype(a.desc(sdfg).dtype.type)
        for a in arrays
    ]
    ...
    diffs = [
        ...
        np.linalg.norm(array_data[13] - array_data[12][5:N, 2 : N - 2, N - 10 : N - 7, 1 : N - 1])
        / ((N - 5) * (N - 4) * 3 * (N - 2)),

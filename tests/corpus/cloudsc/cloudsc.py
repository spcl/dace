        ...
        for jl in range(kidia, kfdia + 1):
            ...
            zfoeewmt[jk - 1, jl - 1] = min(
                ...
                * (
                    ...
                    * np.exp(ydthf_r3les * (ztp1[jk - 1, jl - 1] - ydcst_rtt) / (ztp1[jk - 1, jl - 1] - ydthf_r4les))
                    + (
                        1.0
                        - min(
                            1.0,
                    ...
                    * np.exp(ydthf_r3ies * (ztp1[jk - 1, jl - 1] - ydcst_rtt) / (ztp1[jk - 1, jl - 1] - ydthf_r4ies))
                )
                / pap[jk - 1, jl - 1],
                0.5,
            )
            ...
            zfoeew[jk - 1, jl - 1] = min(
                (
                    ...
                    * (
                        ydthf_r2es
                        * np.exp(
                            ydthf_r3les * (ztp1[jk - 1, jl - 1] - ydcst_rtt) / (ztp1[jk - 1, jl - 1] - ydthf_r4les)
                        )
                    )
                    + (1.0 - zalfa)
                    * (
                        ydthf_r2es
                        * np.exp(
                            ydthf_r3ies * (ztp1[jk - 1, jl - 1] - ydcst_rtt) / (ztp1[jk - 1, jl - 1] - ydthf_r4ies)
                        )
                    )
                )
                / pap[jk - 1, jl - 1],
    ...
    for jk in range(yrecldp_ncldtop, klev + 1):
        ...
        for jl in range(kidia, kfdia + 1):
            if zqpretot[jl - 1] > zepsec:
                ...
                ) / (1.0 - min(za[jk - 1 - 1, jl - 1], 1.0 - 1e-06))
        ...
        for jl in range(kidia, kfdia + 1):
            if zfrzmax[jl - 1] > zepsec and zqxfg[ncldql - 1, jl - 1] > zepsec:
                ...
                zsolqa[ncldql - 1, imelt[ncldql - 1] - 1, jl - 1] = (
                    zsolqa[ncldql - 1, imelt[ncldql - 1] - 1, jl - 1] + zfrz
                )
                zsolqa[imelt[ncldql - 1] - 1, ncldql - 1, jl - 1] = (
                    zsolqa[imelt[ncldql - 1] - 1, ncldql - 1, jl - 1] - zfrz
                )
        if ievaprain == 1:
            for jl in range(kidia, kfdia + 1):

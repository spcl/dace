...
def test_mbconv(use_cpp_dispatcher: bool, device):

    dev = torch_device(device)

    with torch.no_grad():
        dace_inputs = torch.rand(8, 32, 224, 224).to(dev)
...
@pytest.mark.torch
@pytest.mark.parametrize("device", DEVICES)
def test_fast_mb(use_cpp_dispatcher: bool, device):
...
if __name__ == "__main__":
    test_mbconv(use_cpp_dispatcher=True, device="cpu")
    test_mbconv(use_cpp_dispatcher=False, device="cpu")
    test_fast_mb(use_cpp_dispatcher=True, device="cpu")
    test_fast_mb(use_cpp_dispatcher=False, device="cpu")

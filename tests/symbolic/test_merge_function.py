...
"""
Unit tests for the ``merge`` SymPy ``Function`` registered in
:mod:`dace.symbolic`.

``merge(c, a, b)`` is the symbolic stand-in for ``c ? a : b`` used by the
...
- ``SymExpr("merge(c, a, b)")`` parses and round-trips through
  ``DaceSympyPrinter``.
- Free-symbol collection returns the arg symbols, never ``merge`` itself.
- ``merge`` folds eagerly when the predicate is a SymPy boolean literal
...
def test_merge_parses_via_symexpr():
    expr = SymExpr("merge(c, a, b)")
...
def test_merge_round_trips_through_dace_sympy_printer():
    expr = SymExpr("merge(c, a, b)")
    printer = DaceSympyPrinter(arrays=None)
    rendered = printer.doprint(expr)
    # Default sympy function printing: "merge(c, a, b)" (commutative-arg order
    # is preserved because nargs=3 disables sympy's reorder).
    assert rendered.startswith("merge(")
    assert rendered.endswith(")")
    assert set(re.findall(r"\b[a-z]\b", rendered)) == {"c", "a", "b"}


def test_merge_free_symbols_are_args_only():
    expr = SymExpr("merge(c, a, b)")
    free = {str(s) for s in expr.free_symbols}
    assert free == {"a", "b", "c"}
    assert "merge" not in free


def test_merge_folds_when_predicate_is_true():
    a, b = sympy.symbols("a b")
    folded = merge(sympy.true, a, b)
    assert folded == a


def test_merge_folds_when_predicate_is_false():
    a, b = sympy.symbols("a b")
    folded = merge(sympy.false, a, b)
    assert folded == b


def test_merge_stays_symbolic_when_predicate_is_a_symbol():
...
def test_merge_subs_predicate_evaluates_lazily():
    """``merge(c, a, b).subs(c, sympy.true)`` should still collapse to ``a``,
...
def test_merge_nested():
    """Nested ``merge`` should not collapse when predicates remain symbolic."""
    c1, c2, a, b, d = sympy.symbols("c1 c2 a b d")
    expr = merge(c1, merge(c2, a, b), d)
    # Outer head is still ``merge``.
    assert isinstance(expr, merge)
    # The inner ``merge`` is in the argument tree.
    found_inner = any(isinstance(arg, merge) for arg in expr.args)
    assert found_inner
...
def test_merge_header_exists_and_declares_template():
    ...
    assert re.search(r"merge\s*\(\s*bool\s+\w+\s*,\s*T\s+\w+\s*,\s*T\s+\w+\s*\)", text), text


def test_merge_compiled_in_sdfg_matches_python_reference():
    """End-to-end: build an SDFG whose tasklet body calls ``merge(c, a, b)``,
    compile, run, and compare against the plain-Python ternary for every
    combination of ``c`` and a small set of operand values."""
    sdfg = dace.SDFG("merge_e2e")
    ...
    t = state.add_tasklet("m", {"_a", "_b"}, {"_o"}, "_o = merge(c, _a, _b)")
...
def test_merge_recognized_under_alternate_capitalization():
    """SymExpr accepts both lowercase ``merge`` and capitalised ``Merge``
    (registered as an alias) so callers don't have to guess casing."""
    e1 = SymExpr("merge(c, a, b)")
    e2 = SymExpr("Merge(c, a, b)")
    assert isinstance(e1, merge)
    assert isinstance(e2, merge)

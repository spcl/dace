...
PROGRAM CLOUDPROGRAM
  ...
  INTEGER(KIND=JPIM),PARAMETER    :: NPROMA =128
   INTEGER(KIND=JPIM),PARAMETER    :: NGPTOT =65536
...
REAL(KIND=JPRB) :: DEBUG_EPSILON(2)
...
& NSHAPEQ,NBETA,RBETA,RBETAP1, DEBUG_EPSILON)
...
INTEGER(KIND=JPIM) :: NCLDQL    ! liquid cloud water
INTEGER(KIND=JPIM) :: NCLDQI   ! ice cloud water
INTEGER(KIND=JPIM) :: NCLDQR    ! rain water
INTEGER(KIND=JPIM) :: NCLDQS    ! snow
INTEGER(KIND=JPIM) :: NCLDQV    ! vapour
...
DEBUG_EPSILON(1) = 100.*EPSILON(PLUDE(1, 1, 1))
DEBUG_EPSILON(2) = 100.*EPSILON(PTSPHY)
  ...
  DO IBL=1,512
      ...
      CALL CLOUDSC(1,    128,    KLON,    KLEV,&
      !CALL CLOUDSC(1,    32,    KLON,    KLEV,&
      & PTSPHY,&
      & PT(:,:,IBL), PQ(:,:,IBL),&
...
! end old limiter
  ...
  DO JL=KIDIA,KFDIA
    ...
    IF(ZDQS(JL) <= -RLMIN .AND. ZA(JL,JK)<1.0_JPRB-ZEPSEC) THEN
      ...
      ! Increase RHcrit to 1.0_JPRB towards the surface (eta>0.8)
...
!      ! Increase RHcrit to 1.0_JPRB towards the tropopause (trop-0.2) and above
!      ZBOTT=ZTRPAUS(JL)+0.2
!      IF(ZSIGK < ZBOTT) THEN
!        ZRHC=RAMID+(1.0_JPRB-RAMID)*MIN(((ZBOTT-ZSIGK)/0.2)**2,1.0_JPRB)
!      ENDIF
  ...
  DO JL=KIDIA,KFDIA
    ...
    ! defined by cloudy layer below a layer with cloud frac <0.0_JPRB1
...
!      ZDEPOS = ZDEPOS*MIN(RDEPLIQREFRATE+ZCLDTOPDIST(JL)/RDEPLIQREFDEPTH,1.0_JPRB)
      ...
      ZINFACTOR = MIN(ZICENUCLEI(JL)/15000., 1.0_JPRB)
...
!       &ZCOVPTOT(JL)/(MAX(ZA(JL,JK),ZEPSEC))
      ...
      IF (LAERLIQCOLL) THEN 
        ! 5.0_JPRB = N**0.333 with N=125 cm-3 
  ...
  DO JL=KIDIA,KFDIA
    IF(ZTP1(JL,JK) <= RTT .AND. ZLIQCLD(JL)>ZEPSEC) THEN
     ...
     &                  *(ZRHO(JL)*ZSNOWCLD(JL)*RCL_CONST1S)**RCL_CONST8S
      ...
      ! NOT YET ACTIVE
      !------------------------------------------------------------------
!      IF (ZICECLD(JL)>ZEPSEC .AND. ZA(JL,JK)>0.0_JPRB1) THEN
!
!        ! Calculate riming term
!        ! Factor of liq water taken out because implicit
...
!      ENDIF
 ...
 ELSEIF (IEVAPRAIN == 2) THEN
  ...
  DO JL=KIDIA,KFDIA
    ...
    ! Increase RHcrit to 1.0_JPRB towards the surface (eta>0.8)
    !IF(ZSIGK > 0.8) THEN
    !  ZRHC=RAMID+(1.0_JPRB-RAMID)*((ZSIGK-0.8)/0.2)**2
    !ENDIF
    !ZZRH = MIN(ZRHC,ZZRH)
  ...
  ELSEIF (IEVAPSNOW == 2) THEN
   ...
   DO JL=KIDIA,KFDIA
    ...
    IF(LLO1) THEN
      ...
      ZTCG=1.0_JPRB !v1 EXP(RCL_X3I*(273.15-ZTP1(JL,JK))/8.18)
      ! ZFACX1I modification is based on Andrew Barrett's results
      ZFACX1S = 1.0_JPRB !v1 (ZICE0/1.E-5)**0.627

...
def test():
    ...
    myextSDFG=SDFG('extSDFG')
    ensdfg=state.add_nested_sdfg(None, mysdfg, {'input'}, {'output'},name='extSDFG')

    # Add edges
    state.add_memlet_path(A, map_entry, ensdfg, dst_conn='input', memlet=Memlet.simple(A, 'i,j'))
    state.add_memlet_path(ensdfg, map_exit, B, src_conn='output', memlet=Memlet.simple(B, 'i,j'))

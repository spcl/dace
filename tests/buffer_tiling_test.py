...
def conv3x3(weights: dace.float32[3, 3], A: dace.float32[I, J], B : dace.float32[I, J]):
    ...
    def conv3x3(y: _[1:I-1], x: _[1:J-1]):
        ...
        w << weights
        out >> B[y, x]
        out = (w[0, 0] * inp[0, 0] + w[0, 1] * inp[0, 1] +
               w[0, 2] * inp[0, 2] + w[1, 0] * inp[1, 0] +
               w[1, 1] * inp[1, 1] + w[1, 2] * inp[1, 2] +
               w[2, 0] * inp[2, 0] + w[2, 1] * inp[2, 1] +
               w[2, 2] * inp[2, 2])
...
def conv5x5(weights: dace.float32[5, 5], A: dace.float32[I, J], B : dace.float32[I, J]):
    ...
    def conv5x5(y: _[3:I-3], x: _[3:J-3]):
        ...
        out >> B[y, x]
        out = (w[0, 0] * inp[0, 0] + w[0, 1] * inp[0, 1] +
               w[0, 2] * inp[0, 2] + w[0, 3] * inp[0, 3] +
               w[0, 4] * inp[0, 4] + w[1, 0] * inp[1, 0] + 
               w[1, 1] * inp[1, 1] + w[1, 2] * inp[1, 2] + 
               w[1, 3] * inp[1, 3] + w[1, 4] * inp[1, 4] +
               w[2, 0] * inp[2, 0] + w[2, 1] * inp[2, 1] + 
               w[2, 2] * inp[2, 2] + w[2, 3] * inp[2, 3] + 
               w[2, 4] * inp[2, 4] + w[3, 0] * inp[3, 0] + 
               w[3, 1] * inp[3, 1] + w[3, 2] * inp[3, 2] + 
               w[3, 3] * inp[3, 3] + w[3, 4] * inp[3, 4] +
               w[4, 0] * inp[4, 0] + w[4, 1] * inp[4, 1] + 
               w[4, 2] * inp[4, 2] + w[4, 3] * inp[4, 3] + 
               w[4, 4] * inp[4, 4])
...
@dace.program
def conv3x3_5x5_transposed(w3: dace.float32[3, 3], w5: dace.float32[5, 5], A: dace.float32[I, J], B: dace.float32[I, J]):
    buf = dace.ndarray([I, J], dtype=dace.float32)
...
class BufferTilingTest(unittest.TestCase):
    ...
    def test_semantic_eq_transposed(self):
        self.semantic_eq([3,3], conv3x3_5x5_transposed)

    def test_semantic_eq_tile_size_1(self):
        self.semantic_eq([1,1], conv3x3_5x5)

    def test_semantic_eq_tile_size_1_transposed(self):
        self.semantic_eq([1,1], conv3x3_5x5_transposed)
